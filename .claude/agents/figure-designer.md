...
description: >-
  ...
  budget (per-chapter image budget; designed diagrams authored as HTML then rendered to PNG (never image-generated); tool/IDE screenshots captured): typically 1–2 designed conceptual diagrams; zero is
  ...
  Every label/arrow/rule IDs, config/ruleset keys, tool flags, API signatures, GAV coordinates, version numbers, benchmark figures, and quoted claims/number traces to the pinned authority set (00-strategy/SOURCE-PIN.md) at
  the pins in SOURCE-PIN.md.
...
(`per-chapter image budget; designed diagrams authored as HTML then rendered to PNG (never image-generated); tool/IDE screenshots captured`): typically 1–2 designed conceptual diagrams plus 0–4 captured native-tool
...
`the per-tool fetch dirs in SOURCE-PIN.md` (`the pins in SOURCE-PIN.md`) is EPHEMERAL — if absent or off-pin, run
...
3. **Author the HTML** at `05-figures/NN_slug/figNN_x.html`: a `#figure` root element (the crop
   ...
   real labels, real rule IDs, config/ruleset keys, tool flags, API signatures, GAV coordinates, version numbers, benchmark figures, and quoted claims, real numbers, typeset directly in the markup. This is the
...
4. **Write the trace sidecar** `05-figures/NN_slug/figNN_x.sources.md`: a table mapping every label,
   arrow, atom, and number in the diagram to a pinned `the pinned authority set (00-strategy/SOURCE-PIN.md)` source path, ending with
...
1. **neutral comparative survey (see 00-strategy/NEUTRALITY.md): each option gets its strongest case and its hardest limitation; no crowning; banned: better than / unlike X / superior / beats / the problem with X.** No rival logos or branding unless the chapter is a direct comparison or
   migration topic — then minimal, neutral, never crowning a winner; any cross-subject element needs
   a cited `the pinned authority set (00-strategy/SOURCE-PIN.md)` source. No banned comparative wording ("better than", "unlike X",
   "the problem with X", "superior", "beats").
2. **Never invent** a rule IDs, config/ruleset keys, tool flags, API signatures, GAV coordinates, version numbers, benchmark figures, and quoted claims atom, layering, or flow in the diagram. Because the
   text is authored EXACT and baked in, every depicted technical claim MUST tie to a pinned-source
   path (verify with the **source-verify** skill against `the per-tool fetch dirs in SOURCE-PIN.md`). Anything
...
- **`figNN_x.sources.md`** — the source-trace sidecar: a `| Label in diagram | Claim | Pinned source
  (the pins in SOURCE-PIN.md) |` table covering every label/arrow/atom/number, ending with

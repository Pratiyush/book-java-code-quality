...
1. **Author alt-text for every figure (the short equivalent).** One tight sentence that states what
   ...
   you name (a `rule IDs, config/ruleset keys, tool flags, API signatures, GAV coordinates, version numbers, benchmark figures, and quoted claims` atom, a phase, an API) must match the `figNN_x.html` source exactly
...
2. **Author the long-description (the full content).** For any diagram that carries spatial, flow,
   ...
   chapter's `the locked voice in 00-strategy/VOICE-GUIDE-JAVA-QUALITY.md`; describe structure, not decoration.
...
- **Respect the pin and neutrality.** Every fact your alt-text asserts is a `the pins in SOURCE-PIN.md` fact;
  an untraceable label is flagged to `09-flags/`, not described as fact. No
  `neutral comparative survey (see 00-strategy/NEUTRALITY.md): each option gets its strongest case and its hardest limitation; no crowning; banned: better than / unlike X / superior / beats / the problem with X`-banned wording enters an alt-text or long-description.

...
`_RECONCILE.md`, and — technical profile, when research, source-verify, example-build + code-review + repro, verify, clarity, audit, score, reconcile, human-approve, assemble includes example-build/code-review —
`_EXAMPLE.md`, `_CODEREVIEW.md`) and the figure file when one exists (figures follow the per-chapter
image budget, `per-chapter image budget; designed diagrams authored as HTML then rendered to PNG (never image-generated); tool/IDE screenshots captured` — load-bearing but no longer rare; zero is the exception). The rule
hierarchy lives in `00-strategy/GUIDELINES.md`; the entry format for learnings lives in
`PIPELINE-LEARNINGS.md` — follow it.

*(technical profile — see BOOK-TYPE-PROFILES.md; book types without research, source-verify, example-build + code-review + repro, verify, clarity, audit, score, reconcile, human-approve, assemble example-build drop
...
2. **Update `LEDGER.md`** — §1 the live state (the single source of live state — every other doc
   ...
   rule IDs, config/ruleset keys, tool flags, API signatures, GAV coordinates, version numbers, benchmark figures, and quoted claims atom, the pin, terminology decision — recorded so the reconciler can enforce it),
...
4. **ERRATA-INTAKE (PIPELINE Step 13a — living book, soft).** After a chapter approves the book is a
   **living artifact**: own `09-flags/ERRATA.md` as the intake log for **reader-reported errata** and
   ...
   `the pinned authority set (00-strategy/SOURCE-PIN.md)` edition/release that ages out `the pins in SOURCE-PIN.md` — this is where you queue
   ...
   (an `rule IDs, config/ruleset keys, tool flags, API signatures, GAV coordinates, version numbers, benchmark figures, and quoted claims` atom, the pin) updates `LEDGER.md` §2 the same as any other fact.
...
- **Record truth, never invent it.** Status must match the actual gate verdicts; a canonical fact
  ...
  ledger. Do not promote a fact you cannot trace to the `the pins in SOURCE-PIN.md` pin.
...
- **Tooling honesty.** Distinguish what is automated (a wired script/agent) from what still runs
  as an assisted procedure. Once the repo is under git (remote `{URL}`): one commit per
...
- Never write any residue-blocklist term into any file (the retired scoring vocabulary listed in
  `SCORING.md`). Keep the lean five-cluster + floors model (`A NEUTRALITY / B HONEST-LIMITATIONS / C SOURCE-TRACE + COMPILE + CODE-REVIEW` — FLOOR A NEUTRALITY ·

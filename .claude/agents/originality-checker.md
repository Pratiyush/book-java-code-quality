...
the openly-licensed `the pinned authority set (00-strategy/SOURCE-PIN.md)` and the local copyrighted `_ref/` corpus. **You cover
...
2. **Select the distinctive spans.** A web scan of every sentence is noise; scan the spans most
   ...
   `rule IDs, config/ruleset keys, tool flags, API signatures, GAV coordinates, version numbers, benchmark figures, and quoted claims` atoms, and short common phrasings of the subject are out of scope — a
   `rule IDs, config/ruleset keys, tool flags, API signatures, GAV coordinates, version numbers, benchmark figures, and quoted claims` atom matching the source is correct, not plagiarism (that is VERIFY's domain).
...
- **Stay in your lane.** Fact-tracing is VERIFY's; the `_ref/`-corpus + voice + authenticity read is
  ...
  re-adjudicate. Respect the `the pins in SOURCE-PIN.md` pin — a `rule IDs, config/ruleset keys, tool flags, API signatures, GAV coordinates, version numbers, benchmark figures, and quoted claims` atom that matches the

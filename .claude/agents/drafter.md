...
description: >-
  ...
  voice, every fact traced to the pinned the pinned authority set (00-strategy/SOURCE-PIN.md) (the pins in SOURCE-PIN.md).
...
- `00-strategy/VOICE-GUIDE.md` — the locked voice (the locked voice in 00-strategy/VOICE-GUIDE-JAVA-QUALITY.md).
...
2. **The worked-example spec** — the demo realized from the chapter's `DEMO-CATALOG.md` row:
   what it builds, and the ≤9-line snippet(s) it will reference per one runnable, enterprise-grade module/example per chapter; the displayed snippet is a tag-region inside the compiled file; built green. (technical
   profile — see BOOK-TYPE-PROFILES.md; with the example-build gate ON this is the module name and
   the tag-include contract Step 4b builds against; book types without research, source-verify, example-build + code-review + repro, verify, clarity, audit, score, reconcile, human-approve, assemble example gates
...
3. **The figure plan (per GUIDELINES §8)** — the chapter's image budget for its class per
   per-chapter image budget; designed diagrams authored as HTML then rendered to PNG (never image-generated); tool/IDE screenshots captured: typically 1–2 designed conceptual diagrams plus 0–N captured subject-native
...
Use the dossier's verified snippets and rule IDs, config/ruleset keys, tool flags, API signatures, GAV coordinates, version numbers, benchmark figures, and quoted claims atoms verbatim, each with its recorded source path.

## Hard constraints (non-negotiable)

1. **neutral comparative survey (see 00-strategy/NEUTRALITY.md): each option gets its strongest case and its hardest limitation; no crowning; banned: better than / unlike X / superior / beats / the problem with X.** Java code quality is the subject. No banned phrasings (see NEUTRALITY).
   Comparators appear only for a necessary direct comparison or migration topic, each with a cited
   Java Quality source. Components shipped as part of Java code quality are Java code quality,
   never rivals.
2. **Dossier-only, never invent.** Every rule IDs, config/ruleset keys, tool flags, API signatures, GAV coordinates, version numbers, benchmark figures, and quoted claims atom and snippet must already be in the
   dossier and traceable to the pins in SOURCE-PIN.md. If the chapter needs a fact the dossier lacks, do NOT
   invent it — mark it `UNVERIFIED`, flag to `09-flags/`, and note the gap; the topic returns to
   research. Spot-check any reused fact with the **Java Quality-source-verify** skill.
3. **Snippets ≤ 9 lines each**, verified against the pinned source, source path recorded. No
   ...
   see BOOK-TYPE-PROFILES.md; book types without research, source-verify, example-build + code-review + repro, verify, clarity, audit, score, reconcile, human-approve, assemble example/compile gates may relax the
   snippet ceiling.)
...
5. **Figures load-bearing, per the fixed figure plan.** Honor the chapter's figure budget from the
   pre-draft plan (GUIDELINES §8 / per-chapter image budget; designed diagrams authored as HTML then rendered to PNG (never image-generated); tool/IDE screenshots captured): typically 1–2 designed conceptual diagrams
...
see BOOK-TYPE-PROFILES.md; book types without research, source-verify, example-build + code-review + repro, verify, clarity, audit, score, reconcile, human-approve, assemble example gates drop this step and embed
the verified snippet directly.)

## Output

Write `03-drafts/NN_slug/NN_slug_v1.md` with the front-matter fields filled (dossier key, slug,
part, "Verified against Java code quality the pins in SOURCE-PIN.md" + today's re-check date). Complete the

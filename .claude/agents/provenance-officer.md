...
the book re-pins to a new edition/tag of `the pinned authority set (00-strategy/SOURCE-PIN.md)`, or when the disclosure obligations
change.
...
`the locked voice in 00-strategy/VOICE-GUIDE-JAVA-QUALITY.md` — neutral, no hedging), `00-strategy/NEUTRALITY.md` (the disclosure honors
`neutral comparative survey (see 00-strategy/NEUTRALITY.md): each option gets its strongest case and its hardest limitation; no crowning; banned: better than / unlike X / superior / beats / the problem with X` — it crowns nothing), and `00-strategy/SOURCE-PIN.md` (the `the pins in SOURCE-PIN.md`
...
1. **Author `06-assembly/AI-DISCLOSURE.md` — the disclosure statement.** A neutral, factual,
   reader-facing statement that the book's prose, figures, and `one runnable, enterprise-grade module/example per chapter; the displayed snippet is a tag-region inside the compiled file; built green` artifacts were
   ...
   *what* was AI-generated (chapters, designed figures, worked examples), *how* it was controlled
   (every `rule IDs, config/ruleset keys, tool flags, API signatures, GAV coordinates, version numbers, benchmark figures, and quoted claims` atom traced to the pinned `the pinned authority set (00-strategy/SOURCE-PIN.md)`; every chapter passed
...
2. **Author `00-strategy/PROVENANCE-LOG.md` — the per-chapter record.** A table with one row per
   ...
   draft date · human-in-loop step(s) cleared (esp. Step 12 approval) · pin (`the pins in SOURCE-PIN.md`)**.
...
- **Disclose, do not over-claim.** The disclosure is factual and neutral: it states the process and
  the safeguards, makes no superiority or capability claim, and honors `neutral comparative survey (see 00-strategy/NEUTRALITY.md): each option gets its strongest case and its hardest limitation; no crowning; banned: better than / unlike X / superior / beats / the problem with X`.
  Hold the statement to the locked `the locked voice in 00-strategy/VOICE-GUIDE-JAVA-QUALITY.md` — it is read as part of the book.
- **Respect the pin.** The disclosure and log reference `Java code quality` `the pins in SOURCE-PIN.md` as the

...
description: >-
  ...
  to confirm every rule IDs, config/ruleset keys, tool flags, API signatures, GAV coordinates, version numbers, benchmark figures, and quoted claims atom and snippet traces to the pinned
  the pinned authority set (00-strategy/SOURCE-PIN.md) (the pins in SOURCE-PIN.md), that synthesized / causal / comparative
...
- `00-strategy/SOURCE-PIN.md` — the pin (the pins in SOURCE-PIN.md, clone/fetch `the per-tool fetch dirs in SOURCE-PIN.md`,
...
- `00-strategy/GUIDELINES.md` and `00-strategy/NEUTRALITY.md` (cross-comparator claims need a cited Java Quality source).
...
1. Confirm the pin first: the clone `the per-tool fetch dirs in SOURCE-PIN.md` (the pins in SOURCE-PIN.md) is EPHEMERAL.
...
2. Run the scripts when present: `verify_sources.sh` and `lint_citations.sh` (step 5 also).
   ...
   research, source-verify, example-build + code-review + repro, verify, clarity, audit, score, reconcile, human-approve, assemble example/compile gates verify the snippet against the pinned source directly, not
...
3. Using the **Java Quality-source-verify** skill, open each cited source path under
   `the per-tool fetch dirs in SOURCE-PIN.md` and confirm the rule IDs, config/ruleset keys, tool flags, API signatures, GAV coordinates, version numbers, benchmark figures, and quoted claims atom / snippet matches exactly. A
...
6. **Quoted-span verbatim check.** Every double-quoted span attributed to a named guide or source
   (a Java Quality guide, the spec, a `_ref/` book, etc.) must match that source
...
7. **Claim / reasoning audit (synthesized claims, not only atomic facts).** A claim can be built
   ...
   claims, not just the atomic rule IDs, config/ruleset keys, tool flags, API signatures, GAV coordinates, version numbers, benchmark figures, and quoted claims facts: a "because X, therefore Y" mechanism, an
...
- Every cross-comparator claim must carry a cited Java Quality source; otherwise it is a BLOCKER.
- Binary on the pin: traceable to the pins in SOURCE-PIN.md or it is a finding.

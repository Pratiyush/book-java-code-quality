...
description: >-
  ...
  reference terms a reader looks up — rule IDs, config/ruleset keys, tool flags, API signatures, GAV coordinates, version numbers, benchmark figures, and quoted claims atoms, named concepts,
...
the pin discipline) and `SOURCE-PIN.md` (the `the pins in SOURCE-PIN.md` pin). Read whole:
...
1. **Mine the manuscript for index-worthy terms.** Walk `06-assembly/` and collect candidate
   ...
   are this book's `rule IDs, config/ruleset keys, tool flags, API signatures, GAV coordinates, version numbers, benchmark figures, and quoted claims` (the things that must never be invented — they are exactly the
...
- **Include:** `rule IDs, config/ruleset keys, tool flags, API signatures, GAV coordinates, version numbers, benchmark figures, and quoted claims` atoms, named concepts and patterns, and proper-noun components —
...
- **Index only what the manuscript carries.** Never invent an entry, a locator, an `rule IDs, config/ruleset keys, tool flags, API signatures, GAV coordinates, version numbers, benchmark figures, and quoted claims`
...
- Respect the **`the pins in SOURCE-PIN.md` pin** — every atom you index must already be a pinned fact in the

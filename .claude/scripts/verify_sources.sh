...
#          book types without the build/compile gate turned off=example-build keep them, others can
...
#
#   * identifier keys   — e.g. config-key names (rule IDs, config/ruleset keys, tool flags, API signatures, GAV coordinates, version numbers, benchmark figures, and quoted claims)
#   * version strings    — x.y.z literals (must be the pinned version, not drift)
#   * dependency coords  — e.g. Maven GAVs (rule IDs, config/ruleset keys, tool flags, API signatures, GAV coordinates, version numbers, benchmark figures, and quoted claims)

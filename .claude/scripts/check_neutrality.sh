...
#
# USED BY: ALL book types that carry a neutrality stance (neutral comparative survey (see 00-strategy/NEUTRALITY.md): each option gets its strongest case and its hardest limitation; no crowning; banned: better than / unlike X / superior / beats / the problem with X).
...
BANNED='better than|unlike [A-Z]|superior|beats|the problem with '   # e.g. 'better than|unlike [A-Z]|the problem with|is superior|are superior|beats [A-Z]|cleaner than|more elegant than|faster than [A-Z]'
...
FILLER_REGEX='\b(very|really|just|simply|basically|actually|obviously)\b'     # e.g. '(\<easy\>|\<easily\>|\<just\>|\<simply\>|\<obviously\>|of course|\<simple\>)'
FILLER_MAX=0           # e.g. 8   (over this many -> advisory FLAG)
# 3. EM-DASH ceiling per 1,000 words (the VOICE rhythm cap). Over it -> FLAG.
EMDASH_PER_1000=8 # e.g. 8
...
#    scope — your neutral comparative survey (see 00-strategy/NEUTRALITY.md): each option gets its strongest case and its hardest limitation; no crowning; banned: better than / unlike X / superior / beats / the problem with X buckets). Extended-regex alternation;
#    leave EMPTY for a book with no rival subjects (the heading scan then checks
#    only the generic comparative words below).
RIVAL_NAMES='__NO_SINGLE_RIVAL__' # e.g. 'spring|micronaut|helidon|jakarta ee'

...
- `@Property` (in place of `@Test`) with `@ForAll` parameters: jqwik **generates** values, by default 1,000 tries per property (the `tries` attribute, overridable per property or globally).
...
- **Integration/Testcontainers** (`java.testcontainers.org`; `github.com/testcontainers/testcontainers-java`): real-dependency integration vs mock-drift / in-memory-substitute infidelity; `GenericContainer` / `PostgreSQLContainer` / `KafkaContainer`; JUnit 5 `@Testcontainers` + `@Container` (instance = per-test, static = per-class shared); module ecosystem (DB/Kafka/RabbitMQ/Elasticsearch/LocalStack/Selenium); reusable containers (opt-in via `withReuse(true)` / `testcontainers.reuse.enable=true`, **experimental and not suited for CI usage** per the docs and Ch 20) + Ryuk resource-reaper cleanup (the sidecar that removes dead containers at JVM shutdown; disabled with the `TESTCONTAINERS_RYUK_DISABLED` env var); HARD Docker-runtime dependency (dev + CI); test slices (Spring `@DataJpaTest`, Quarkus `@QuarkusTest`) cited neutrally; fixture quality (real API/factories, minimal). Pyramid middle (Ch 20). GAV `org.testcontainers:testcontainers` + `:testcontainers-junit-jupiter` + module (e.g. `:testcontainers-postgresql`), test scope. *(model/JUnit5 verified; **version 2.0.5 verified** against SOURCE-PIN §3 and resolved on Maven Central (`org.testcontainers:testcontainers:2.0.5`). **`@Container` per-test-vs-static semantics + reuse/Ryuk flags verified** at the Testcontainers docs (`java.testcontainers.org`, dated-at-use 2026-06-28, rolling docs): a `@Container` **instance** field is "started and stopped for every test method"; a **static** field is "shared between test methods … started only once before any test method is executed and stopped after the last"; reuse is opt-in/experimental/not-for-CI; Ryuk = resource reaper, disabled via `TESTCONTAINERS_RYUK_DISABLED`. **GAV correction:** the 2.0 line renamed the module artifacts with a `testcontainers-` prefix — `:junit-jupiter`→`:testcontainers-junit-jupiter` and `:postgresql`→`:testcontainers-postgresql` (each resolves at `2.0.5` on Central; the un-prefixed names stop at `1.21.4` and do NOT exist at 2.0.5). Docker-gating resolved: cited-not-built — integration realized in-JVM, flagged in `09-flags/45_testcontainers_docker_gated_not_built.md`.)*
- **Parameterized/property-based** (⚠ approaches, crown none) — JUnit Jupiter `@ParameterizedTest` + `@ValueSource`/`@CsvSource`/`@EnumSource`/`@MethodSource`/`@ArgumentsSource` (`docs.junit.org`; `junit-jupiter-params`). jqwik (`jqwik.net`; `github.com/jqwik-team/jqwik`): `@Property` + `@ForAll`, `Arbitraries`/`@Provide`, shrinking to minimal counterexample, own `TestEngine` on the JUnit Platform, `@BeforeProperty`/`@AfterProperty`, `jqwik-testcontainers`. Invariant shapes: round-trip `parse(format(x))==x` / commutativity / idempotence / never-throws-on-valid. GAV `net.jqwik:jqwik`, test scope. *(model verified; **jqwik version 1.10.1 verified** against SOURCE-PIN §3 and resolved on Maven Central (`net.jqwik:jqwik:1.10.1`). **Default `@Property` tries = 1,000 verified** at the jqwik user guide (`jqwik.net/docs/current/user-guide.html`, version 1.10.1): "jqwik will run 1000 tries … unless you override the `tries` attribute"; `Arbitraries`/`@Provide` are the documented generator-description API. The JUnit param-source annotations the build exercises (`@ParameterizedTest`/`@ValueSource`/`@CsvSource`/`@MethodSource`) are confirmed unchanged + green under JUnit 6. **jqwik maintenance-mode (2026)** verified against SOURCE-PIN §3 — tool-vitality caveat, stated plainly, crown none.)*

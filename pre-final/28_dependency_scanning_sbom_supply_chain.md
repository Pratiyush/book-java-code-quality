...
```xml
            <configuration>
              <failBuildOnCVSS>7.0</failBuildOnCVSS>                 <suppressionFiles>
...
```xml
  <suppress>
    <notes><![CDATA[
      Reviewed false positive (2026-06-26, on-call + security review). Fuzzy CPE matching flagged a
      finding whose affected code path is not present in this artifact, so it is not exploitable here.
      Scoped to one coordinate AND one named finding, not a blanket rule. Re-review on the next bump.
    ]]></notes>
    <packageUrl regex="true">^pkg:maven/org\.apache\.commons/commons-lang3@.*$</packageUrl>
    <vulnerabilityName>ILLUSTRATIVE-FP-PLACEHOLDER</vulnerabilityName>
  </suppress>
```
...
```xml
        <configuration>
          <schemaVersion>1.6</schemaVersion>                    <projectType>application</projectType>
          <outputFormat>json</outputFormat>                     <outputName>bom</outputName>                           <includeBomSerialNumber>true</includeBomSerialNumber>
        </configuration>
```
...
And increasingly this is not only an engineering concern. As of 2026, regulatory drivers have pushed SBOMs toward a *compliance expectation* in some sectors, moving supply-chain hygiene from a purely voluntary practice toward a regulated one. In the United States, Executive Order 14028 (*Improving the Nation's Cybersecurity*, signed 12 May 2021), §4(e), directs federal agencies to require an SBOM from software vendors. In the European Union, the Cyber Resilience Act (Regulation (EU) 2024/2847, in force 10 December 2024, with its main obligations applying from 11 December 2027) requires manufacturers of products with digital elements to draw up an SBOM "in a commonly used and machine-readable format covering at the very least the top-level dependencies" (Annex I, Part II); it mandates no specific format, so CycloneDX or SPDX both serve. Which products and sectors are covered, which obligations attach, and on what timeline all vary by jurisdiction and are still settling. Treat the above as a *factual signal of direction, dated to the texts cited, and not legal advice*: confirm the requirements that currently apply to a given product with counsel.
...
- **Supply chain / SBOM** (key 66, ⚠ SPDX/CycloneDX) — supply-chain attacks target what you ship/build, not what you write. SBOM = complete component inventory → "am I affected?" in minutes. Formats: **CycloneDX** (OWASP, security-focused) / **SPDX** (Linux Foundation, **ISO/IEC 5962:2021**, licensing/provenance). Generate: Maven/Gradle CycloneDX plugins or **Syft** (both); consume: Dependency-Track. Provenance: **in-toto** attestation + **Sigstore cosign** signing + CI OIDC keyless identity, verified at deploy. **SLSA** = tiered build-integrity framework (Build L0–L3, roadmap not tool). Pipeline: build→SBOM→scan→sign+attest→publish→re-scan. Compliance (dated, factual, NOT legal advice): **US EO 14028** §4(e) (signed 2021-05-12) directs agencies to require vendor SBOMs; **EU CRA** (Regulation (EU) 2024/2847, in force 2024-12-10, main obligations 2027-12-11) requires an SBOM "covering at the very least the top-level dependencies" (Annex I, Part II). *(scope verified; CONFIRMED @pin SOURCE-PIN **§4 + §4a** (web-verified 2026-06-28): **CycloneDX 1.6** (released 2024-04-09, ECMA-424; verified in the built module's `target/bom.json`: `specVersion 1.6`), **SPDX = ISO/IEC 5962:2021**, **SLSA v1.0** ("Supply-chain Levels for Software Artifacts," Build track L0–L3, roadmap-not-tool), **EO 14028** (86 FR 26633), **EU CRA** (Regulation (EU) 2024/2847, Annex I Part II + recitals 22/77) — all cited dated-at-use, legal rows factual-not-legal-advice. Residual ⚠ @pin (NOT in the pin table): Syft version, in-toto/Sigstore-cosign/GitHub-OIDC specifics — see 09-flags/65_supply_chain_prose_atoms_not_pinned.md.)*
- **Routing** — dependency pinning/currency → Ch 27 (63/64); SAST (your code) + secure coding → Part VIII (70/69); reproducible builds → Ch 29 (67); license compliance → Ch 29 (68); security gate → later (73); release/publish SBOM → later (83); suppression → Ch 19 (39). SOURCE-PIN §4 (2026-06-27) + §4a (web-verified 2026-06-28): tool + standards + regulatory rows PINNED (CycloneDX 1.6 / Dependency-Check 12.2.2 / SPDX = ISO/IEC 5962:2021 / SLSA v1.0 / Grype / Trivy / Snyk / EO 14028 / EU CRA — legal rows dated-at-use, factual-not-legal-advice); residual unpinned prose atoms (cyclonedx-maven-plugin, Syft, Dependency-Track, in-toto/cosign/OIDC) flagged → 09-flags/. REPRO: the live NVD scan needs DB download/network → PENDING-RUNTIME offline.

...
The human gate is irreducible, rather than a transitional safeguard that better models will eventually retire, because of the **intent ceiling**, approached from both directions. AI *generation* cannot guarantee correctness because it has no ground truth of what the code should do; it infers intent from a prompt. AI *review* cannot verify correctness for the same reason; it infers intent from the code. Both hit the same wall: *intent lives outside the code*, in the requirement, the user's need, the business rule, the thing the system is *for*. A model trained on code has no authoritative access to it. A human author *has* the intent, having been told the requirement; a human reviewer can *reconstruct and challenge* it ("is this what we actually want?"). That is the one job in the quality pipeline that is structurally human, and it is why every gate in this book ultimately answers to a person. The trade-off a senior team weighs here is uncomfortable. AI makes everything *around* the intent-check cheap and fast, so more code arrives per reviewer-hour while the entire residual risk concentrates onto the one step that did not get cheaper. The intent-check becomes the bottleneck precisely because it cannot be automated away. The AI era does not dissolve the human gate; it sharpens it, leaving the intent-check as the visible, load-bearing constraint it always quietly was.

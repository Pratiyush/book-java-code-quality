...
**Current phase:** Phase 3 — DRAFT + GATE; **Phase G (post-WS-E) COMPLETE (2026-06-27).** Phases 0–2 ✅ + `/pin-source` ✅. FINAL_INDEX LOCKED = 47 chapters. **47/47 drafted + voice-lifted + figured + source-verified + code-reviewed; all editorial gates except the external SCORE are done.** **FLOOR-C COMPLETE book-wide** (compile + CODE-REVIEW: 45 modules green + 2 N/A); floors A/B/C-source PASS. **0 chapters approved (`04-approved/` empty)** — the sole remaining per-chapter gate is the external-LLM independent SCORE ≥88% (8/47 at 68–80%; queue `09-flags/external-review/QUEUE.md`) → Claude lift → `status.py` auto-approve → Step-12. See the dated paragraphs below + `09-flags/PENDING-TASKS.md` for remaining (all blocked/not-Claude-solo). RESUME: read this line + 01-index/FINAL_INDEX.md (locked TOC) + 01-index/CHAPTER-TRACKER.md (per-ch status); dossiers in 02-research/; drafts in 03-drafts/.
...
| 3 — Draft + Gate | Draft confirmed chapters through the gated pipeline | **DRAFTING + GATES (non-score) COMPLETE — gated on external SCORE.** 47/47 drafted + voice-lifted + figured + source-verified + **code-reviewed**. **FLOOR-C COMPLETE book-wide** (compile + CODE-REVIEW: 45 modules green + 2 N/A; 45 `_CODEREVIEW.md`). Floors A/B/C-source PASS. **0 chapters approved (`04-approved/` empty)** — the sole remaining per-chapter gate is the **external-LLM independent SCORE ≥88%** (8/47 scored at 68–80%; queue `09-flags/external-review/QUEUE.md`) → Claude lift → `status.py` auto-approve → Step-12. |
...
> All four are **blocked / not Claude-solo** (external model, networked tooling, or the human gate). The durable copy of this list lives in `09-flags/PENDING-TASKS.md`.

1. **The approval loop (THE gate to clearing Phase 3).** External LLM independently re-scores each lifted chapter (one-pager → `_SCORE_INDEP.md`) → Claude lifts the 8 already at 68–80% (per `09-flags/external-review/QUEUE.md` targets, clean-first order) → `status.py` auto-approves at ≥88% + floors A/B/C-source PASS → chapter lands in `04-approved/`.
2. **Networked `/pin-source` for the ~182 flagged residuals** (`@pin` / `AHEAD-OF-PIN` / `UNVERIFIED`), incl. the WS-D pin corrections to confirm (JaCoCo 0.8.16→0.8.15; Spotless Maven 3.x vs Gradle 8.7.0 line; Checkstyle/SpotBugs engine top-lines). Needs network; SaaS/web-only rows stay flagged.
3. **Engine-version bump + env-gated REPRO** (networked Maven) — bring the house analyzer engines to the pin top-lines and run the reproducibility check.
4. **Phase 4 — ASSEMBLE** once chapters reach `04-approved/` (`/assemble`), then the human **Step 16 MANUSCRIPT-GATE**. Deferred follow-up: figure-caption-by-key normalization.

...
**Current phase:** Phase 1 — RESEARCH (Parts I–VIII done). **74/110 dossiers banked** (keys 01–74). Mode = **cheaper main-loop solo research** (no subagent fan-out; the spend limit aborted the Part V workflow). Verify status: 07–40 have `_VERIFY.md`; 01–06 + 41–74 are research-done with formal SOURCE-VERIFY pending → fold into `/pin-source`. SOURCE-PIN versions still `TO-PIN` (JUnit row = JUnit 6 current). Next: Part IX (CI/CD & quality gates, keys 75–83).

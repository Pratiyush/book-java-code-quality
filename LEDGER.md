...
**Current phase:** Phase 1 — RESEARCH (Parts I–VI done). **61/110 dossiers banked** (keys 01–61). Mode = **cheaper main-loop solo research** (no subagent fan-out; the spend limit aborted the Part V workflow). Verify status: 07–40 have `_VERIFY.md`; 01–06 + Part V (41–52) + Part VI (53–61) are research-done with formal SOURCE-VERIFY pending → fold into `/pin-source`. SOURCE-PIN versions still `TO-PIN` (JUnit row = JUnit 6 current). Next: Part VII (build/supply-chain, keys 62–68).

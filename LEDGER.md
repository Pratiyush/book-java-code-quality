...
**Current phase:** Phase 1 — RESEARCH (Parts I–V done). **52/110 dossiers banked.** Mode switched to **cheaper main-loop solo research** (no subagent fan-out) after the monthly spend limit aborted the Part V workflow. Part V now complete: keys 41/44/47/48/49/50/51 salvaged from the failed run + 42/43/45/46/52 authored main-loop. Formal SOURCE-VERIFY pending for all of Part V (01–06 too) — fold into `/pin-source`. Keys 07–40 have `_VERIFY.md`. SOURCE-PIN versions still `TO-PIN` (JUnit row updated: JUnit 6 current). Next: Part VI (architecture, keys 53–61) in cheaper mode.

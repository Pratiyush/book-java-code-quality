...
- **Assertion Roulette** — many bare assertions in one test, so a failure does not say *which* expectation broke. Fix: a message per assertion, or one logical assertion per test (`assertAll`).

...
The dashboard wasn't lying about coverage. It was lying about *quality*, because coverage isn't quality and the manager read one as the other. This chapter is about the two halves of that mistake: knowing what the goal actually is (readable, changeable code), and measuring it without fooling yourself with a number that looks precise and means nothing.
...
**What this chapter does NOT cover.** It does not configure the tools (Part IV) or set CI gate thresholds (Part IX). It gives you the goal and the measurement discipline; later chapters wire them up.

**If you already hold one idea**, hold this: you have read code that was "correct" and still miserable to change. Readability is the name for the difference, and metrics are the imperfect instruments we use to see it.

## How it works

### Readability is the goal because reading is the cost

From Chapter 1, internal quality's most leverage-heavy sub-characteristic is **analysability** — can a developer understand and locate things? The reason is pure economics. *Clean Code* states it: "the ratio of time spent reading versus writing is well over **10 to 1** … because this ratio is so high, we want the reading of code to be easy, even if it makes the writing harder." You optimize the operation you perform ten times for every once, not the reverse.

A few things are near-universally agreed, and worth stating before the contested parts:

- **Naming carries most of the load.** The old joke — "there are only two hard things in Computer Science: cache invalidation and naming things" (attributed to Phil Karlton) — is a joke because it's true. Names are the densest documentation in the code.
- **Local reasoning lowers cognitive load.** Code you can understand without holding the whole system in your head is more readable — both schools below agree on this, even where they disagree on how.
...
To manage readability you want to measure it, and two complexity metrics dominate. The distinction between them is the single most useful measurement idea in this chapter.
...
**Cognitive Complexity** (G. Ann Campbell, SonarSource) was designed to fill that gap. It "assigns incremental costs for each code structure that breaks linear reading flow," and — critically — it **increments more for nesting**. The same number of branches laid out flat scores far lower than when they are nested four deep.
...
- **`var`** removes redundant type noise — though it can *hurt* readability when the type isn't obvious from the right-hand side (a contested style point).
...
Complexity is one family. The broader landscape — folded in from the measurement dossier — is worth a map, because choosing the *wrong* metric is the most common failure.
...
The reason "the number lies" is captured by **Goodhart's Law**, in Marilyn Strathern's pithy form: *"When a measure becomes a target, it ceases to be a good measure."* Set coverage as a target and you get assertion-free tests that touch lines; set lines-of-code as a target and you get bloated code; set deploy frequency as a target and you get artificially split deploys. The metric stops measuring the thing the moment people optimize the metric.
...
Complexity is the most directly *gate-able* readability proxy, so it is where the discipline gets tested. The senior move is to treat a threshold breach as a **refactor trigger**, not a number to suppress:

- A method over the cognitive-complexity threshold is a prompt to extract a method, replace nested conditionals with guard clauses, or model the branches with sealed types + pattern matching (Chapter 5/13) — *not* to split it arbitrarily into pieces that lower the per-method number while raising overall coupling.
...
> **WARNING** Splitting a method purely to dodge a complexity threshold can *raise* coupling and *lower* readability — you've optimized the metric and harmed the goal. This is Goodhart in miniature. Low cognitive complexity with terrible names is still unreadable.

### The contested zone — two reputable schools

Here is where honesty matters most. *That* readability matters is consensus. *How* to achieve it — function size, comments — is genuinely contested among reputable practitioners, and this book presents the disagreement fairly rather than crowning a side.
...
- **School B — *A Philosophy of Software Design* (John Ousterhout):** favors **deep modules** (a simple interface over a substantial implementation) over many tiny ones, argues that **excessive decomposition adds cognitive load** (you jump between fragments to follow one idea), and explicitly **values comments** as capturing design intent the code cannot. It contradicts School A on both points.
...
The way to use this: treat the disagreements as **context-dependent trade-offs**, not winners. Tiny functions aid navigation but can fragment a readable algorithm; comments rot but capture *why*; `var` cuts noise but can hide a type. A team picks a position deliberately and applies it consistently (Chapter 6, 37).
...
- **Every code metric is a proxy** for an unmeasurable target. High correlation in studies is not causation in your repo; a clean number can accompany unreadable code.
- **The prescriptions conflict.** You cannot follow *Clean Code*'s tiny-function rule and *APoSD*'s deep-module rule at once; a team must choose, contextually — that is the whole point of the contested zone.
...
- **A single composite score** (e.g. a Maintainability Index or one "quality gate" grade). Convenient for a dashboard, but collapses information and invites gaming; this book prefers a counter-metric'd panel. Neither is "correct" — the trade-off is legibility versus information.
...
- **Gate on complexity** where you want an automatic, objective signal — but gate *new* code, treat breaches as refactor triggers, and pick cognitive complexity for readability questions.
- **Reach for a metric** to start a conversation, never to end one or to rank a person. If a number can't change a decision, it's vanity — stop collecting it.
...
You now know the goal (readable, changeable code) and the discipline for measuring it without lying to yourself. But a metric on a dashboard is downstream of a hundred small decisions a developer makes while typing — naming, structure, the shape of a method. The next chapters drop from "what to measure" to "what to write": the craft of quality Java, starting with the canon that distilled it.
...
- **Goodhart's Law:** a measure that becomes a target stops measuring. Use outcomes, counter-metrics, trends; never single targets or individual rankings.
...
- *Vanity metric* — a number that looks meaningful but doesn't change a decision (e.g. LOC, raw coverage %).
...
If readable code is the goal, where do you learn the moves that produce it — and which of the old rules has Java itself made obsolete?

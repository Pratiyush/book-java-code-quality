<!--
Dossier key: 20 (owner) + folds 21 + 23 — per 01-index/FINAL_INDEX.md Ch 13 (OPENS Part III)
Slug: 20_thread_safety_jmm
Part / arc position: Part III — Concurrency & Correctness, Chapter 13 (opens Part III; Part II = Ch 5-12)
Companion module: 08-companion-code/20_thread_safety_jmm/ — ⚠ EXAMPLE-BUILD = PENDING-RUNTIME (no JDK). Spec at foot.
Verified against SOURCE-PIN: 2026-06-20. Sources: JLS SE 21 ch.17 (§17.4.5 happens-before/data-race/correctly-synchronized — verbatim; §17.5/§17.5.1 final-field freeze — verbatim; §17.7 long/double non-atomic — verbatim; §17.6 word tearing); java.util.concurrent package-summary "Memory Consistency Properties" (happens-before edge list — verbatim, JDK 21); JEP 444 (Virtual Threads, GA Release 21, synchronized-pinning caveat); JEP 491 (Synchronize VTs without Pinning, Release 24); JEP 453/505 (Structured Concurrency, PREVIEW @21→@25 — AHEAD-OF-PIN); JEP 506 (Scoped Values, final @25 — AHEAD-OF-PIN @21); JEP 395 (records, Java 16); j.u.c API @JDK 21 (ReentrantLock/ReadWriteLock/StampedLock; AtomicInteger/LongAdder; ConcurrentHashMap computeIfAbsent; BlockingQueue; CountDownLatch/Semaphore; ExecutorService/CompletableFuture; Executors.newVirtualThreadPerTaskExecutor); Goetz et al. *Java Concurrency in Practice* (2006, §3.4-3.5, DATED post-2006 APIs); tool rules Error Prone GuardedBy(ERROR)/DoubleCheckedLocking(WARNING)/Immutable(ERROR)/FutureReturnValueIgnored, SpotBugs MT_CORRECTNESS (VO_VOLATILE_INCREMENT/DC_DOUBLECHECK/DC_PARTIALLY_CONSTRUCTED/IS2_INCONSISTENT_SYNC/LI_LAZY_INIT_STATIC/DL_SYNCHRONIZATION_ON_BOXED_PRIMITIVE/JLM_JSR166_UTILCONCURRENT_MONITORENTER), Sonar java:S2168/S3077/S2142/S3078/S2445; JCStress (OpenJDK, experimental runtime harness).
⚠ verify-at-pin: JLS SE 25 §§ unchanged; Sonar rule titles/defaults (rules.sonarsource.com offline — use RSPEC repo); SpotBugs MT verbatim descriptions/ranks; EP severities at pin; JCIP verbatim/pages; tool GAVs. ⚠ AHEAD-OF-PIN: structured concurrency (preview @21/25), scoped values (GA @25), JEP 491 no-pinning (@24).
DRAFT v1 — gates manual; happens-before-spine + four-publication-idioms + version-bound-advice + library-vs-hand-rolled shapes; EXAMPLE-BUILD pending JDK.
-->
...
A counter is off by a few hundred. The code looks fine — a shared `int`, incremented by a pool of worker threads, read at the end. Every unit test passes. It passes on your laptop a thousand times. It passes in CI. It ships, and three weeks later, under production load on a 32-core machine, the total is wrong, intermittently, by an amount that changes every run.

This is the defining property of a concurrency defect, and the reason it gets its own chapter to open Part III: **it is invisible to ordinary testing.** A data-racy program can pass every single-threaded test and most multi-threaded ones, then corrupt state on different hardware or under different load, because the compiler, the JIT, and the CPU are all allowed to *reorder* memory operations in ways that only become visible across threads. You cannot test your way to thread-safety the way you test correctness on one thread. You have to *reason* your way there — and the document you reason with is the Java Memory Model.

This chapter teaches that reasoning. It defines what "correct" even means when two threads share memory (the JMM and the *happens-before* relation), shows the disciplined way to hand an object from one thread to another so the receiver sees it fully built (**safe publication**), and surveys the tested building blocks — `java.util.concurrent` — that encode all of this so you rarely have to hand-roll a lock at all.
...
- How the race you can't test for *is* caught — by static analysis (`@GuardedBy`, SpotBugs MT) and by a runtime stress harness (JCStress).
...
**If you hold one idea**, hold the epigraph: *a program is correctly synchronized if and only if all its sequentially-consistent executions are data-race-free; get it there, and the spec guarantees the intuitive reading.* Everything else is technique for establishing enough happens-before edges to make that true.
...
> **CONCEPT** *The whole job, in one line.* Establish enough happens-before edges that every pair of conflicting accesses is ordered — i.e. make the program data-race-free — and the JLS guarantees the simple, sequentially-consistent reading you naturally want. Thread-safety is not "add locks until it works"; it is "establish the edges that remove the races."
...
Two design moves make this class of bug *structurally impossible*. The first is **immutability**: an object whose state never changes after construction has no stale read or torn update to suffer. The second is the disciplined act of **safe publication**, and there are exactly four idioms — each one of the happens-before edges above:

| Idiom | What you write | The edge that makes it safe |
|---|---|---|
...
- **Explicit locks** (`ReentrantLock`, `ReadWriteLock`, since 1.5) do what `synchronized` does plus timed/interruptible/`tryLock` acquisition and fairness — at the cost of a mandatory `lock(); try { … } finally { unlock(); }` idiom (the JVM won't release it for you).
...
## Deep dive: detecting the race you can't test for

If a data race passes every ordinary test, how is it ever caught? Two ways — before the binary ships, and as it runs — and a third complication: some of the advice is *version-bound*.

**Before it ships — static analysis.** The detectors catch *patterns* of unsafe sharing. SpotBugs ships a whole Multithreaded-correctness (`MT_CORRECTNESS`) category: `VO_VOLATILE_INCREMENT` (the non-atomic `volatile++`), `DC_DOUBLECHECK` (double-checked locking that is, in its own words, "not correct according to the semantics of the Java memory model"), `IS2_INCONSISTENT_SYNC` (a field synchronized on some paths but not others), `DL_SYNCHRONIZATION_ON_BOXED_PRIMITIVE` (locking on a cached `Integer`). Error Prone turns a *documentation* annotation into a *compile-time guarantee*: `@GuardedBy("lock")` declares which lock guards a field, and the `GuardedBy` check (severity **ERROR**) fails the build on any unguarded access. That is the cluster's defining move — the annotation that documents the locking discipline is the same one the tool verifies.

But static analysis is necessarily incomplete: race detection is undecidable in general, so the tools catch patterns, not all races, and `@GuardedBy` only helps fields you actually annotate. A clean static scan is never *proof* of thread-safety.

**As it runs — JCStress.** The OpenJDK Java Concurrency Stress harness is the runtime instrument: it runs threads concurrently over shared state, collects *every* observed outcome across many iterations and hardware reorderings, and labels each ACCEPTABLE / FORBIDDEN / INTERESTING. It can make the hook's race reproducible — surface a FORBIDDEN interleaving the unit tests never triggered — and then show it disappear once you add the missing edge. It is, by its own label, *experimental*, probabilistic, and slow; it proves the *presence* of a bug far more reliably than its absence. Reserve it for genuinely subtle lock-free or publication code.
...
The unifying thread of the chapter: thread-safety is the one quality dimension you reason about rather than test for, and the reasoning is always the same — *which happens-before edge makes this write visible to that read?* The tools help you find the missing edge; the JMM tells you which edges exist; immutability and safe publication let you need fewer of them.
...
- **Double-checked locking is fragile even when fixed.** The classic lazy-init idiom is broken without `volatile` (a reader can see a partially-constructed object); it is correct *with* a `volatile` field, but Sonar `java:S2168` discourages the pattern outright while `java:S3077` flags the `volatile`-on-a-reference fix — the two rules pull in different directions. Prefer the initialization-on-demand holder idiom (a `static final` holder gets laziness and safety from class-init locking).
- **`ConcurrentHashMap` atomicity is per-operation, not per-transaction** — `get`-then-`put` still races; use `computeIfAbsent`/`merge`. It also rejects `null` keys and values, unlike `HashMap`.
- **`CopyOnWriteArrayList` copies on every write** — for read-mostly listener lists only, never a write-heavy path. **`StampedLock` is not reentrant** — a deadlock trap if you expect reentrancy.
...
- **Thread confinement:** the cheapest "thread-safety" is not sharing at all — confine mutable state to one thread (a `ThreadLocal`, or an event loop). No edge needed because there is no cross-thread access.
...
- **`VarHandle` / atomics for lock-free code:** for measured hotspots where a lock's contention is the bottleneck — expert territory, verified with JCStress.
...
- **For a multi-variable invariant:** a lock (`synchronized` for simple scoped exclusion; `ReentrantLock` when you need timed/interruptible/fair acquisition) — and at Java 21, prefer `ReentrantLock` around blocking calls in virtual-thread code (narrowed at 24+).
...
You now have the model — happens-before — and the disciplines that satisfy it: immutability, safe publication, and the `java.util.concurrent` building blocks. All of it assumed threads are a scarce, expensive resource you pool and reuse. The next chapter removes that assumption. **Virtual threads** (GA at Java 21) make a thread per task cheap, which changes how you use executors and tempts far more sharing — raising the value of everything in this chapter — and **structured concurrency** (still preview) reshapes how concurrent tasks are forked, joined, and fail together. The JMM doesn't change; the scale does. We also pick up the concurrency *testing* problem — JCStress in depth — because the race you can't test for still has to be tested somehow.
...
**Companion module (spec — ⚠ EXAMPLE-BUILD = PENDING-RUNTIME, no JDK):** `08-companion-code/20_thread_safety_jmm/` — a deliberately data-racy shared counter / unsafely-published holder (no happens-before edge), then the corrected versions via (a) `synchronized`, (b) `AtomicLong`, (c) `final`-field immutable publication / initialization-on-demand holder, plus a `@GuardedBy("lock")` field. **Failure paths:** the `broken` package trips SpotBugs `IS2_INCONSISTENT_SYNC`/`VO_VOLATILE_INCREMENT`; an unguarded `@GuardedBy` access fails Error Prone (ERROR) at compile; a JCStress test shows a FORBIDDEN interleaving for the racy counter and only ACCEPTABLE outcomes once the edge is added. Snippet tags: `racy-counter`, `guarded-by`, `final-publication`, `lazy-holder`, `jcstress-test`. *(No preview features in the compiled module; structured concurrency/scoped values are forward-notes only.)*

## Next chapter teaser

Threads used to be precious — pooled, counted, reused. The next chapter is virtual threads and structured concurrency: a thread per request becomes cheap enough to write blocking code at scale, the executor patterns shift, and `StructuredTaskScope` (still preview) reshapes how concurrent work is forked and joined. The memory model from this chapter doesn't change — but with millions of cheap threads, getting publication and cancellation right matters more, not less, and we'll finally tackle how to *test* a race.

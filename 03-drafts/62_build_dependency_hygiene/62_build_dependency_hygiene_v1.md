...
Companion module: 08-companion-code/ (parent POM: BOM import + Enforcer dependencyConvergence + wrapper; a seeded convergence failure + fix; a renovate.json/dependabot.yml) — ⚠ EXAMPLE-BUILD = PENDING (toolchain READY: JDK 21.0.11+25.0.3). The flagship's aggregator already demonstrates BOM-import + pinned plugins. Spec at foot.
...
⚠ verify-at-pin: Maven/Gradle versions + lifecycle phase names + Enforcer rule names (dependencyConvergence/requireUpperBoundDeps) + `check` semantics; Maven "nearest wins" wording; BOM-import syntax + catalog format; Renovate/Dependabot config keys (grouping/automerge/schedule) + security-alert sources (NVD/OSV/GitHub Advisory); versions-plugin/dependencyUpdates goal names. Reproducible-build flags → key 67.
Routes: toolchain map → Ch 3 (05); reproducible builds → key 67; build speed → CI part (79); local↔CI parity → CI part (82); CI gates → CI part (75/76); vuln scanning/SCA → Ch 28 (65); SBOM/provenance/SLSA → Ch 28 (66); version migration → key 95; contract tests catch update breaks → Ch 24 (50); suppression discipline → Ch 19 (39). SOURCE-PIN: Maven/Gradle/Enforcer/Renovate/Dependabot §-rows TO-PIN.
DRAFT v1 — gates manual; build-as-gate-host + tool-agnostic-quality-properties + two-tools-crown-neither + single-source-of-version-truth + pin-vs-currency-tension + strategy-over-tool shapes; PART VII OPENER. EXAMPLE-BUILD pending (flagship aggregator already shows BOM-import + pinned plugins).
-->

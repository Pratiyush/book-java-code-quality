...
DRAFT v1 — gates manual; throwable-decision + item-to-rule crosswalk + two-paths-validation + lifecycle-card shapes; EXAMPLE-BUILD pending JDK.
-->

# When Things Go Wrong

*Exceptions, resource cleanup, and not trusting your inputs — the failure paths done right · 12 (folds 16, 18) · Part II*

> The happy path is where the program does its job. The failure paths are where it earns your trust.
...
- **JLS SE 21** — §11 (`Throwable` hierarchy; §11.2 checked catch-or-specify), §14.20/§14.20.3 (try/catch/finally; try-with-resources basic §14.20.3.1 / extended §14.20.3.2, reverse-order close, suppressed exceptions), §14.10 (`assert`). *(constructs verified; ⚠ §§ re-confirm @pin.)*
- **JEPs** — 358 (helpful NPEs, `-XX:+ShowCodeDetailsInExceptionMessages`, default-on since JDK 15 — ⚠ verify @pin); 213 (effectively-final resources, Java 9); 421 (deprecate finalization, Java 18); 395/409/441 (records/sealed/pattern-`switch`, GA at 21); 453 (`StructuredTaskScope`, preview — AHEAD-OF-PIN).
...
- **Jakarta Validation 3.1** (Final 2024-03-28; renamed from Jakarta Bean Validation) — `@NotNull`/`@NotEmpty`/`@Size`/`@Email`/`@Valid`/…; `Validator`/`ValidatorFactory`/`ConstraintViolation`/`ExecutableValidator`; groups + `@GroupSequence`; custom `ConstraintValidator`. **Hibernate Validator 9.1.0.Final** (RI of 3.1.1, Java 17+). *(⚠ GAVs/EL impl @pin.)*
...
**Companion module (spec — ⚠ EXAMPLE-BUILD = PENDING-RUNTIME, no JDK):** `08-companion-code/12_error_handling_exceptions/` — an order-service error model: a guard-clause/compact-constructor `Money` invariant (fail-fast `IllegalArgumentException`), a recoverable path throwing a checked domain exception with the cause chained (Item 73), a sealed `Result` typed alternative with an exhaustive `switch`, a try-with-resources demo asserting reverse-order close and `getSuppressed()`, a `Cleaner` backstop, and a `@Valid` request record validated at the boundary returning `400` + `ConstraintViolation`. **Failure paths:** a swallowed catch + raw `throw new RuntimeException` trip `EmptyCatchBlock`/`java:S112`/`IllegalCatch`; a leaky resource trips `java:S2095`/`OS_OPEN_STREAM`; an empty `items` list yields a `@NotEmpty` violation. Snippet tags: `checked-vs-unchecked`, `exception-translation`, `result-model`, `boundary-handler`, `twr-basic`, `suppressed`, `cleaner-backstop`, `guard-clause`, `constraints`.

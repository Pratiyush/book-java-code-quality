# SCORING PACKET — Printed Chapter 12  (dossier 19_code_smells_antipatterns)
# 1. Paste EVERYTHING below the line into a fresh chat in a DIFFERENT-VENDOR LLM (not Claude).
# 2. Save its one-pager reply VERBATIM as: 03-drafts/19_code_smells_antipatterns/19_code_smells_antipatterns_SCORE_INDEP.md
...
⚠ contested (key 61): patterns-as-vocabulary vs simplicity-first — two-schools, no crown. ✅ resolved 2026-06-27: JEP numbers (395/409/441/378, all finalized in-pin-range); Sonar `java:S3776`=15 and `java:S107`=7 (source-verified). ⚠ verify-at-pin (remaining): PMD Design-rule + Sonar `java:S138`/`java:S1192`/`java:S1448` default thresholds (move per version, differ per tool); Fowler 2e complete smell list + quoted spans; EJ item numbers; GoF/AntiPatterns/Ousterhout verbatim; undetectable-smell list. (Named-canon verbatims are not in-repo by design — SOURCE-PIN §7; flagged `09-flags/19_unverified_thresholds_and_undetectable_smells.md`.)
DRAFT v1 — gates manual; smell-card + canon-dating + two-schools + item-to-rule shapes; EXAMPLE-BUILD = BUILT GREEN (mvn -B -Pquality verify, JDK 21.0.11; 13 tests pass, 0 Checkstyle, 0 SpotBugs after the one reviewed suppression — per target/ build artifacts).
-->
...
| Classic pattern / smell | Hand-built solution | Modern-Java answer (JEP — finalized, verified 2026-06-27) |
|---|---|---|

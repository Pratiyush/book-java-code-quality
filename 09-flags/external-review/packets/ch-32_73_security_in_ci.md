# SCORING PACKET — Printed Chapter 32  (dossier 73_security_in_ci)
# 1. Paste EVERYTHING below the line into a fresh chat in a DIFFERENT-VENDOR LLM (not Claude).
# 2. Save its one-pager reply VERBATIM as: 03-drafts/73_security_in_ci/73_security_in_ci_SCORE_INDEP.md
...
Companion module: 08-companion-code/73_security_in_ci/ (runnable gate POLICY org.acme.secgate.SecurityGate aggregating findings → three-way decision, + illustrative ci/security-pipeline.yml) — ✅ EXAMPLE-BUILD = BUILT-GREEN (`mvn -B -Pquality verify`: 14 tests, 0 Checkstyle, 0 SpotBugs; JDK 21.0.11 / Maven 3.9.16; see _EXAMPLE.md). Residual: DAST-against-staging is REPRO PENDING-RUNTIME (needs a live deployment); SaaS/unpinned CI tools dated-at-use. Spec at foot.
...
RESOLVED-AT-PIN (SOURCE-PIN 2026-06-27 + BUILT module): the block-policy / severity-threshold MECHANICS are realized and unit-tested in org.acme.secgate.SecurityGatePolicy / SecurityGate (green build confirms aggregate-findings → three-way decision, scoped to new code). Trivy (0.71.0, §4) and OWASP Dependency-Check (12.2.2, §4) are now PINNED rows, appearing only as dated illustrative pipeline steps. ⚠ DATED-AT-USE (unconfirmable — left marked): DAST/IAST tool specifics (OWASP ZAP — no SOURCE-PIN row), gitleaks (no row), GitHub Actions @v4 (rolling SaaS, §5) — each dated-at-use 2026-06 per the per-tool CLI flag, not asserted as timeless; flagged 09-flags/73_security_pipeline_saas_dated_at_use.md. ⚠ AHEAD-OF-PIN: none material. ⚠ REPRO PENDING-RUNTIME: DAST against staging needs a live deployment (network/environment-gated) — described and wired as config, not executed.
...
![Figure 32.1 — The security gate: five testing types ordered fast-to-slow — Static checks (secrets, SAST, SCA) run at pre-commit and PR; dynamic checks (DAST, IAST) gate the release against staging. Block only high-severity new findings; warn and triage the rest.](../../05-figures/73_security_in_ci/fig73_1.png)

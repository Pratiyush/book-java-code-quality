# SCORING PACKET — Printed Chapter 27  (dossier 62_build_dependency_hygiene)
# 1. Paste EVERYTHING below the line into a fresh chat in a DIFFERENT-VENDOR LLM (not Claude).
# 2. Save its one-pager reply VERBATIM as: 03-drafts/62_build_dependency_hygiene/62_build_dependency_hygiene_SCORE_INDEP.md
...
<!--
Dossier key: 62 (owner, leads) + folds 63 + 64 — per 01-index/FINAL_INDEX.md Ch 27
Slug: 62_build_dependency_hygiene (owner key 62)
Part / arc position: Part VII — Build, Dependencies & Supply Chain, Chapter 27 (OPENS Part VII)
Companion module: 08-companion-code/62_build_dependency_hygiene/ (module POM: BOM import + Enforcer dependencyConvergence/requireUpperBoundDeps/bannedDependencies + versions plugin; a transiently-proven convergence failure + fix; a renovate.json/dependabot.yml) — EXAMPLE-BUILD = BUILT GREEN (Maven 3.9.16, JDK 21.0.11; 5 tests pass, 0 Checkstyle, 0 SpotBugs; see _EXAMPLE.md 2026-06-26). The flagship's aggregator already demonstrates BOM-import + pinned plugins. Spec at foot.
Verified against SOURCE-PIN: 2026-06-20. Sources (concise main-loop dossiers; ⚠ Maven/Gradle + Renovate/Dependabot crown neither, each cited to own docs):
- Build (62, ⚠ Maven/Gradle): the build is where every gate runs (formatters/linters/analyzers/tests/coverage/scanners all hang off Maven/Gradle — Ch 3 map). Disciplined build = quality artifact (reproducible/fast/fail-fast/local==CI). Lifecycle as gate host: Maven validate→compile→test→verify; Gradle `check` aggregates verification. Order cheap/fast first → heavier last. Maven (declarative POM, convention-over-config, stable lifecycle; maven-surefire/failsafe, maven-enforcer-plugin: dependencyConvergence/banned deps/require-JDK; analyzer plugins). Gradle (programmable Groovy/Kotlin DSL, incremental + build cache, `check` lifecycle, version catalogs). Tool-agnostic build-quality properties: reproducible (pinned versions, NO LATEST/RELEASE/ranges — key 67), fast (incremental/cache/parallel — key 79), fail-fast clear errors, local↔CI parity (key 82), single source of dep truth (BOM/catalog), wrapper committed (mvnw/gradlew pins the build-tool version itself). LIMITS: build complexity itself a risk (over-engineered Gradle / 2000-line POM); Maven rigidity vs Gradle-programmability-becomes-unreadable (each its cost, crown neither); slow builds erode quality (30-min verify → devs skip locally); plugin/version drift.
- Dependency mgmt (63): deps are most of a modern app; unmanaged = quality+security liability (conflicts, transitive surprises, works-on-my-machine). Hygiene = one source of version truth + no conflicting transitive + no unpinned/range + minimal justified set. Single source: Maven <dependencyManagement> + imported BOM; Gradle version catalogs (gradle/libs.versions.toml) + platforms; child modules omit versions. Convergence: transitive conflicts; Maven "nearest wins" can silently downgrade; enforce maven-enforcer-plugin dependencyConvergence / requireUpperBoundDeps; inspect mvn dependency:tree/analyze; Gradle resolution strategies + dependencyInsight. No moving versions (ban LATEST/RELEASE/open ranges — key 67). Minimal surface (dependency:analyze used-undeclared/declared-unused; fewer deps = smaller attack surface key 65 + faster). Scope discipline (test/provided/runtime). Repo hygiene (trusted repos, checksum/signature → key 66). LIMITS: strict convergence noisy (over-strict→disabled key 39); BOM/catalog needs buy-in; pinning-vs-currency tension (pinned go stale → key 64); doesn't judge dep QUALITY (just version agreement).
- Dependency currency (64, ⚠ Renovate/Dependabot): pinned deps reproducible but ROT — stale = #1 source of known vulns (key 65) + painful big-bang upgrades (key 95). Automated updates = scary annual migration → steady small reviewable PRs. Bots open PRs w/ changelogs: Dependabot (GitHub-native, dependabot.yml) / Renovate (multi-platform, configurable grouping/schedules/automerge, renovate.json); both run build/tests per PR. Strategy (senior content): auto-merge low-risk (patch of trusted deps + green CI); review majors/minors + security; GROUP related (all test deps); SCHEDULE (weekly) to batch. Local discovery: versions:display-dependency-updates (Maven), dependencyUpdates (Gradle). Gate interaction: updates flow through SAME gates (tests/analyzers/vuln scan key 65) → break or CVE caught automatically. Security updates prioritized (Dependabot/Renovate vs GitHub Advisory/OSV — overlaps key 65). LIMITS: PR fatigue (without grouping/scheduling/automerge tuning → ignored); breaking updates still break (green build ≠ semantic break key 50; majors need human judgment + changelog); automerge risk (gate on strong tests); bot ≠ strategy. Renovate vs Dependabot ⚠ crown neither.
VERIFIED at EXAMPLE-BUILD (Maven 3.9.16, JDK 21.0.11 — see _EXAMPLE.md 2026-06-26): Maven version 3.9.16 (SOURCE-PIN §4); Maven lifecycle phases validate→compile→test→verify (ran green); Enforcer rule names dependencyConvergence/requireUpperBoundDeps/bannedDependencies (class names confirmed in the resolved enforcer-rules jar AND all rules executed+passed on the graph); BOM-import syntax (`<type>pom</type>`+`<scope>import</scope>`, resolved+built); versions-plugin goal `display-dependency-updates` (ran during verify).
⚠ verify-at-pin (NOT confirmed by the Maven companion build — Gradle/SaaS/doc-only atoms): Gradle 9.6.0 task `check` semantics + version-catalog `libs.versions.toml` format + resolution strategies/`dependencyInsight` + `dependencyUpdates` goal (Gradle docs not fetched; module is Maven-only); Maven "nearest wins" resolution wording (doc claim, not exercised by the build); Renovate/Dependabot config keys (grouping/automerge/schedule) + security-alert sources (NVD/OSV/GitHub Advisory) — SOURCE-PIN §4 marks these ⚠ rolling, pin schema dated-at-use; the exact maven-enforcer-plugin/versions-maven-plugin version literals (SOURCE-PIN §4 pins "Maven 3.9.16 (+ enforcer, versions plugins)" as one row, not separate plugin versions → 09-flags/62_enforcer_versions_plugin_versions_unpinned.md). Reproducible-build flags → key 67.
Routes: toolchain map → Ch 3 (05); reproducible builds → key 67; build speed → CI part (79); local↔CI parity → CI part (82); CI gates → CI part (75/76); vuln scanning/SCA → Ch 28 (65); SBOM/provenance/SLSA → Ch 28 (66); version migration → key 95; contract tests catch update breaks → Ch 24 (50); suppression discipline → Ch 19 (39). SOURCE-PIN §4 PINNED (2026-06-20): Maven 3.9.16, Gradle 9.6.0; Renovate/Dependabot ⚠ rolling (SaaS, schema dated-at-use); enforcer/versions plugin version literals not separately pinned → 09-flags/62.
DRAFT v1 — gates manual; build-as-gate-host + tool-agnostic-quality-properties + two-tools-crown-neither + single-source-of-version-truth + pin-vs-currency-tension + strategy-over-tool shapes; PART VII OPENER. EXAMPLE-BUILD built green (module `08-companion-code/62_build_dependency_hygiene/`; all five Enforcer rules pass on the resolved graph + the convergence failure path proven transiently — see _EXAMPLE.md).
-->

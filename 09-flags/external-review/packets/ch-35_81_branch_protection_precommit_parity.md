# SCORING PACKET — Printed Chapter 35  (dossier 81_branch_protection_precommit_parity)
# 1. Paste EVERYTHING below the line into a fresh chat in a DIFFERENT-VENDOR LLM (not Claude).
# 2. Save its one-pager reply VERBATIM as: 03-drafts/81_branch_protection_precommit_parity/81_branch_protection_precommit_parity_SCORE_INDEP.md
...
Companion module: 08-companion-code/ (branch-protection config + merge-queue note + feature-flag example; .pre-commit-config.yaml: Spotless + gitleaks + fast Checkstyle mirroring CI fast stage) — ✅ EXAMPLE-BUILD = BUILT GREEN [MANUAL — tooling pending] (JDK 21.0.11; mvn -B -Pquality verify: 10 tests, 0 Checkstyle, 0 SpotBugs; ruleset.yml + .pre-commit-config.yaml parse; the local↔CI parity core is runnable+unit-tested). SaaS surfaces (GitHub rulesets + pre-commit framework/hook rev:) dated-at-use 2026-06 → 09-flags/. Spec at foot.
...
Resolved-at-build (EXAMPLE-BUILD 2026-06-27): pre-commit framework config schema + the Spotless/fast-Checkstyle local hooks (the `.pre-commit-config.yaml` parses and its `local` hooks invoke the wrapper, in the BUILT-GREEN module); `git commit --no-verify` is a stable documented git flag (pre-commit pinned SOURCE-PIN §5, 2026-04); the local↔CI parity assertion is runnable+unit-tested (`org.acme.parity.GateParity`). ⚠ Still SaaS dated-at-use (LEAVE marked; 09-flags/81): GitHub merge-queue + branch-protection/required-checks/ruleset setting names (rolling, SOURCE-PIN §5); the gitleaks hook `rev:` (placeholder, no pinned row); the exact dora.dev capability wording for trunk-based development (the capability traces to the pinned *Accelerate* 2018 + 2025 DORA report; the prose paraphrases, crowns no model). REPRO: CI/git-platform config = artifacts not buildable.
...
DRAFT v1 — gates manual; gate-needs-teeth-and-speed + branch-protection-makes-it-unbypassable + trunk-based-demands-trustworthy-gates + merge-queue-keeps-main-green + feedback-latency-ladder + hooks-are-feedback-not-enforcement shapes; PART IX. EXAMPLE-BUILD = BUILT GREEN [MANUAL — tooling pending] 2026-06-27 (parity core built+tested; config artifacts parse; SaaS surfaces dated-at-use → 09-flags/81).
-->

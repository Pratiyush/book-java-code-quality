# SCORING PACKET — Printed Chapter 14  (dossier 22_virtual_threads_structured_concurrency)
# 1. Paste EVERYTHING below the line into a fresh chat in a DIFFERENT-VENDOR LLM (not Claude).
# 2. Save its one-pager reply VERBATIM as: 03-drafts/22_virtual_threads_structured_concurrency/22_virtual_threads_structured_concurrency_SCORE_INDEP.md
...
✅ build-confirmed (mvn -Pquality verify GREEN, JDK 21.0.11, 2026-06-27): virtual threads GA @21 + the newVirtualThreadPerTaskExecutor() one-thread-per-task idiom (FanOutFetcher); the pinning trap = synchronized-around-blocking + the ReentrantLock fix, dated to 21 (PinningDemo); the @GuardedBy/IS2_INCONSISTENT_SYNC lock-discipline counter-example + AtomicLong fix (InconsistentlySyncedCounter/GuardedCounter); the CountDownLatch deterministic race harness (RaceHarness); the bounded-lifetime structured-concept demo in stable APIs (StructuredConceptDemo).
⚠ still verify-at-pin (could NOT be confirmed offline — verify_sources.sh blocked: no fetchable pin clone in this env; JLS / JEP pages / rules.sonarsource.com / jcstress repo all unreachable): JLS §17 §§ + the verbatim JEP/JLS quoted spans; flag/property names (jdk.virtualThreadScheduler.parallelism, jdk.tracePinnedThreads post-491); tool versions/severities/SpotBugs ranks (RSPEC repo); JCStress version 0.16 (not yet a SOURCE-PIN row — see 09-flags/24_jcstress_not_pinned.md); Sonar S6906/S6881 titles; Lincheck unpinned. (flagged: 09-flags/22_tool_rule_defaults_unverified.md, 24_jcstress_not_pinned.md) ⚠ AHEAD-OF-PIN (kept — never asserted stable; flagged 09-flags/22_structured_concurrency_scoped_values_status.md): structured concurrency / StructuredTaskScope (preview 21→25), scoped values (GA @25), JEP 491 no-pinning (@24).
DRAFT v1 — gates manual; GA-vs-preview-discipline + version-bound-advice + prove-a-bug-exists + approximation-of-a-spec-property shapes; EXAMPLE-BUILD GREEN on pinned JDK 21.0.11 (mvn -B -Pquality verify: 10 tests pass, 0 Checkstyle, SpotBugs total_bugs=0 with one reasoned IS2_INCONSISTENT_SYNC suppression) — confirmed 2026-06-27.
-->

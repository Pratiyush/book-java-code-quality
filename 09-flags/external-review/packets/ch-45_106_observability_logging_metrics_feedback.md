...
> A service is down at 3am. The on-call engineer opens the logs and finds ten thousand undifferentiated lines, no request IDs, no way to follow one failing request — and three lines above the buried error, a customer's password in plain text. At the moment it matters most, the system can't say what it's doing.
...
- **Structure it.** Emit logs as structured data (key-value/JSON), not free text, so a log platform can *query* them — and include consistent context: request ID, user ID, and the **correlation/trace ID** from the active span (OpenTelemetry populates SLF4J's MDC for exactly this). Structured, correlated logging is the single biggest logging-quality upgrade: the change that lets an engineer follow one request across five services, the capability the hook's logs lacked entirely.
...
- **High-cardinality tags are the metrics disaster.** A user ID or unbounded value as a tag explodes storage and cost; bound cardinality, instrument the golden signals, don't over-instrument.
- **Observability is not the quality of the code.** It observes the running system; it doesn't make the code correct or maintainable (Parts II–V). One axis among several, and a late one.
- **Shift-right is not a replacement for shift-left.** Relying on production to catch bugs the gates should have caught is the most expensive way to find them (Chapter 1). Observability complements the gates; it doesn't excuse skipping them.
...
- **Structured vs string-soup logging** — queryable key-value/JSON vs free text that doesn't scale; structured is the upgrade, requiring a consistent standard (Chapter 37).
...
- **Shift-right feedback vs shift-left gates** — production feedback complements the build-time gates (the loop), it doesn't replace them; the cheapest bug is the one a gate caught.

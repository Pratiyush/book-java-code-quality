# SCORING PACKET — Printed Chapter 20  (dossier 41_testing_landscape_quality)
# 1. Paste EVERYTHING below the line into a fresh chat in a DIFFERENT-VENDOR LLM (not Claude).
# 2. Save its one-pager reply VERBATIM as: 03-drafts/41_testing_landscape_quality/41_testing_landscape_quality_SCORE_INDEP.md
...
CONFIRMED 2026-06-27 (vs corrected SOURCE-PIN + BUILT module 08-companion-code/41_testing_landscape_quality/): JUnit (SOURCE-PIN §3 = JUnit 6.1.0; companion build resolves the JUnit 6 line via the aggregator junit-bom at 6.0.3 — both the pinned JUnit 6 line; aggregator-vs-row 6.0.3↔6.1.0 gap recorded in 09-flags/42_example_build_dep_decisions.md §2) and AssertJ 3.27.7 (SOURCE-PIN §3; resolved in the build). The Jupiter API atoms in the body (@TestInstance(PER_METHOD), MethodOrderer.Random, @Execution/@ResourceLock/@Isolated, assertTimeoutPreemptively, assertAll, AssertJ containsExactlyInAnyOrder, Clock.fixed/Instant.now(clock)) all compile green and pass in the built module.
⚠ verify-at-pin (genuinely unconfirmable here — NOT in SOURCE-PIN and NOT exercised by THIS module's build; routed to their owning chapters' SOURCE-VERIFY; tracked in 09-flags/41_test_pyramid_canon_and_flakiness_stats_verify_at_pin.md): remaining Part-V tool versions/GAVs not used by this module (jqwik, PITest, JaCoCo, Testcontainers, REST-assured/Pact, JMH); jqwik max-discard-ratio (5); PITest DEFAULTS/STRONGER mutator membership; JaCoCo check has NO built-in default threshold; Awaitility 100ms poll; Surefire 3.0.0-M4/M6; Micco 16%/84% (cite paper not blog); Luo ten-category list; Gruber 59% order-dependency is PYTHON-specific (not Java/universal). ⚠ AHEAD-OF-PIN: structured concurrency JEP505 preview@25. Note: the JUnit User Guide is on the 6.x line and SOURCE-PIN §3 now pins JUnit 6 (6.1.0) — the earlier "pin = 5.x" framing was stale and is corrected; the body's "JUnit 5"/Jupiter naming is the programming-model name, and every API it cites is stable across JUnit 5 and 6 (confirmed by the JUnit-6 build). SOURCE-PIN §7 canon gaps (named-book verbatims, unconfirmable here — flagged): Cohn 2009 + Meszaros 2007 not pinned rows; Vocke quotes; van Deursen 2001. FIRST = heuristic not spec.
...
DRAFT v1 — two-axis-test-quality + pyramid-as-heuristic + cause→determinism-fix-matrix + test-smell-card + umbrella-routing shapes; PART V OPENER. EXAMPLE-BUILD = BUILT (green at JDK 21.0.11, Maven 3.9.16; mvn -B -Pquality verify: 15 tests, 0 Checkstyle, 0 SpotBugs — see _EXAMPLE.md). The determinism-axis atoms below (clock-injection, per-method isolation, order-independent assertions, poll-not-sleep, assert-all) are confirmed runnable+tested in 08-companion-code/41_testing_landscape_quality/.
-->

# SCORING PACKET — Printed Chapter 13  (dossier 20_thread_safety_jmm)
# 1. Paste EVERYTHING below the line into a fresh chat in a DIFFERENT-VENDOR LLM (not Claude).
# 2. Save its one-pager reply VERBATIM as: 03-drafts/20_thread_safety_jmm/20_thread_safety_jmm_SCORE_INDEP.md
...
Companion module: 08-companion-code/20_thread_safety_jmm/ — EXAMPLE-BUILD = BUILT (green `mvn -B -Pquality verify`, Java 21.0.11; 9 tests pass, 0 Checkstyle/SpotBugs findings — racy counter's VO_VOLATILE_INCREMENT/AT_NONATOMIC_OPERATIONS_ON_SHARED_VARIABLE suppressed narrowly with a reason). Spec at foot.
...
⚠ verify-at-pin: JLS SE 25 §§ unchanged; Sonar rule titles/defaults (rules.sonarsource.com offline — use RSPEC repo); SpotBugs MT verbatim descriptions/ranks; EP severities at pin; JCIP verbatim/pages; tool-rule GAVs for SpotBugs/Sonar/PMD (the one GAV the module compiles against — com.google.errorprone:error_prone_annotations:2.36.0 — is confirmed against the built pom and the SOURCE-PIN Error Prone / NullAway rows, 2026-06-27). ⚠ AHEAD-OF-PIN: structured concurrency (preview @21/25), scoped values (GA @25), JEP 491 no-pinning (@24).
DRAFT v1 — gates manual; happens-before-spine + four-publication-idioms + version-bound-advice + library-vs-hand-rolled shapes; EXAMPLE-BUILD BUILT green (see companion module note at foot).
-->
...
<!-- include: 20_thread_safety_jmm/src/main/java/org/acme/concurrency/RacyCounter.java#racy-counter -->
...
<!-- include: 20_thread_safety_jmm/src/main/java/org/acme/concurrency/ServiceConfiguration.java#final-publication -->
...
<!-- include: 20_thread_safety_jmm/src/main/java/org/acme/concurrency/SynchronizedCounter.java#guarded-by -->
...
<!-- include: 20_thread_safety_jmm/src/main/java/org/acme/concurrency/LazyResource.java#lazy-holder -->
...
**Companion module:** `08-companion-code/20_thread_safety_jmm/` (`thread-safety-jmm`) — BUILT, green under `mvn -B -Pquality verify` on Java 21.0.11 (Checkstyle + SpotBugs, 9 tests). A deliberately data-racy `volatile` counter (`RacyCounter`, no atomicity edge), then the corrected forms via (a) `synchronized` over a two-field invariant with a `@GuardedBy("lock")`-documented field (`SynchronizedCounter`), (b) `AtomicLong` (`AtomicCounter`), (c) `final`-field immutable publication (`ServiceConfiguration`) and the initialization-on-demand holder idiom (`LazyResource`). A `WorkCoordinator` wires an executor sized from externalized `dev`/`prod` config, with a readiness probe + processed-count metric (observability) and a graceful `shutdown()` → `awaitTermination()` → `shutdownNow()` fallback (the failure path). **Failure path made visible:** `RacyCounter` trips SpotBugs `VO_VOLATILE_INCREMENT` (and the broader `AT_NONATOMIC_OPERATIONS_ON_SHARED_VARIABLE`) — the one reviewed, narrow suppression, pointed at the racy counter so the data race itself is what the SpotBugs finding exhibits; the concurrent stress test then pins only the safe invariant (a correct count never exceeds the expected total, so the assertion stays non-flaky), and under that hammer the atomic and lock-guarded counters lose no update while the racy one can. **JCStress note:** JCStress carries no pinned coordinate in `SOURCE-PIN.md`, so the `jcstress-test` region is a compiling JUnit concurrency probe of the same shape (an assertion in place of the harness's ACCEPTABLE/FORBIDDEN classification), not faked harness output (flagged to `09-flags/`). Snippet tags: `racy-counter`, `guarded-by`, `final-publication`, `lazy-holder`, `jcstress-test` (each ≤9 lines, resolved). *(No preview features in the compiled module; structured concurrency/scoped values are forward-notes only.)*

# SCORING PACKET — Printed Chapter 16  (dossier 27_checkstyle)
# 1. Paste EVERYTHING below the line into a fresh chat in a DIFFERENT-VENDOR LLM (not Claude).
# 2. Save its one-pager reply VERBATIM as: 03-drafts/27_checkstyle/27_checkstyle_SCORE_INDEP.md
...
Companion module: 08-companion-code/27_checkstyle (Checkstyle) — EXAMPLE-BUILD = GREEN (JDK 21.0.11; `mvn -B -Pquality verify` SUCCESS — 6 tests, 0 Checkstyle violations, 0 SpotBugs). Peer modules 28+30 (PMD/Error Prone) not yet authored. Spec at foot.
Verified against SOURCE-PIN: 2026-06-20 (pin re-confirmed 2026-06-27: Checkstyle = 13.6.0). Sources (each tool cited to its OWN docs; verdict routed to Ch 17):
- Checkstyle (SOURCE-PIN 13.6.0): source AST single-file ("cannot determine the type … cannot determine the full inheritance hierarchy" verbatim ⚠ verify-at-pin); Checker->TreeWalker->Check (built); severity error/warning/info/ignore (built, default error); bundled google/sun/openjdk/doc_comments configs (⚠ verify-at-pin — not in this module); LineLength max=80 default ⚠ verify-at-pin (Google config 100 ⚠ verify-at-pin; module sets house max=120); ConstantName ^[A-Z][A-Z0-9]*(_[A-Z0-9]+)*$ (CONFIRMED LIVE by engine on strip-and-rebuild); suppression filters (built — SuppressWarningsFilter/Holder); maven-checkstyle-plugin 3.6.0 bundles Checkstyle 9.3 / two-pin trap (built — engine overridden to 10.26.1); RecordComponentName/PatternVariableName (built).
...
⚠ verify-at-pin (still unconfirmed): PMD/SpotBugs/Error Prone tool versions+GAVs (those modules not yet built); per-rule DEFAULT values/thresholds/severity/default-on membership at the pinned versions (except Checkstyle ConstantName, confirmed live by the built engine); the named-doc/named-book verbatim quotes; CPD Maven minimumTokens=100; FindSecBugs pattern count; rules.sonarsource.com offline. RESOLVED by the built Checkstyle module (08-companion-code/27_checkstyle, green): Checker/TreeWalker/Check hierarchy, severity default=error, ConstantName regex, RecordComponentName/PatternVariableName, SuppressWarningsFilter/Holder, and the maven-checkstyle-plugin 3.6.0 -> engine 10.26.1 two-pin override. No AHEAD-OF-PIN items. Routes: cross-tool verdict/layered stack -> Ch 17 (key 37); custom rules -> Ch 18 (key 38); FP policy/baselines -> Ch 19 (key 39); SAST/FindSecBugs depth -> security part; concurrency slice -> Ch 13/14.

...
> *`{{VOICE.AUDIENCE}}` (technical profile example): "a working developer who wants to understand Java Quality — how it works, why it works that way, and when to reach for it — not a beginner who needs to be taught." `{{VOICE.REGISTER}}` example: "clear, precise, and example-driven."*
...
- **Problem-first.** Open on a concrete problem the reader hits, then build toward how Java Quality addresses it. Best for feature and mechanism chapters.
...
- Open with something concrete: a problem, a snippet, a figure, or a scene — never an abstract preamble. *(`one runnable, enterprise-grade module/example per chapter; the displayed snippet is a tag-region inside the compiled file; built green` sets what "a snippet" is for this book type.)*
...
- Use worked examples to carry the point, not to decorate it. *(What counts as a worked example, and any size limit, is set by `one runnable, enterprise-grade module/example per chapter; the displayed snippet is a tag-region inside the compiled file; built green`.)*
- Where a chapter carries a figure, use it to clarify a structure or flow that prose handles poorly. Figures are load-bearing; the per-chapter image budget is set by `per-chapter image budget; designed diagrams authored as HTML then rendered to PNG (never image-generated); tool/IDE screenshots captured` and `GUIDELINES.md` §8.
...
- No unsourced `rule IDs, config/ruleset keys, tool flags, API signatures, GAV coordinates, version numbers, benchmark figures, and quoted claims` (see `SOURCE-PIN.md`).
- No crowning a winner. `neutral comparative survey (see 00-strategy/NEUTRALITY.md): each option gets its strongest case and its hardest limitation; no crowning; banned: better than / unlike X / superior / beats / the problem with X` governs this; its banned phrasings are listed in `NEUTRALITY.md`. *(Technical-profile default banned set: "better than", "unlike X", "the problem with X", "superior", "beats". When an alternative appears at all, it appears only for a necessary comparison or migration, presented as a difference, never a verdict.)*
...
- A figure is **load-bearing** per the per-chapter image budget (`per-chapter image budget; designed diagrams authored as HTML then rendered to PNG (never image-generated); tool/IDE screenshots captured`, `GUIDELINES.md` §8). Refer to it before it appears, naming what it shows; never promise a diagram the chapter does not contain.
...
> *(Technical profile — see BOOK-TYPE-PROFILES.md; book types with `the build/compile gate turned off` covering example-build read "snippet" as "worked example" and drop the compile clause. The size limit, the source-trace requirement, and "introduce in prose first" are invariant.)*
...
- Every snippet is verified against the pinned authority (the pinned authority set (00-strategy/SOURCE-PIN.md) at the pins in SOURCE-PIN.md, `SOURCE-PIN.md`) and the verifying source path is recorded in the dossier/verify report. No snippet enters a draft unverified.
...
> *(Technical profile — see BOOK-TYPE-PROFILES.md; book types without a runnable `one runnable, enterprise-grade module/example per chapter; the displayed snippet is a tag-region inside the compiled file; built green` drop this whole section. The two invariant rules that survive into any book are: **a comment never restates the line**, and **quoted strings are copied, never invented**.)*

The companion artifact behind each chapter (`EXAMPLES-GUIDE.md`, `08-companion-code/NN_slug/`) is a large authenticity surface: a printed snippet is a tagged region carved out of a full file that builds green at the pins in SOURCE-PIN.md (`./mvnw -B verify`), so a sharp reader reads the surrounding code, not only the displayed lines. The same authenticity bar that governs the prose governs that code.
...
- **One name per concept across the whole book.** Pick the term `the pinned authority set (00-strategy/SOURCE-PIN.md)` uses and keep it. Do not alternate between synonyms to vary the prose — consistency outranks variety.
...
- The book is pinned to **the pinned authority set (00-strategy/SOURCE-PIN.md) at the pins in SOURCE-PIN.md** (`SOURCE-PIN.md`). State a version/edition/date only when it is load-bearing — when behaviour, a key, a figure, or a claim changed at a known point — and then state it plainly: "As of the pins in SOURCE-PIN.md, the default/finding is X."
...
- State uncertainty **once, clearly**, then move on: "This is still evolving at the pins in SOURCE-PIN.md; the current behaviour is X." Do not stack hedges — "may", "might", "possibly", "perhaps" piled together signals that the fact was never verified.
...
- **Voice a caveat by naming its condition, not by hedging.** Where a fact is version- or context-dependent, state the exact trigger tied to the pin ("As of the pins in SOURCE-PIN.md, the default is X") rather than softening with "should" or "would". Over-hedging reads as the author not knowing. *(Google tense guidance, applied to the never-invent-a-fact law.)*

...
- **Lesson (two parts).** (1) **Evidence read ≠ evidence exists.** Every gate that writes a report needs a matching reader in `status.py` (mirror the `_EXAMPLE.md` upgrade pattern — read the report and *upgrade* the cell/floor from it; never trust the older transcribed value in a sibling file). A floor copied from a stale `_SCORE.md` lies the moment a later gate changes the truth. (2) **Parsing a verdict is a real parser.** The reports legitimately (a) state PASS while *counting* failures inline ("PASS-WITH-FIXES … 0 FAIL", "no BLOCKER"), (b) open `## VERDICT: FAIL`, narrate the fix, then **upgrade** ("Verdict upgraded FAIL → PASS-WITH-FIXES"), and (c) carry narrative N/A mentions ("the dossier's N/A call was revised") on a module actually built green. A naïve "first line with PASS/FAIL" mis-flagged 5 chapters. Working rules: test PASS before FAIL; ignore negated/zero counts ("0 FAIL"); follow `→` upgrades to the target; take the LAST decisive declaration; scope "module N/A" to verdict lines. After the fix: 45/47 green, 45 CODE-REVIEW PASS / 0 FAIL — Ch03 + Ch26 opened FAIL but were fixed→upgraded, so **no unresolved FLOOR-C blocker on disk**.
- **Also (audit honesty).** `audit.jsonl` had drifted to mostly `report`-regen lines; the Phase C/D/E/G/H/I milestones were missing. Backfilled 15 at real git-log dates via a new `AUDIT_TS` override on `audit_log.sh`. AUTO per-tool capture still needs the `PostToolUse` hook wired in `settings.json` (human sign-off) — until then the milestone log depends on explicit `audit_log.sh` calls per gate, which is exactly how it fell behind.
- **Promoted-to:** NOT YET — promotion candidates for `/retro`: (1) a reporting-discipline rule that **every new gate-report type ships with its `status.py` reader in the same change** (a gate must never be done-on-disk-but-invisible); (2) prefer-freshest-artifact-per-floor (don't transcribe a floor verdict into a sibling file that won't be re-read); (3) wire the `PostToolUse` audit hook. Logged in OPEN ITEMS.

## 2026-06-27 — Phase G CODE-REVIEW (FLOOR-C 2nd half, all 45 modules): NEUTRALITY (FLOOR-A) slips live in COMPANION-CODE strings, where the prose-only AUDIT scan can't see them

- **Trigger.** Running CODE-REVIEW across all 45 built modules surfaced **two FLOOR-A banned-phrasing failures in shipped/displayed content the AUDIT gate never scanned:** Ch26 "beats" inside an `@SuppressFBWarnings(justification=…)` string that ships *inside the displayed `tag::justified-suppression` snippet* (the text readers paste), and Ch84 "better than". Both had passed every prose gate because the NEUTRALITY blocklist is scanned over **draft prose only** — Java string literals, comments, and Javadoc in `08-companion-code/**` are a blind spot. The Ch26 case is doubly load-bearing: it is both a code string and a displayed snippet region.
- **Lesson.** A book whose examples are tag-includes of compiled code has a SECOND surface for every content floor: the module's `.java` (strings/comments/Javadoc/justification args), not just the prose. FLOOR-A (and by extension FLOOR-C source-trace) must be enforced on companion-code source, because a banned verdict word or an invented atom inside a `// tag::` region reaches the reader exactly like prose does. CODE-REVIEW is currently the only gate that looks there, and it runs late — by which point the phrasing has shipped through EXAMPLE-BUILD.
- **Also confirmed (not new, but re-paid):** displayed snippets drift from the module unless the binding is checked — Ch03 duplicate `// end::`, Ch22 region starting mid-statement, Ch48 wrong boundary operator; and invention/off-pin atoms hide in code too (Ch70 unverified OWASP ordinal, Ch75 off-pin JaCoCo `0.8.16` + a false binding). `check_snippets` catches a missing/duplicate tag pair but NOT a region that is syntactically valid yet semantically wrong or off-pin — that still needs a reading gate.
- **Promoted-to:** NOT YET — promotion candidate for `/retro`. Proposed: (1) add `check_neutrality.sh` (the standing OPEN-ITEM banned-phrase pre-pass) and run it over `08-companion-code/**` source — strings, comments, Javadoc, annotation args — not just `03-drafts/**`, wired into EXAMPLE-BUILD so a banned word fails the build before it reaches CODE-REVIEW; (2) note in `GUIDELINES §… (content floors)` and the EXAMPLE-BUILD agent spec that FLOOR-A / never-invent apply to companion-code source identically to prose, since the snippet ships to the reader. Logged in OPEN ITEMS.
...
- [ ] **Wire/review `.claude/scripts/` config** — `audit.sh`, `coverage.sh`, and the source-pin scripts carry best-effort default config and the source-pin scripts assume a single repo; adapt for the multi-authority model so `/stage-report` is runnable. **(2026-06-27: confirmed `audit.sh` is currently NOT runnable — it parses the templated PIPELINE HARD-gate token `research|source-verify|…` as a literal gate name (23 false "drift" FAILs) and its AUDIT-3/3 awk dies on the CHAPTER-TRACKER with `illegal field $(NN)`. The authoritative anti-drift guard is `status.py` (clean); fix `audit.sh`'s gate-token + awk field parse before relying on it / `/stage-report`.)**
...
- [ ] **Reporting discipline: every new gate-report type ships with its `status.py` reader in the same change** (2026-06-27 maintenance finding). FLOOR-C was done-on-disk-but-invisible because the 45 `_CODEREVIEW.md` had no reader and the scoring floor was transcribed from a stale `_SCORE.md`. Promote to GUIDELINES "Reporting discipline" + a `status.py` self-check: warn if a `03-drafts/*/*_<GATE>.md` report type exists on disk that the matrix does not surface. Prefer the freshest on-disk artifact per floor; never re-read a transcribed copy.
- [ ] **Wire the `PostToolUse` audit hook** (`.claude/hooks/post-tool-audit.sh` → `settings.json`) so per-tool calls auto-append to `10-logs/audit.jsonl` (needs human sign-off — the `settings.json` edit was blocked). Until wired, the milestone trail depends on explicit `audit_log.sh` calls and silently lags.
- [ ] **Remove the stray nested `08-companion-code/08-companion-code/` path** (earlier-copy artifact). Canonical capstones live at `08-companion-code/capstones/` (the dir with `CAPSTONE-STATUS.json`); the nested duplicate should be deleted by the orchestrator (git op).
- [ ] **Extend the banned-phrase pre-pass to companion-code source** (Phase-G G3 finding). The NEUTRALITY blocklist scan must also cover `08-companion-code/**` `.java` — string literals, comments, Javadoc, annotation args (`@SuppressFBWarnings(justification=…)`) — not just `03-drafts/**`, and run **inside EXAMPLE-BUILD** so a banned word fails the build before CODE-REVIEW. G3 caught "beats" (Ch26) inside a *displayed* snippet region and "better than" (Ch84) that every prose gate had passed. Pair with a note in GUIDELINES (content floors) + the EXAMPLE-BUILD agent spec that FLOOR-A / never-invent apply to companion-code source identically to prose. (See 2026-06-27 entry.)
...
## Ch20 / key 41 — independent score (2026-06-28, chapter-scorer)

- **Emphasis is a confidence signal the ACCURACY cluster must police.** Ch 20 bolded two unpinned,
  `09-flags`-tracked statistics (Micco `16%` flaky / `84%` of post-submit pass→fail transitions are
  flakes) in the running prose — contradicting the chapter's own flag, which says cite the paper not the
  blog and lists both as verify-at-pin. This capped ACCURACY at 7 and held the aggregate at 43. The
  in-bounds fix (de-bold to attributed/flagged form, keep "verify at SOURCE-VERIFY") took A 7→8 and the
  aggregate to 44 — a clean single-pass lift with no fact change. Propose an AUDIT/score rule: a
  verify-at-pin or `09-flags`-tracked number must NOT appear in bold/italic in running prose, because
  emphasis implies a confidence the source does not yet carry. This is the ACCURACY twin of the
  em-dash-density 43→44 lever.
- **Whole-file em-dash density mis-flags reference apparatus.** Ch 20 measured 10.6/1000 whole-file but
  7.26/1000 in the reading body (hook→hand-off); the gap was entirely back-matter source-traceability
  bullets and the companion-module block (necessarily dense `—` usage). A naive whole-file scan would
  have sent a compliant chapter into a needless READABILITY lift. Propose the em-dash scan measure the
  reading body only, excluding back-matter source lists, figure captions, and companion blocks. (Pairs
  with the existing "run the em-dash numeric pre-pass before SCORE" learning — but the pre-pass must
  scope to the reading body.)
- **An independent score is reachable without `_VERIFY/_CLARITY/_AUDIT` on disk** when FLOOR-C
  compile/code-review state is in `_EXAMPLE`/`_CODEREVIEW` and the other floors are judgeable on the
  draft — but the resulting SHIP is the scoring gate ONLY. Ch 20 still owes VERIFY/CLARITY/AUDIT reports
  and the Step-5 SOURCE-VERIFY closes (Micco figures + named-book verbatims) before the human approval
  gate. Worth a tracker note so a scoring SHIP is not mistaken for full soft-gate completion.

## Ch 23 / key 48 (coverage, mutation & test effectiveness) — independent score 45/50 SHIP, 1 lift pass — 2026-06-28

- **Read body prose against the chapter's OWN back-matter/flag apparatus, not only against the pin.** Ch 48 correctly marked the JaCoCo JDK-support mapping (Java 21→0.8.11, 25→0.8.14) `⚠ @pin` in the front-matter, the back-matter, AND an open flag (`09-flags/48_jacoco_pitest_doc_defaults_verify_at_pin.md`, atom #1: "the chapter must not print an unverified default as settled fact") — yet still asserted it as settled fact in one body sentence (line 75). The flag↔body contradiction is invisible to a pin-only check because the *version* (0.8.15) IS pinned; only the historical *mapping* is unverified. One in-bounds hedge ("by the project's changelog … confirm against the release notes for your JDK") lifted ACCURACY 8→9 and took the aggregate 44→45. Propose a reconcile lint: for every atom a chapter's back-matter marks `⚠ @pin`, grep the body for an unhedged assertion of it. Prose analogue of the code-reviewer's snippet-comment/operator lint.
- **Re-confirm the em-dash budget after any hedging edit.** The hedge added two em-dashes (7.08→7.56/1000); still under <8 but a denser chapter could tip. A lift pass that adds an aside should re-run the density check rather than assume headroom.
- **"Demonstrate the thesis by scoping the failing run" is a reusable, strong failure-path pattern.** Scoping PITest to the weak test alone (`-DtargetTests=…WeakTest` → 33%, build FAILURE) makes "a covered line can be untested" reproducible on demand while the default `verify` stays green (full suite 87%). Cleanest failure-path design seen for a necessary-not-sufficient metric pairing; worth promoting into EXAMPLES-GUIDE.

## Ch 17 / key 35 (SonarQube, IDE inspections & the layered stack) — independent score 44/50 SHIP, 1 lift pass — 2026-06-28

- **A green build at an off-pin DISPLAYED version literal does not satisfy FLOOR-C source-trace by itself.** Ch 35's `layered-gate` snippet prints `<version>10.26.1</version>` (Checkstyle engine) + `3.6.0` (plugin) against the SOURCE-PIN value 13.6.0. The literal is real (builds green) and not invented, so the floor is not a FAIL — but it is off-pin and in print, and is flagged only in CODE-REVIEW F2, not in `09-flags/`. Harsh-skeptic resolution: floor PASSES (real, traceable to the build), ACCURACY takes the point (8, not 9), and the repo-wide reconcile is carried as a MANUSCRIPT-GATE condition. Propose a one-line SCORING.md FLOOR-C clause: *a displayed version atom must match the pin OR carry a `09-flags/` displayed-atom entry; a green build at an off-pin literal does not satisfy source-trace.*
- **Measure em-dash density on the prose body only — strip front-matter + figure captions.** Raw file-wide count was 43; 9 sat in the front-matter provenance HTML comment and 2 in the figure caption. True prose-body density was 8.04/1000 (lines 15–199 minus include directives + caption), lifted to 6.04 by recasting 8 appositive em-dashes. An AUDIT/score em-dash pre-pass that counts the raw file over-reports cadence readers never see; it should exclude the `<!-- ... -->` front-matter and `*Figure N …*` caption lines before dividing.
- **Before spending a lift pass, check the weakest cluster's fix is actually in-bounds.** Ch 35's tied-weakest cluster (ACCURACY) was NOT liftable in-bounds: the only fix (move the displayed Checkstyle version to the pin) collides with the CODE-REVIEW gate's explicit "fix at the aggregator repo-wide, do not patch ch 35 in isolation." Forcing it would mean editing displayed code against the gate or fabricating a flag. The loop correctly spent its one pass on the genuinely-liftable cluster (READABILITY). Add to the lift-loop discipline: when the weakest cluster's only fix is out-of-bounds (floor risk, repo-wide-reconcile, or new facts), move to the next-weakest liftable cluster rather than burning a pass.

## Ch 22 / key 45 (+folds 46) (Integration & property-based testing) — independent score 43/50 LIFT, 1 lift pass — 2026-06-28

- **A semantically-innocent word can still trip a binary scripted NEUTRALITY floor.** "kills duplication" (§Parameterized) crowns no rival — its object is a code smell — but the blocklist's scripted pre-pass matches the literal token "kills" and FLOOR A is binary, so it is a live ship-blocker until neutralized (→ "removes duplication"). The chapter reached the scorer with no `_AUDIT.md` (gates manual), so nothing had run the banned-phrase scan first. Propose: when `check_neutrality.sh` is built, scope its blocklist to denigration-of-a-named-option contexts or whitelist quality-improvement verbs over code-smell objects ("kills/removes duplication", "dead code"); and run the banned-phrase scan during scoring whenever no `_AUDIT.md` exists.
- **"Don't pad DEPTH / don't invent for @pin atoms" can legitimately strand a chapter at 43.** Both guardrails are correct, but together they fenced off every in-bounds lever here: DEPTH is right-sized to two concise Tier-B dossiers (padding barred); ACCURACY's −2 is a verify-at-pin surface (jqwik tries count, @Container per-test/static, reuse/Ryuk flags, GAVs) that can only become verified via a SOURCE-VERIFY re-fetch — out of bounds for a prose lift loop. CLARITY/UTILITY honestly at 9 (the figure-intro lever was already satisfied: prose lead-in + 3 numbered refs + caption). Propose: the lift loop may *trigger* (not perform) a targeted SOURCE-VERIFY when the sole path to the bar is converting correctly-flagged `⚠ @pin` atoms to verified, recorded as a verify step distinct from a prose pass — so the chapter isn't mislabeled CUT for lacking a prose fix.
- **Prose-only lift edits keep FLOOR-C green for free.** Both READABILITY edits (splitting a 76-word and a 59-word snippet lead-in) fell outside any tag-include marker, so the recorded `mvn -B -Pquality verify` green build stands and only `check_snippets` (4/4) needed re-running — no rebuild. The tag-region snippet model makes prose lifts cheap to re-gate; worth a note in EXAMPLES-GUIDE.

## Ch 18 (key 38+40) — INDEPENDENT score + bounded lift (2026-06-28)

- **Confirms the Ch-35 em-dash-scoping learning, with a sharper rule.** Ch 18's raw all-body em-dash was 8.79/1000; the *narration-only* figure (stripping the `## Back matter` sources/citation apparatus, the figure alt-text/captions, and `<!-- include -->` lines) was the real cadence number. Two in-bounds passes took all-body 8.79→6.65 and narration to 4.14/1000. Promote to a scripted scoping spec for the AUDIT/score em-dash pre-pass: **exclude (a) the front-matter `<!-- ... -->` block, (b) `![...]`/`*Figure N …*` lines, AND (c) the back-matter sources/traceability section** — citation lists legitimately carry dashes and otherwise inflate the tell by ~2/1000.
- **The double-appositive "X — Y — Z" is the single highest-yield conversion.** Two double-appositive sentences (4 em-dashes) plus three single appositives carried most of the over-target density; converting them to commas/parentheses moved narration ~4 points/1000 with zero fact risk. Worth calling out by name in VOICE-GUIDE's em-dash line as the first pattern to hunt.
- **Re-confirms: a flagged, deferred verify surface caps ACCURACY at 8 but is NOT a Floor-C fail, and is NOT in-bounds liftable.** Ch 18's unpinned analyzer authoring SDKs + two attributed-verbatim-but-unconfirmed JSR-269 spec spans are all routed to `09-flags/` and never asserted as settled. Correct resolution = honest 8 + SHIP-on-aggregate; the lift loop must target the genuinely-liftable cluster (READABILITY here) rather than try to "fix" the accuracy cap (which would invite fabrication or a `/pin-source` action outside lift scope). Same shape as Ch 35's off-pin-literal case — this is now a recurring pattern; recommend a one-line SCORING.md ACCURACY-anchor note: *flagged-and-deferred ≠ floor fail, but does cap the cluster ≤8.*
- **PASS-WITH-FIXES from CODE-REVIEW is a passing Floor-C gate when 0 BLOCKER / 0 FAIL.** Both Ch-18 FIX items were already resolved in the current artifact by score time (F1 Chapter-88→45 fixed in-module + rebuilt; F2 @CheckReturnValue now motivated in prose), so the score read a clean gate. Confirms the score gate must read the *current* draft + the *current* module state, not only the gate report's verdict line.

### Ch 29 (key 67+68, reproducible builds & license compliance) — independent score + 2-pass lift to 44/50 (2026-06-28)

- **A prose-owned CODE-REVIEW MINOR left unapplied is a live ACCURACY drag at score time.** CODE-REVIEW finding #1 (bare `LGPL-2.1` → SPDX-canonical `LGPL-2.1-only`) was correctly routed to the drafter but had not been applied when the independent score ran — so the draft carried a factually-wrong SPDX id in reader prose while its own companion code (`LicensePolicy`/`LicenseCategory`/`BuildIntegrityTest`) already used the canonical `-only` form. Pre-lift ACCURACY = 7 for that single defect. Recommend the reconcile/source-verify step fold prose-owned CODE-REVIEW findings back into the draft *before* the SCORE gate; the scorer closing it inside the lift loop works but is later than ideal.
- **SPDX-identifier-form is a recurring fidelity trap (2nd sighting after the CODE-REVIEW learning).** Bare `LGPL-2.1` / `GPL-2.0` / `GPL-3.0` (missing `-only`/`-or-later`) read natural but are not valid SPDX ids. A cheap VERIFY-stage lint flagging the bare forms would catch this class before it reaches scoring.
- **Re-confirms the em-dash scoping rule (Ch 18 / Ch 35 lineage): measure on reader-facing prose, exclude the back-matter traceability ledger.** Whole-file em-dash here was 10.72/1000, dominated by the back-matter sources block at 24/1000 (a dense internal record, not cadence). The honest cadence number is the Hook→Hand-off body: 8.28/1000 pre-lift, pulled to 7.59/1000 (under the <8 lever) by converting 2 flowing-prose dashes to colons. List-item definitional dashes were left intact.
- **The reproducibility-double-build is a strong cross-artifact scoring signal.** EXAMPLE-BUILD and CODE-REVIEW *independently* re-ran two `clean package` builds and both matched SHA-256 `b5b3d7be…`, and the `09-flags/` file records the same hash — three artifacts agreeing turns the chapter's headline claim from "configured" into "proven." Reinforces the proposed `check_reproducible.sh` (two builds + `shasum` + `cmp`) recorded with the SHA for any repro-claiming chapter.
- **DEPTH held honestly at 8, not padded to clear the bar.** Both source dossiers are concise Tier-B; the in-bounds rule forbids inventing the missing worked transitive-surprise example to reach 9. Correct resolution = 8 + clear the bar on the other four 9s. Same discipline as the Ch-18/Ch-35 "flagged-and-deferred caps the cluster but is not a floor fail" pattern.

## 2026-06-28 — Ch 30 (key 69, secure-coding/OWASP) independent score + 1-pass lift to 44/50 (SHIP)

- **Snippet-tag names are a recurring reader-facing leak vector.** A `// tag::` identifier (e.g. `crypto-pbkdf2`) is a build artifact, not reader vocabulary; prose referencing "the snippet above" must use a human name ("the PBKDF2 example"), never the tag id. Recommend a cheap greppable lint at CLARITY/AUDIT/score: flag any backticked token in **body** prose that exactly matches a declared `tag::` region name (exclude the back-matter snippet-tags manifest). One in-bounds reword cleared it here.
- **Hand-rolled config syntax that mirrors a framework's is a stealth ACCURACY trap.** The Ch-30 module's `application.properties` uses a `%profile.` prefix (Quarkus's profile syntax) parsed by its own loader — but the module is plain JDK and Quarkus is not in its pin. Surfacing literal `%dev`/`%prod` in teaching prose silently implies a Quarkus mechanism that is not there. House rule for JDK-only/secure-coding modules: in reader prose name the mechanism in plain language ("a development profile / a production profile"); reserve framework-specific syntax for back-matter that explicitly documents the file. Candidate promotion to EXAMPLES-GUIDE.
- **Edition-discipline is the model for "pin confirms the edition, not the full enumeration."** SOURCE-PIN's OWASP row confirms only *Top 10:2025 + A01 leads + supply-chain elevated*. The dossier→draft→flag chain correctly held the full 2025 ordinal list (A02/A03/A07/A08-style numbering) as ⚠ verify-at-pin and the draft used category **names** in prose, never asserting the 2021-style numbers as 2025-current — keeping the pin-confirmed edition fact while avoiding the standards-edition trap. Keep as the reference example for any standards chapter in the retro.
- **Re-confirms (3rd instance, after Ch 18 / Ch 35): two near-bar clusters rooted in one sentence lift together with a single in-bounds pass.** Ch 30 pass-0 = 43/50 with ACCURACY 8 + READABILITY 8 both caused by one §Crypto sentence (tag leak + unexplained `%dev/%prod` + overloaded clause). Naming the single weakest cluster (ACCURACY) and fixing that one sentence lifted ACCURACY 8→9 to reach 44; READABILITY was honestly left at 8 (ceiling effect for a dense Part-opener), not inflated to manufacture the bar. The loop must name one cluster but may fix a defect that serves two.
- **Re-confirms: CODE-REVIEW PASS-WITH-FIXES + 0 BLOCKER = a passing Floor-C gate, and the score must read current state.** Both Ch-30 FIX items were resolved in the artifact by score time (FIX-1 package-info `*Defenses`→real fix types; FIX-2 `SecurityGate`/`TokenCrypto` verified to consume `SecurityProfile.active()` so the externalized-config claim is true in the running path). Verified by re-running the build (19 tests / 0 CS / 0 SB / BUILD SUCCESS) rather than trusting the report's verdict line.

## Ch 27 (key 62+63+64, "The build & dependency hygiene") — independent score 2026-06-28

- **Em-dash density MUST be measured on printed narrative prose, not the whole `.md`.** Ch 27 whole-body = 9.38 em-dashes/1000 (over the 8 target) but the printed narrative prose (hook→"When to use what", excluding the `## Back matter` citation apparatus + figure-caption lines) = **5.9/1000 — under target**. The back-matter source-traceability block legitimately runs ~20/1000 because it is dense citation, not narration. A whole-file scan would have produced a false READABILITY flag. **Recommend the AUDIT/score em-dash scan exclude the back-matter/citation region + figure captions, or report prose-only and apparatus densities separately.** (Generalizes the Ch-30 "filler grep tripped on the editorial HTML header" lesson: always scan the *printed body*, never the editorial/apparatus regions.)
- **A floor-clean, build-green chapter can sit honestly at 42/50 — and the bar is held, not lowered.** Ch 27 floors A/B/C all PASS (banned-sweep 0; full when-NOT set; build green + CODE-REVIEW PASS + source-trace clean with one flagged atom), yet the honest aggregate is 42 (CLARITY 9 / ACC 8 / UTIL 9 / DEPTH 8 / READ 8). The 2-point gap to the 88% bar is *unverified breadth* (Gradle/SaaS/"nearest-wins"/plugin-literal atoms, all correctly `⚠ @pin`) + *concise-Tier-B depth* — **neither closable by an in-bounds prose pass** (the only +2 paths are padding DEPTH or asserting `⚠ @pin` atoms, both out of bounds). Correct disposition = **LIFT→(bounded)→flag to the human gate as a scope question**, never a manufactured 44. Record as the canonical "honest 42, bar held" case.
- **Rolling/SaaS authorities need a dated-at-use beat in the PROSE, not only in the back-matter flag.** Ch 27 marks Renovate/Dependabot `⚠ rolling` in the apparatus but the reader-facing prose introduces `renovate.json`/`dependabot.yml` schema keys as settled. The highest-leverage in-bounds lift = one sentence at the teaching first-use naming them as continuously-released hosted tools whose schema is confirmed against current docs (restates SOURCE-PIN §4 status; no new fact). **Suggested drafter checklist item: any `⚠ rolling` authority gets a one-sentence "confirm schema against current docs" beat at teaching first-use**, per VOICE-GUIDE "voice a caveat by naming its condition."
- **Independent gate divergence from self-score is in disposition, not the number.** Self-score = 42 with FLOOR-C COMPILE marked PENDING; the build has since gone green so floors now all PASS — yet the aggregate is unchanged and still under 44. The independent gate's job was to refuse to read "floors now pass" as "ship," hold the 44 bar, and route to LIFT/flag. Confirms: an independent re-score is a *gate*, not a recount.

## Ch 26 (key 55 · enforcing architecture / fitness functions) — independent score 45/50, SHIP

- **Gate-report freshness lag is a scoring hazard — score against the LIVE module, never the report's counts.** For Ch 26, `_EXAMPLE.md` said "8 tests" and `_CODEREVIEW.md` listed FIX-5a as OPEN with "5 ArchitectureFitnessTest", but the live test file had 6 arch tests (FIX-5a already applied) and surefire showed 9 total — the draft front-matter (9 tests) was the only artifact matching truth. **Lesson: the scorer reads `src/` + `target/surefire-reports/` directly and re-runs the build; gate-report counts are advisory.** Reporting-discipline fix: when a CODE-REVIEW FIX is applied, re-stamp `_CODEREVIEW.md` (or append an "applied" row) so a report never claims an open fix that is closed.
- **Freeze/ratchet tests must assert BOTH halves; the clean in-bounds recipe is a description-keyed second store.** FIX-5b: the freezing test asserted only `isNotNull()` plus two baseline re-checks (proves "old suppressed", not "new caught"). Added the discriminating half WITHOUT touching the displayed `freezing-ratchet` tag region by freezing a **separately-`.as(...)`-described** copy of the rule over an empty `CLEAN_LAYERS` baseline, then `assertThatThrownBy(() -> freeze(fresh).check(LAYERS))`. Rebuilt green (9 tests), check_snippets 5/5. Promote the two-phase shape (record-baseline → introduce-new-breach → assert-still-fails) to `EXAMPLES-GUIDE`.
- **A FLOOR-C code-rigor fix is distinct from a cluster lift-loop pass — log it as such.** The draft cleared the 44 bar on first independent score (45/50); the FIX-5b edit was a floor-rigor fix to the companion module, not one of the ≤3 cluster lift passes. Keeping them separate in the lift-pass log preserves the honest "no lift needed" record while still showing the floor work.
- **Live-jar `javap` against the pinned artifact in `~/.m2` is near-free ACCURACY insurance.** Confirmed `CanOverrideDescription.as`, `FreezingArchRule.freeze/.as`, `layeredArchitecture`, `DoNotIncludeTests` are real ArchUnit 1.4.2 API before trusting the prose/edit. Suggest the scorer default to a `javap` spot-check on any load-bearing API atom when the pinned jar is already resolved.
- **An unpinned named-canon source caps ACCURACY even when everything else is clean.** Ch 26's central fitness-function definition/taxonomy + *Building Evolutionary Architectures* attribution are correctly ⚠ verify-at-pin → `09-flags/55_*` because BEA is still not a SOURCE-PIN §7 canon row. Floor C source-trace PASSES (flagged, not asserted), but ACCURACY sits at 9 not 10. The +1 is a pin/scope action (add the §7 row, re-trace, clear the flag), never an in-bounds prose lift.
- **Cross-reference DIRECTION is a silent ACCURACY leak — resolve every "later/earlier part" against FINAL_INDEX.** Ch 24 (key 50) routed revapi/japicmp four times as "a later part," but key 60 folds into **Ch 7** (Part II) — an *earlier* chapter. A cold read almost misses it because the phrasing is plausible. Lift pass 1 corrected all four to "Chapter 7." Suggest a `lint_xrefs.sh` / reconcile-gate check that resolves every "Chapter N", "Part X", and "later/earlier part" against `01-index/FINAL_INDEX.md` + the merge map and flags direction mismatches (folded-key chapter number vs the current chapter's position).
- **Score em-dash density on the reading body separately from the citation back-matter.** Ch 24 whole-file narration read 9.42→7.86/1000, but the reading body (hook→hand-off) was 4.57→4.54 after fixes; the residual is dense source-trace back-matter using em-dashes as legitimate citation separators. The AUDIT/score em-dash scan should report the hook→hand-off body figure as primary and treat back-matter as reference apparatus, or it over-flags clean prose.
- **Read the live code for FLOOR C, not just the gate report — a "pending" fix may already be landed.** Ch 24 `_CODEREVIEW.md` logged F1 (the `rubberStampingAWrongBaselineHidesABug` test asserting the inverse of its name) as a pending PRIORITY lift item, but the test source already had the correct fix (render wrong output → approve it unscrutinised → assert a re-verify PASSES → assert 14999≠6200). Reading `src/` directly is what let FLOOR C score PASS rather than blocked. Confirms the Ch-26 "score the live module, never the report's counts" lesson — extend it to fix-status, not just counts.
- **The lift loop legitimately bottoming out below the bar is a valid outcome when the gap is a human-only blocker.** Ch 24 reached an honest 42/50 after the full 3 passes; the last 2 points needed runtime provisioning (real Pact provider verification + REST-assured via Testcontainers) + a SOURCE-PIN row for ApprovalTests.Java — both already in `09-flags/`. Do not manufacture the points by padding DEPTH or inflating clusters to "exceptional"; route to the human gate with the specific out-of-bounds move named. Cited-not-built honestly caps UTILITY (not a copy-me production setup) without failing FLOOR C.

---

## Ch 25 (key 53 + folds 54/57) — independent score + bounded lift (2026-06-28)

- **Em-dash density is the single cheapest READABILITY lever — make it a scripted draft-time pre-pass.** Ch 25 sat at 13.1/1000 in the reading body (≈1.6× the house <8 norm; the chapter had been flagged em-dash-heavy ~54). ≈22 mechanical mid-sentence dash→colon/comma/period conversions, using only already-verified material, moved READABILITY 7→9 with zero content change. Propose `check_em_dash.sh`: count `—` per 1000 prose words, excluding the HTML front-matter dossier comment, bold list-label dashes (`- **SRP —**`), and back-matter citation rows (where dashes are legitimate separators). Report the hook→hand-off body figure as primary.
- **Number cross-refs; never leave "the metrics chapter"/"the next chapter" as bare phrases.** Ch 25 named routed chapters only by role. Pinning them to "Chapter 2" (folds key 04) and "Chapter 26" (enforcement) — validated against `01-index/FINAL_INDEX.md` + the merge map — both removed dashes and made the refs greppable. Reinforces the Ch-24 `lint_xrefs.sh` proposal: every "Chapter N"/"Part X"/"later-earlier" reference must resolve to a FINAL_INDEX row and direction.
- **Read the figure RASTER before docking ACCURACY for a figure-drift the flag describes.** The Ch 25 flag still said `fig53_1.png` "shows Ch 15 until re-rendered"; reading the PNG showed it was already re-rendered ("Chapter 5", zero "Ch 15"). I nearly held ACCURACY at 8 for a defect that no longer exists. Flags lag artifacts — verify the live asset (PNG, src, build) before a cluster deduction. Extends the Ch-24/26 "score the live thing, not the report" lesson to figures.
- **Confined, correctly-routed `⚠` canon-gap markers are rubric-CORRECT and do not cap ACCURACY below 9.** Ch 25's named canon (Martin SOLID, North CUPID, Liskov-Wing LSP, Constantine-Yourdon) is not in SOURCE-PIN §7; it is used as attributed concepts, body-prose marker-free, with all `⚠` confined to back-matter rows pointing at `09-flags/`. That is the honest handling of non-pinnable secondary sources — FLOOR C PASSES and ACCURACY earns 9 (not a deduction). Scorers should not reflexively dock for the *presence* of `⚠` when it is back-matter-confined and routed; dock only when an unconfirmable atom is *asserted as fact in the body*.
- **DEPTH must be honestly capped, not padded, when a chapter deliberately defers scope.** Ch 25 routes metric definitions to Ch 2 and enforcement to Ch 26 to avoid triplication; several threads are named not developed. That correctly holds DEPTH at 8. The bar (44) was met via legitimate READABILITY + ACCURACY lifts, never by adding deferred material — padding DEPTH would have duplicated two other chapters and is out of bounds.
- **A prose-only lift needs no rebuild/`check_snippets`.** Only the draft `.md` body was edited (no `08-companion-code/` file touched), so the green build + CODE-REVIEW from `_EXAMPLE.md`/`_CODEREVIEW.md` stand; FLOOR-C COMPILE/CODE-REVIEW are re-asserted from those reports, not re-run. Confirm 0 prose edits crossed into `<!-- include: -->` lines (6/6 intact here).

## Ch 33 (key 75) — independent score 44/50 SHIP (2026-06-28)

- **A re-pin must sweep displayed-snippet CROSS-REFS, not just version digits.** The 2026-06-27 orchestrator fix corrected JaCoCo `0.8.16 → 0.8.15` in `ci/quality-gates.yml:67` but introduced a worse defect in the same displayed comment: `see Ch 48` — the **dossier key 48 written as a printed chapter number**. Coverage is printed **Ch 23** (FINAL_INDEX L52; the chapter body cites "Ch 23" 4×); the book has 47 chapters, so "Ch 48" resolves to nothing. Fixed to `see Ch 23`, rebuilt green (11 tests / 0 Checkstyle / 0 SpotBugs), check_snippets 7/7. **Suggestion:** extend `check_snippets`/`reconcile_facts` to grep displayed `# tag::`/`// tag::` regions for `Ch <NN>`/`Chapter <NN>` and assert each resolves to a real printed chapter in FINAL_INDEX — and specifically warn when an `NN` matches a *dossier key* but not a printed chapter (the exact confusion here). Cross-refs are load-bearing facts.
- **Dossier-key vs printed-chapter is a recurring hand-off confusion.** Any `Ch NN` an orchestrator writes into prose/snippets must be checked against the *printed* number, never the frozen dossier key. Worth a one-line guardrail in PIPELINE.md's reconcile step.
- **Scope FLOOR-C/ACCURACY to reader-visible facts.** The write-only `evaluations` counter (CODE-REVIEW M1) is internal/non-displayed — correctly a deferred MINOR, not a source-trace failure. Don't let internal code nits read as FLOOR-C blockers.
- **Score an umbrella opener for correct DEFERRAL, not against it.** DEPTH 8 here reflects deliberate routing onward (coverage→Ch 34, branch-protection→Ch 35, DORA→Part X), not thinness; a lift pass would be scope-creep into the next chapters. "DEPTH = verified substance, never word count" handled this cleanly. Bar (44) met without a lift loop once the cross-ref was repaired.

## Ch 34 (key 80+77+78) — independent score (chapter-scorer, 2026-06-28)
- **A correctly-flagged rolling/SaaS atom must NOT lower ACCURACY — only an unflagged drift does.** Ch 34's main-loop self-score docked ACCURACY −2 for the SaaS/verify-at-pin surface (Codecov/Coveralls/Danger, GitHub Actions `@v4/@v5`, Sonar PR analysis, platform syntax). On independent read that surface is *correctly* carried dated-at-use against SOURCE-PIN §5 + `09-flags/80_coverage_pr_platforms_saas_dated_at_use.md` — which the ACCURACY 9–10 anchor sanctions ("fully traced … zero drift"; flagging is the path for non-pinnable rolling atoms). The deduction was double-counting an honest flag. Proposed `SCORING.md` ACCURACY-anchor note. This single correction is the 42→44 gap.
- **Greppable Step-8 pre-pass: flag any `Ch <key>` where `<key>` > chapter count (47).** Ch 34 body prose correctly maps key 84 → Chapter 37, but the dossier-style back-matter leaked the raw key ("Ch 84"). A `Ch <n>` with n above the FINAL_INDEX chapter count is almost always a *key* mis-written as a *chapter* — cheap to grep, catches a recurring cosmetic ACCURACY nit before print.
- **Re-pin ripple reaches non-built comments (third sighting; JaCoCo 0.8.16→0.8.15).** Active `<version>` literals, README, and prose were re-pinned cleanly; two `pom.xml`/CI *comments* lagged with stale "0.8.16 row"/"deviation" framing (already in `_CODEREVIEW.md`). Standing fix reaffirmed: after any re-pin, `grep -rn "<old-version>"` the whole `08-companion-code/` tree including comments, not just active literals.
- **No code touched in a pure scoring pass → no rebuild / no check_snippets.** FLOOR-C COMPILE (green 2026-06-26) + CODE-REVIEW (PASS 2026-06-27) were read from `_EXAMPLE.md`/`_CODEREVIEW.md` and re-asserted, not re-run; the scorer only read + measured (em-dash density, banned-phrase sweep, cross-ref vs FINAL_INDEX, version atoms vs pom/SOURCE-PIN). Verdict: SHIP at 44/50.

### Ch 35 (key 81, folds 82) — independent score, 2026-06-28 → SHIP 44/50
- **Independent gate recomputes the scriptable READABILITY/ACCURACY checks rather than inheriting them.** Em-dash density (6.54/1000, under the ~8 ceiling) and printed-cross-ref validity vs FINAL_INDEX were recomputed cold. All *printed* cross-refs (Ch 33/34/36/37/6/17/20/22/27/29/31; Part IX = 33–36) validate; the non-printed HTML research header alone routes "Ch 1 key 06" where key 06 actually lives in Ch 4 — confirms the standing "scan the header's chapter routes too, they seed the next agent" rule, extended to the *research header comment*, not only the back matter.
- **Folded-dossier chapters (two keys → one chapter) systematically land DEPTH at 8, not 9** — each half is a complete-but-compact section-pair. Correct behaviour (no padding to reach 9); the chapter clears the 44 bar with little headroom by carrying 9s elsewhere. Candidate `SCORING.md` note: do NOT lift a fold chapter by padding the thinner half — lift a different cluster or ship at 44.
- **SOURCE-TRACE half of FLOOR C stayed independent even though COMPILE/CODE-REVIEW were read from reports:** the scorer spot-checked that all 5 displayed `tag::` regions resolve to real files in `08-companion-code/81_.../` (5/5) — that spot-check is what keeps it a verdict, not a rubber stamp.

## Ch 36 (key 83) `release_quality` — independent score 45/50 SHIP (2026-06-28)

- **Cross-reference chapter NUMBERS are a recurring, low-visibility accuracy leak that the build/code-review gates do not catch.** Ch 36 built green and passed CODE-REVIEW yet shipped two wrong in-prose cross-refs vs `FINAL_INDEX`: "semver contract of **Chapter 26**" (semver = Ch 7) and "the design flaw of **Chapter 30**" for the logic-flaw class (that catch = Ch 37). EXAMPLE-BUILD/CODE-REVIEW validate code atoms + source-trace, not that a "Chapter NN"/"Part N" pointer resolves to the right title. **Recommend** a scripted `lint_xrefs` pre-pass at Step 4a: resolve every "Chapter NN"/"Part N" against `01-index/FINAL_INDEX.md` and fuzzy-match the cited topic word to that row's title. Cheap; removes a whole class of reader-facing error pre-score.
- **A draft citing the SAME chapter number for two different topics is a strong self-inconsistency signal** — this draft used "Chapter 26" correctly (fitness functions) AND incorrectly (semver) in one file. Worth a `lint_xrefs` heuristic.
- **DORA-by-names-only is the clean pattern for a chapter that needs DORA but not its bands.** Ch 36 asserts the stability metric NAMES (change-failure rate, failed-deployment recovery time) + the speed/stability correlation FRAMING (both pinned, SOURCE-PIN §5) and parks the elite/high/medium/low bands as `⚠ still @pin` in back-matter — no numeric band in the body. Model for the other DORA-adjacent chapters (33, 38, 44).
- **The "tested-policy-core + illustrative-config" companion shape (peers 62/67/75/83) keeps FLOOR C clean and reviews fast** — re-confirmed; worth naming canonical in `EXAMPLES-GUIDE` for any "gate/policy made runnable" chapter.

### Ch 38 (key 85, folds 87+88) — independent re-score, 2026-06-28 → SHIP 44/50 (C9/A8/U9/D9/R9)
- **A stale independent score must never gate a revised draft.** The prior re-score (Sonnet 4.6, 40/50, 2026-06-20) was correct for the revision it read, but the draft was edited 2026-06-27 and every blocking lift item was applied (em-dash 14.1→7.0/1,000; banned `easy`→"straightforward on a greenfield"/"cheap to collect"; figure intro sentence added L40; the L110 meeting-reframe added). The independent gate must re-measure the current file empirically (em-dash, banned words, figure intro, cross-refs, snippet resolution) rather than carry forward the prior cluster numbers — that re-measurement tipped a stale 40 to a true 44. Suggest the scorecard header record the **draft mtime** it scored, so a later reader can tell whether a score predates a revision.
- **"Model the mechanism, flag the figure" earns the ACCURACY ceiling honestly.** Cleanest book example of a `⚠ verify-at-pin` headline number (DORA performance bands) refused in *both* prose and code while the definitional/algorithmic core (four-key formulas, baseline+ratchet) stays runnable+pinned (§5+§7, corrected 2026-06-27). ACCURACY is honestly capped at 8 only by the §7 canon gap (SPACE is not a pinned row; "failed-deployment recovery time" is a paraphrase of "time to restore service") — not by any invention. A chapter can SHIP at the bar with a flagged canon gap, but the cluster cannot reach 9 until `/pin-source` closes it. The pin/runbook action, not an in-bounds prose lift, is the path to 9 — the lift loop cannot fix a canon gap.
- **A flagged-unverified atom belonging to a sibling chapter imposes no penalty here.** The DORA "capabilities-over-maturity" framing the task flagged is a Ch 47/key 110 atom (per the key-85 flag's own cross-reference §), and does not appear in the Ch 38 draft — so it carried no load-bearing-unverified cost in this score. Scorers should locate where a flagged atom is actually asserted before capping a cluster for it.
- **Cross-ref integrity held across the 47-chapter merge.** All ten numbered cross-references resolved to the correct merged-chapter topic against FINAL_INDEX, including folded-key routing (Ch 19 = key 39 "living with findings"; Ch 34 = key 80 clean-as-you-code). The self-label "Figure 38.1" matches the printed chapter number while siblings are referenced by printed number — consistent, no drift.

### Ch 39 (key 91, folds 92+93+95) — independent re-score, 2026-06-28 → SHIP 42/50 (C9/A8/U8/D9/R8), 42→44 gap disclosed
- **Re-score the current artifact, never the artifact the last pass saw.** The prior independent score (Sonnet 4.6, 37/50, 2026-06-20) was correct for the draft it read (EXAMPLE-BUILD PENDING, em-dash 19.7/1,000, figure dropped cold). The live draft had moved on: EXAMPLE-BUILD green + CODE-REVIEW PASS (2026-06-27), six tag-include snippets embedded, em-dash audit done (~0.3/1,000 in running prose), figure now introduced in prose, banned words removed. Carrying forward the stale cluster notes would have under-scored by ~5 points. The independent gate must empirically re-measure (em-dash on the running-prose span, banned-word grep, cross-ref-vs-FINAL_INDEX, snippet-tag 1:1) on the file as it stands.
- **Measure em-dash density on the running-prose span, not the whole file.** Whole-file em-dash count = 57; running-prose body (Hook→Hand-off) = 1. The other 56 live in the non-printed HTML-comment header and the back-matter citation apparatus — not prose, must not count against READABILITY. A naive `grep -c '—'` would falsely fail a clean chapter. Codify: density is measured over the printed running-prose line span, excluding the comment header and the back-matter source list.
- **Cross-ref-vs-book-of-record is a high-yield, low-cost independent-score check.** One grep of every printed "Chapter NN" against FINAL_INDEX caught a real misroute (Boy Scout Rule → "Chapter 1"; key 06 is actually Chapter 4) that three prior gate passes missed because the dossier's own routing notes carried the same error. Fixed in-bounds this pass (printed CONCEPT callout + two back-matter lines → Ch 4). Add as a standing chapter-scorer checklist line: resolve every printed cross-reference to its FINAL_INDEX row by key, not by remembered topic.
- **An honest verify-at-pin flag caps ACCURACY at 8 — it is not a defect, and the lift loop cannot clear it.** The named-canon wording (Fowler 2e, Feathers WELC) and OpenRewrite recipe-id spelling are correctly flagged ⚠ verify-at-pin (text not redistributable; network-gated) and routed to 09-flags/. That is the right behavior, and it legitimately holds ACCURACY below 9 until SOURCE-VERIFY clears the flag. The cap is a property of the pin state, not the prose; the path to 9 is /pin-source, not an in-bounds prose lift.
- **A correctly-scoped-out scale is a disclosed UTILITY cap, not a hole to paper over.** The migration (OpenRewrite) scale is network-gated, honestly recorded as REPRO-pending and not-built — Floor-C-honest and the right call — and it legitimately holds UTILITY at 8 (three of four scales demonstrated, the fourth described). Lifting to 9 requires building that scale, not an in-bounds prose move. The 42→44 gap is therefore gated on two named non-prose, non-fakeable events (SOURCE-VERIFY clearing the canon/recipe flags → ACCURACY 9; building the OpenRewrite scale when the network gate lifts → UTILITY 9), and is disclosed to the human gate rather than closed by inflating clusters. A harsh scorer records the gap honestly and refuses to pad DEPTH or un-flag an atom to manufacture 44.

---

## 2026-06-28 — Ch 41 (key 97) independent re-score (Opus 4.8): hook↔body contradiction; floor false-positives; un-liftable ceilings

- **A hook can pass every content-floor and still carry a thesis-defeating accuracy bug.** Ch 41's hook said the AI "imports a dependency that does not exist" (a *loud* build failure) while the body correctly taught slopsquatting (a hallucinated name that an attacker pre-registers, so it *resolves silently*). Floor-clean, yet it taught the wrong, self-defeating version of its own "zero visible warning signs" thesis at the highest-visibility point. Fixed in-bounds (slopsquatting mechanism already verified in the body + the flag file). Proposed CLARITY/SOURCE-VERIFY checklist line: "hook claims must match the body's mechanism; where the thesis is 'this fails silently,' verify the hook example actually fails silently."
- **Mutation-testing "kills" is a standing NEUTRALITY pre-pass false-positive.** "A spec-derived test that kills them" = kills mutants (PITest term-of-art), not a rival-crowning verdict. The future `check_neutrality.sh` must whitelist "kill(s) … mutant(s)" or it will FAIL every mutation chapter (Ch 23, Ch 41, …). Already cleared by hand here and by the prior CODE-REVIEW dim-6.
- **The lift loop needs a terminal state for "no in-bounds lever to the bar" (distinct from "3 passes exhausted").** Ch 41 sits at 42/50: ACCURACY is capped by un-pinnable AI-era figures (arXiv 2502.01853 / 2409.19182 / CodeScene guardrails) and UTILITY/DEPTH by opener-chapter scope. The only move to 44 is pinning a dated primary AI-stat source (a SOURCE-PIN §7 decision) — out of bounds for an in-bounds prose loop. Such a chapter should route to SOURCE-VERIFY / the human gate, not CUT. Do not manufacture the 44 by padding or by asserting flagged figures.
- **Prose lift passes can silently breach the em-dash ceiling.** One inserted appositive moved body density 7.68→8.07/1000 (over 8); re-running the density check and converting to parentheses restored 7.62. Re-check em-dash density on every prose pass.
- **Re-score when the artifact materially changes.** The prior independent score (2026-06-20, 39/50) predated the companion module; once the module went BUILT-GREEN + CODE-REVIEW-PASS and the snippets were embedded, FLOOR C gained live COMPILE+CODE-REVIEW evidence and the clusters needed a fresh independent judgment.

## Ch 44 (key 105, perf-regression gates) — independent score + bounded lift (40 → 44/50, 2026-06-28)

- **Alt-text duplicating the caption is a recurring accessibility miss no scripted gate flags.** Fig 44.1's alt-text was the caption verbatim. Caption and alt-text do different jobs: caption = identifying label/thesis; alt-text = a content description for a reader who cannot see the diagram. Fixed in-bounds (alt now describes the four-stage flow + noise band + knobs, every element tracing to the figure sources). Suggest FIGURE-GUIDE require alt-text to describe rendered content, plus a lint flagging alt==caption.
- **Plain-language-first glossing slips most on terms that first appear in the subtitle/hook.** `baseline` and `tolerance band` debuted bare in the hook/Overview before any gloss. Structural fix: gloss at the first *body* use even when the term debuted in the title. Worth a VOICE-GUIDE one-liner.
- **The em-dash ceiling is a reliable mechanical lift lever.** 9.11 → 7.26/1,000 with ~5 appositive→period/comma conversions, zero meaning lost; the appositive cadence is the densest single AI tell. Surface the per-draft em-dash count to the drafter before the scoring gate so it is fixed at draft time. (Consistent with the prior learning to measure density on the printed running-prose span only — header comment + back-matter excluded.)
- **Honest deferral of a measured number protected FLOOR C and bounded DEPTH honestly.** The single move that would most lift DEPTH (a worked band-calibration on a real CI variance distribution) is exactly the REPRO PENDING-RUNTIME the chapter defers; forcing it would invent a benchmark figure → FLOOR C FAIL. The loop stopped at 44 rather than reach a ninth DEPTH point by fabrication. Worked example that "do not pad / do not invent" and "never lower the bar" coexist: lift the cluster with a real in-bounds move (UTILITY — surface the already-verified half-width→band mechanism), hold the one without (DEPTH).
- **Re-confirmed: cross-ref-vs-FINAL_INDEX is high-yield, low-cost.** All seven printed "Chapter NN" refs (20/26/33/34/35/38/43) resolved correctly by key; "previous/next chapter" = Ch 43/45 correct. The dossier-comment Part-XIII-close tension lives only in the non-printed header; printed prose is consistent with the book of record. No misroute this time — but the check is cheap and catches what topic-memory misses.

## 2026-06-28 — Ch 47 / key 110 (maturity model & roadmap) — independent re-score (Opus 4.8), lifted 41→44

- **For a pure-synthesis closer, the cross-references ARE the load-bearing facts.** The only real accuracy defect in the book's final chapter was a chapter pointer drifting from the LOCKED FINAL_INDEX: ArchUnit cited as "Chapters 16, 25" when ArchUnit + fitness functions live in **Ch 26** (keys 55/33) and Ch 16 is the four bug/style analyzers (keys 27–30). No tool/version was wrong — yet the pointer drift was a genuine ACCURACY ding (7) because the chapter's whole value is "the book mapped into an order." Fix moved ACCURACY 7→9. **Promote candidate:** a scripted `check_xrefs.sh` (or a `reconcile_facts` clause) that extracts every "Chapter NN / key NN" claim and resolves it against `01-index/FINAL_INDEX.md`; mismatch = ACCURACY ding. Cheapest high-yield check for synthesis/closer chapters.
- **Re-confirmed (third instance): em-dash density must exclude the back-matter sourcing block.** Whole-file density read 10.1/1,000; the chapter's reader-facing prose was 7.0/1,000 — already within the ~8 target. The inflation came entirely from the back-matter traceability apparatus (routing arrows `→`, the one dense source-map bullet). This now matches two prior learnings; **strong enough to promote into VOICE-GUIDE / the AUDIT em-dash scan spec**: measure on the printed running-prose span only (exclude the header comment and `## Back matter — sources & traceability`).
- **DEPTH is correctly capped at 8 for synthesis-by-design — not penalized, and not padded to 9.** The chapter adds no new primary atom on purpose; the only DEPTH-lifting move would have been inventing material (out of bounds). The loop pivoted to READABILITY instead. Worked example that "do not pad / never lower the bar" and "ship at 44" coexist.
- **The load-bearing-unverified DORA framing was handled exactly right and did not need capping below 9.** The chapter's central organizing lens (DORA capabilities-over-maturity-levels) is `⚠ verify-at-pin` (flag 85), but it is attributed by name, hedged ("still being confirmed at the pin"), self-flagged in front- and back-matter, asserts **no** DORA band/statistic, and the code names no DORA tier. Because the rationale stands on its own and the claim is never asserted as settled fact, ACCURACY is not bought by overstatement — so the honest handling earns a 9, not a penalty. The pin action (add DORA/SPACE as SOURCE-PIN §7 rows; re-confirm wording) remains owed at `/pin-source` before Step 16.

## Ch 45 (key 106 +107 +108) — independent re-score + bounded lift (Opus 4.8, 2026-06-28)

- **An unpinned-authority cluster has a hard ACCURACY/DEPTH ceiling the lift loop cannot raise — escalate, don't score around.** Ch 45's named standards (SLF4J, Micrometer, OpenTelemetry, Google SRE, Sentry) are all TO-PIN (SOURCE-PIN §7). The chapter handles them correctly (attributed + flagged), but that caps ACCURACY and DEPTH at ~8 each no matter how clean the prose, because "fully traced, zero drift" (9) is impossible without the pin rows. The example-builder flagged the same fork for keys 106/107/108. The lift loop reached 40/50 (floors all PASS) and stopped — 44 is structurally unreachable in-bounds. Correct outcome: route to the human gate as "floors-pass, pin-gated," and let the maintainer make ONE SOURCE-PIN decision for the whole observability cluster rather than per chapter. (Mirrors the concurrency/jcstress unpinned-tool fork.)
- **Re-confirmed (Ch 44 learning): scope the em-dash check to the printed running-prose span.** The standing "Ch 45 em-dash-heavy ~41" flag double-counted the dossier-header HTML comment (not prose) AND the back-matter "sources & traceability" ledger (where em-dashes are field separators). Body-narrative density was 7.5/1000 — within the ~8 ceiling — so a lift pass spent thinning body em-dashes would have chased a non-problem. The drag was elsewhere (a cold figure, two unattributed trend claims). Lesson: confirm WHERE the mechanical tell actually lives before spending a bounded pass on it.
- **A figure under "How it works" needs a prose lead-in and a caption distinct from its alt-text.** Ch 45's figure appeared cold (heading → image → caption that was a verbatim copy of the alt-text → next subsection, with a stray double blank line). The fix — a one-paragraph orientation naming the shape the figure shows + a caption that teaches the loop steps (every element already in the figure sources.md, no new facts) — was a clean in-bounds READABILITY/CLARITY lift (+1 READABILITY). Pairs with the Ch 44 alt==caption learning: caption and alt-text do different jobs and must not be identical, and a load-bearing figure earns its place with a sentence of setup.
- **Two independent models converged on the same verdict (LIFT-exhausted / not ship) from different aggregates (35 vs 40).** The gap was entirely in the READABILITY em-dash measurement and cross-ref confidence — not in the structural ceiling diagnosis. The floors and the pin-gated ceiling are robust across scorers; the cluster point estimates are where model variance shows. The independent-re-score gate is working, and the binding decision (the SOURCE-PIN cluster decision) is correctly escalated to a human rather than scored around.

### Ch 46 (key 109, CAPSTONE) — independent re-score + bounded lift, 2026-06-28 (Opus 4.8)
- **Far-right padded inline comments in displayed XML are a print-render trap that `check_snippets` cannot see.** The capstone's marquee `checkstyle-two-pin` pom snippet passed check_snippets and the element-balance audit yet rendered mangled in print: lines were 97/95 cols because trailing `<!-- … -->` comments were column-aligned far to the right, so they soft-wrap in a narrow print code box and visually jam under the value. The failure mode is *display width on wrap*, not malformed XML. Fix: collapse to the single-space trailing-comment style the module's own `spotless-reference.xml` already used (97→78, 95→70 cols; still ≤9 lines; xmllint well-formed; build unaffected since it is comment-whitespace only). Suggest a max-display-width check (~80 cols) + an "aligned trailing comment" flag added to `check_snippets.sh`, and an EXAMPLES-GUIDE one-liner: displayed-snippet inline comments use a single leading space, never column alignment. Highest-impact on the capstone, which has the most wide full-file lines.
- **A stale independent score must be re-read cold, not deltaed.** The prior `_SCORE_INDEP` (38/50, Sonnet 4.6, 2026-06-20) carried READABILITY caps for self-narration and "carve-out" repetition the drafter had since removed, and a UTILITY cap for a build that is now EXAMPLE-BUILD = PASS with in-prose module snippets. Scoring the *current* draft cold (not subtracting from the old card) recovered 6 legitimate points to 44/50 SHIP. Suggest the scorer diff draft mtime vs prior-scorecard date and re-verify each cited offending line still exists before carrying any cap forward.
- **Scope AI-tell metrics (em-dash, hedges) to reader-facing prose only.** Whole-file em-dash density (9.32/1000) was dominated by the machine-shorthand back-matter dossier dump; the prose a reader actually reads was 9.29 → 5.93/1000 after ~9 in-bounds appositive→colon/comma conversions. A whole-file metric would have demanded harmful cuts to clean prose. (Re-confirms the prior printed-running-prose-span learning.)

## Ch 8 (key 10) — READABILITY lift-pass #1 + re-score (2026-06-28, independent)
- **The em-dash density verdict is a denominator argument — fix the denominator before lifting.** Ch 8's prior independent card called READABILITY=7 on "~44 dashes / ~3.9k words ≈ 11–13/1,000," but 44 was the *whole-file* count including the protected back-matter `**Label** — description` source list (21), the HTML front-matter comment (6), captions/callouts (5), and code (1). On printed running narration (protecting quotes/captions/tables/bold-labels) the draft was already ~5.7/1,000; the lift took it to ~0.4/1,000 (whole-file 44→31). This is the third chapter to hit this — promote it: the AUDIT-gate em-dash scan must strip fenced code, table rows, blockquotes/callouts, figure captions, the HTML header comment, and bold-label list rows, and report the narration denominator, before flagging density.
- **When the weakest cluster (READABILITY) and the densest mechanism are the same lines, one pass moves two clusters.** Converting the four-contracts appositive dashes *and* adding a plain-language lead per contract was a single edit to the same paragraphs and moved R 7→9 and C 8→9 (aggregate 39→42). Run cadence + plain-language-first as one work order.
- **A prose lift cannot honestly close an ACCURACY/verified-material ceiling — mark per-point whether the path is prose or verify.** Ch 8's last 2 points to the 44 bar are an ACCURACY ceiling (two cited-only SpotBugs IDs needing a docs re-fetch to verbatim-confirm) and a reference-dense UTILITY section, not cadence. Manufacturing a 9 by editing prose would be padding; the loop correctly stopped at the prose ceiling and routed the rest to a source/verify task. Suggest the scorecard tag each remaining point prose|verified-material so the bounded loop does not grind passes on cadence already at target.
- **Atom-diff after any prose lift (cheap floor insurance).** Confirmed the lift touched no rule ID / GAV / flag / API sig / snippet `include` / tag-region (7 includes + 7 tags intact, `HE_EQUALS_NO_HASHCODE` still absent) before trusting FLOOR C still passes. Make "atom-diff after a prose lift" a standing lift-loop checklist line.

## Ch 4 (key 06, folds 90) — DEPTH lift-pass (the carried in-bounds move applied) + re-score (2026-06-28, independent)
- **A carried-forward in-bounds lift item that survives multiple passes is real, applies in one edit, and is worth exactly its named band — no more.** Ch 4's DEPTH bus-factor tension sat "available, un-applied" across passes 0–2. Applying it (convert the §"Knowledge is a quality asset" definition-plus-list into an argued tension) moved DEPTH 7→8 and aggregate 39→40, exactly as the prior card predicted. The lesson stands from Ch 8's mirror: tag each remaining point prose|verified-material|out-of-scope so the loop spends its ≤3 passes on the points it can actually move.
- **In-bounds DEPTH synthesis can strengthen the floors instead of risking them.** The lift added ZERO facts (pure synthesis: connect two halves the chapter already states — collective ownership raises the bus factor / drifts without the gates — into one argued tension, then resolve with the chapter's own recurring gate-as-enabler thesis). It *strengthened* NEUTRALITY (ownership models now held as explicitly costed trade-offs, none crowned) and HONEST-LIMITATIONS (every bus-factor raiser now names its cost; "None of these is free"). Promote the template: *connect two owned halves into a tension, resolve with the chapter's own thesis* — a no-new-facts DEPTH lift that is floor-positive.
- **Atom-diff confirmed the lift introduced no hard atoms** (no rule ID / config key / flag / API sig / GAV / version / benchmark; no numeric DORA/defect band; copyrighted-book atoms 6–8 untouched and still flagged). The "atom-diff after a prose lift" checklist line held for a non-code chapter too.
- **A pure Part-I culture/process chapter that by design defers its mechanics has a structural ceiling at ~40 (even all-8), not the 88% (44) auto-approval bar.** After the only in-bounds point was spent, Ch 4 is an even 8/8/8/8/8 with all floors passing — the best an in-bounds pass can produce — yet 4 short of 44, because three clusters (CLARITY/UTILITY/READABILITY) and the upper half of DEPTH are design-capped by routing every enforceable mechanic to a later chapter (Chs 33/34/35/37/38). The remaining +4 is unreachable in-bounds (scope creep to import deferred mechanics) or out-of-pass (figure re-render + `_ref/` copyrighted-edition diffs for ACCURACY 8→9, worth at most +1 → ~41). Pipeline suggestion: declare a human-gate carve-out for the small set of pure-culture/process chapters whose 88% is unreachable without scope creep, and flag the class early so the bounded loop does not grind passes chasing 8→9 lifts the design forbids. Ch 4 is the canonical instance: lift loop exhausted in-bounds at 40/50 → escalate to the human gate, do not lower the bar or pad.

## Ch 40 (key 96, folds 94) — OpenRewrite REPRO upgraded from PENDING-RUNTIME to RAN (2026-06-28)
- **"REPRO PENDING-RUNTIME (network-gated)" is a *resume-when-network-returns* state, not a permanent one — and lifting it is cheap when the network is back.** The Ch 40 OpenRewrite recipe RUN was honestly scoped PENDING because the original build was offline (the local `~/.m2` had zero `org/openrewrite` artifacts). With Maven Central reachable, the *unchanged* opt-in command `mvn -B -Prewrite -pl 96_… rewrite:dryRun` resolved the pinned engine + recipe module, validated the book's composite recipe, built the LST, ran it, and emitted a real patch — `BUILD SUCCESS`. No code/config change was needed; the network-gated profile pattern paid off exactly as designed. Pipeline suggestion: track PENDING-RUNTIME REPRO items as a worklist to drain whenever a networked runtime is available, rather than treating them as closed.
- **`dryRun` is the correct verification verb for a migration demo — it proves resolve+validate+execute WITHOUT mutating the tree.** The dryRun emitted `target/rewrite/rewrite.patch` (a build artifact) and left `git status` over `08-companion-code/` empty. A `run` would have half-applied a migration into a module that is deliberately held at its target idiom; `dryRun` gets the full proof (recipe tree printed, patch generated) with zero risk. Promote: for any "show the migration works" REPRO, use `dryRun` and capture the patch + recipe tree; reserve `run` for a throwaway copy only.
- **Execution corroborates IDENTITY that web-lookup alone left as a flag.** The recipe ID + recipe-module GAV were previously web-verified (docs.openrewrite.org + Maven Central) and flagged; the dryRun *executing* the recipe is stronger evidence than the lookup — the engine itself validated and ran `org.acme.remediation.ModernizeForJava21 → UpgradeToJava21`. When a networked runtime is available, prefer execution over lookup to close a recipe/GAV identity flag.
- **A type-aware migration over an already-modernized module correctly yields a near-empty, honest diff — not a no-op.** Because the before/after Java pair is authored at the Java-21 target idiom, `UpgradeToJava21` found nothing to change in the source and proposed only a stale-plugin bump (`spotbugs-maven-plugin` 4.9.3.0 → 4.9.8.5 via `UpgradePluginVersion`). That is the LST doing its job (it would have caught real legacy idioms had they been present) — a small genuine result, which reads more honestly than a fabricated large diff.

## Ch 24 (key 50) — ApprovalTests.Java REPRO upgraded from cited-not-built to BUILT GREEN (2026-06-28)
- **A "cited-not-built because unpinned" tool is a *verify-then-pin* state, not a permanent substitute — and the lift is cheap once Central is reachable.** Ch 24's approval test had been realized with a hand-rolled `SnapshotVerifier` because ApprovalTests.Java had no SOURCE-PIN row. With Maven Central reachable, the lift was a tight sequence: (1) read the latest `<release>` from `repo1.maven.org/.../approvaltests/maven-metadata.xml` (**31.0.0** — never guessed; the version MUST come from Central, not memory), (2) inspect the jar's bytecode major version (52 = Java 8 → safe on the JDK-21 anchor) and the POM's mandatory-vs-`<optional>` deps (only `approvaltests-util` is mandatory; a tiny footprint, no TestNG/XStream/Jackson dragged in), (3) `javap`-decompile the public API from the pinned jar so the test is written against verified signatures (`Approvals.verify(String, Options)`, `Options#withScrubber/#withReporter`, `RegExScrubber`, `QuietReporter`) — zero invented atoms, (4) prove it in a throwaway POC project before touching the book module, then wire it. Pipeline suggestion: keep a worklist of "cited-not-built (unpinned)" tools to drain whenever a networked runtime is available — same drain-the-backlog discipline as PENDING-RUNTIME REPRO items (Ch 40).
- **Two ApprovalTests-specific CI gotchas worth promoting to EXAMPLES-GUIDE.** (a) Use `QuietReporter.INSTANCE` as the reporter — the default reporter *launches a diff GUI* on mismatch, which hangs or fails a headless CI build; QuietReporter writes `*.received.txt` and fails the test instead. (b) Place the baseline as `<TestClass>.<method>.approved.txt` *next to the test source file* (the default `StackTraceNamer` resolves it there, not under `src/test/resources`), so the first run is green rather than the library's intended "fails-by-design" first run. Both are non-obvious from the prose and are exactly the kind of integration detail the EXAMPLE-BUILD gate exists to surface.
- **Generate a byte-exact approved baseline from the real renderer + scrubber, don't hand-type it.** ApprovalTests compares byte-for-byte (modulo line-ending normalization); a hand-typed baseline risks a one-byte trailing-newline mismatch → red build + a stray `*.received.txt`. Compiling the module's own `OrderReport.render` + applying the real scrub regex to write the baseline guaranteed the bytes match `verify()` on the first run (106 bytes, trailing newline confirmed). Promote: when seeding an approval/golden-master baseline, produce it from the actual production code path, never by eye.
- **Keep the hand-rolled mechanism stand-in alongside the real library when the chapter's honest-edge needs it.** The real `Approvals.verify` is the headline (and now backs the displayed `scrubber`/`approval-verify` tags, closer to the prose's `Approvals.verify(result)`), but the missing-baseline / changed-output / rubber-stamp edge tests stayed on `SnapshotVerifier` — those demonstrate the "verifies unchanged, not correct" spine without reaching into library internals or fighting the namer. A REPRO can *add* the real tool to the marquee path without ripping out a teaching scaffold that still earns its place. Net: ApprovalTests moves cited-not-built → built-green, the chapter can re-score for UTILITY, and Pact + REST-assured stay honestly flagged as runtime-gated (still in-JVM).

## Ch 4 (key 06) — independent re-score 40 → 41 after both ACCURACY residuals cleared (2026-06-28)
- **A figure re-render clears an ACCURACY residual only when the BAKED BODY ATOM changes — verify by reading the rendered PNG, never by mtime or an adjacent fix.** This artifact spent a whole scoring pass (pass 5) stuck at 40 because a re-render had freshened `fig06_1.png`'s mtime and fixed the figure *number* ("Figure 4.1") while the body footer still baked "Westrum, 1988". Pass 6 cleared it only after **opening the PNG and reading the footer directly** — now "Westrum, 2004". Promote: a "figure residual cleared" claim is proven by re-reading the *specific baked atom* in the rendered image; file freshness + a corrected neighbouring atom are not evidence.
- **`grep` the pin every pass — a prior "off-pin" finding goes stale the moment a `/pin-source` follow-up lands.** The prior scorecard correctly recorded Vogels + Deming as off-pin (`grep`=0) and held ACCURACY at 8; this pass re-grepped and found both now on SOURCE-PIN §7 (lines 145/142, web-verified). Confirming pin completeness by re-running the grep — not by trusting the previous scorecard's snapshot or the brief's claim — is what let ACCURACY move 8→9 honestly. Promote (reinforces the Ch 24 drain-the-backlog note): re-verify every "still open" residual against the live artifact each pass; scorecards record a moment, not a standing truth.
- **Empirically located: a pure Part-I culture/process closer tops out at ~41 (9/8/8/8/8) with EVERY artifact-completeness gap closed — the remaining 3 to 44 is structural design-deferral, not in-bounds-liftable.** Once both ACCURACY residuals cleared, ACCURACY hit a genuine 9 and the chapter stopped at 41: CLARITY/UTILITY/DEPTH/READABILITY are each design-capped at 8 because the chapter (correctly) routes every enforceable mechanic — CODEOWNERS→37, gate policy→33, clean-as-you-code/ratcheting→34/38 — to later chapters, so it cannot self-contain enough mechanism to reach 9 without scope creep. This is no longer a "finish the pin/figure work" question; it is the **declared-carve-out / human-gate-path** decision for the small class of pure-culture/process chapters whose 88% bar is unreachable without importing later chapters' content. Recommend a standing human-gate rule for this class so the lift loop does not re-spend budget chasing 8→9 lifts the chapter's design forbids.

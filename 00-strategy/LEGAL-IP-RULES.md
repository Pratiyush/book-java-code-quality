# LEGAL & IP RULES — Java code quality Book
...
Every rule IDs, config/ruleset keys, tool flags, API signatures, GAV coordinates, version numbers, benchmark figures, and quoted claims in the book **must** trace to the single pinned authority source:

- **the pinned authority set (00-strategy/SOURCE-PIN.md)** — pinned at **the pins in SOURCE-PIN.md** (the frozen tag+SHA / edition+year / corpus-snapshot date).
- **Source pin / commit:** `the pins in SOURCE-PIN.md`
- **Local (often ephemeral) clone/fetch root:** `the per-tool fetch dirs in SOURCE-PIN.md`
...
1. **Primary authority — the pinned authority set (00-strategy/SOURCE-PIN.md) (pinned at the pins in SOURCE-PIN.md)** — the primary authority.
...
Hard prohibitions:
- **Never** invent a source or any of the rule IDs, config/ruleset keys, tool flags, API signatures, GAV coordinates, version numbers, benchmark figures, and quoted claims.
- **Never** cite content farms, AI-generated text, anonymous pages, AI wiki-clones, or advocacy sites as a factual source.
- Any cross-subject / comparative claim needs a cited source from the pin and must respect `NEUTRALITY.md` (neutral comparative survey (see 00-strategy/NEUTRALITY.md): each option gets its strongest case and its hardest limitation; no crowning; banned: better than / unlike X / superior / beats / the problem with X).

---

## 4. Citation format

- **In-text:** name the source inline ("The Java Quality guide states…", "Per the spec…").
...
- Every entry uses this schema:
  ...
  Example:
  ```
  the pinned authority set (00-strategy/SOURCE-PIN.md) docs · the pins in SOURCE-PIN.md · "<Section title>" · <URL> · 2026-06-02
  ```

---

## 5. Code/content reuse & attribution (mixed OSS per tool (Apache-2.0 / LGPL / EPL / MIT / etc.); named books quoted under fair use - see 00-strategy/LEGAL-IP-RULES.md)

Java code quality material is licensed under **mixed OSS per tool (Apache-2.0 / LGPL / EPL / MIT / etc.); named books quoted under fair use - see 00-strategy/LEGAL-IP-RULES.md**. Reusing the subject's code or docs in the book must follow that license.

- Snippets/excerpts reproduced from the source are short, illustrative excerpts under fair use (§2) and remain covered by the source's license (mixed OSS per tool (Apache-2.0 / LGPL / EPL / MIT / etc.); named books quoted under fair use - see 00-strategy/LEGAL-IP-RULES.md).
- When an excerpt is taken substantially verbatim from a specific source file, **attribute it** in-text or in the chapter sources (name the guide/file and the pinned source — see the pins in SOURCE-PIN.md).
...
- **Companion-code licensing** *(technical profile — see BOOK-TYPE-PROFILES.md; book types without the build/compile gate turned off on the build drop this)* — the runnable modules under `08-companion-code/NN_slug/` are licensed **mixed OSS per tool (Apache-2.0 / LGPL / EPL / MIT / etc.); named books quoted under fair use - see 00-strategy/LEGAL-IP-RULES.md (code)**, with a **separate notice for the book prose**; the licensing terms and any public-repo ({URL}) publication are decided here and in `COMPANION-REPO.md`, gated on human/legal sign-off.
- **Attribution gate (companion code)** *(technical profile — see BOOK-TYPE-PROFILES.md; book types without the build/compile gate turned off on the build drop this)* — at Step 4b the **code-reviewer / example-builder must confirm every companion-code file under `08-companion-code/NN_slug/` is ORIGINAL-FOR-THIS-BOOK** — written for this book, not a lightly-edited copy of an upstream sample, guide quickstart, or `_ref/` listing. Any file or block taken substantially verbatim from a specific source must **name its guide/file and the pinned source** (§1) in the chapter sources; an unattributed verbatim copy is a gate FAIL.

---

## 6. Trademark handling (Java code quality / owner marks)

"Java code quality" and its logo, plus the owning organization's name and related marks, are trademarks of their respective owners. The book references the **technology/subject**, not the brand, and must not imply endorsement.

- Use marks descriptively and accurately (nominative use): refer to "Java code quality" the subject, not as the book's own product.
- **Do not** alter, restyle, or recreate the Java code quality or owner logos. Logos appear only when directly relevant and only in unmodified, licensed form (see §7).
- **Do not** imply sponsorship, endorsement, partnership, or affiliation with the owning organization or the Java code quality project.
- The same restraint applies to comparable subjects' marks: descriptive reference only, no logo use beyond a necessary direct comparison, never crowning a winner (per neutral comparative survey (see 00-strategy/NEUTRALITY.md): each option gets its strongest case and its hardest limitation; no crowning; banned: better than / unlike X / superior / beats / the problem with X).

---

## 7. Image licensing

Composition, palette, and layout rules for figures live in `templates/FIGURE-GUIDE.md`. This section covers **licensing only** (per per-chapter image budget; designed diagrams authored as HTML then rendered to PNG (never image-generated); tool/IDE screenshots captured).

- Every designed figure is an original diagram authored for this book and rendered to a final asset (no third-party diagram is traced or reused). Captured screenshots show only Java code quality-native UIs from the book's own running example — never a third-party tool's UI.
...
- No fabricated facts, sources, or rule IDs, config/ruleset keys, tool flags, API signatures, GAV coordinates, version numbers, benchmark figures, and quoted claims may be introduced by the model; everything traces to the pin (§1) and the source ranking (§3).

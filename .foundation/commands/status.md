...
2. **Source pin** — pin from `SOURCE-PIN.md`; whether `{{AUTHORITY_CLONE_PATH}}` is currently on-pin (a quick read-only verification check is allowed since it changes nothing).
3. **FINAL_INDEX** — confirmed or still PROPOSED (gates drafting), and the chapter count.
4. **Dossiers** — banked keys, which are verified @ `{{AUTHORITY_PIN}}`, which are still UNVERIFIED.
...
- Report the ACTUAL repo state: the current branch and short HEAD (a read-only branch/HEAD check is allowed), and that work is on a feature branch off the default branch with the configured remote `{{REPO_REMOTE}}`. Do not claim the repo is not under version control if it is.

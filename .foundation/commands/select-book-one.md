...
1. **Step 2 first — never score an unverified dossier.** A candidate is only scoreable once its source-trace audit (`/research` step 2: `verify_sources.sh` + `source-verifier`) has passed at the pin `{{AUTHORITY_PIN}}`. If a dossier is still UNVERIFIED (e.g. researched against an earlier or unpinned version of `{{AUTHORITY_SOURCE}}`), it cannot be scored for inclusion — flag and hold.

2. **Step 3 — inclusion score.** Run the `chapter-scorer` agent on each verified dossier against `SCORING.md`:
   - **Content-floors, checked first (PASS/FAIL):** {{FLOORS}} — FLOOR A NEUTRALITY ({{NEUTRALITY_STANCE}}); FLOOR B HONEST-LIMITATIONS (every feature has a when-NOT-to-use); FLOOR C SOURCE-TRACE (zero invented {{INVENT_UNITS}} — everything traces to `{{AUTHORITY_SOURCE}}` per `SOURCE-PIN.md` or is flagged to `09-flags/`) *(technical profile — see BOOK-TYPE-PROFILES.md; book types without {{GATES_OFF}} extend FLOOR C with COMPILE / CODE-REVIEW)*. A floor failure is fatal regardless of cluster scores.
   - **Five clusters, 1–10:** {{SCORING_CLUSTERS}} — with **DEPTH** as the central cull test (is there enough *verified* material for a full chapter, or is it a section/merge?).
   - **Ship/include bar:** {{SHIP_BAR}} — all floors PASS **and** the clusters meet the rubric threshold with no single cluster below its minimum.

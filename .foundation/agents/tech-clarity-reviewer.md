...
description: >-
  ...
  {{SUBJECT_SHORT}} expert: is the mechanism right and followable, does the
...
`SCORING.md` (the CLARITY and DEPTH clusters of {{SCORING_CLUSTERS}}, the HONEST-LIMITATIONS floor),
...
- **The subject's central distinction holds** — the spine of a {{SUBJECT_SHORT}} chapter. Use the
  **{{SUBJECT_SHORT}}-source-verify** skill to re-open the source where a causal claim is load-bearing.
...
- Stay in your lane: neutrality/voice/authenticity belong to the AUDIT gate, fact-tracing to
  VERIFY. Note overlaps but do not duplicate their verdicts. Respect the version pin ({{AUTHORITY_PIN}} only).

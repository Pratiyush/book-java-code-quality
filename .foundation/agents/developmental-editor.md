...
READABILITY clusters of {{SCORING_CLUSTERS}} — your ramp judgement leans on these), and `SOURCE-PIN.md`.
...
- **Never invent.** A concept name, prerequisite, or objective you record must trace to a real chapter
  ...
  `{{AUTHORITY_PIN}}` pin; never order toward an off-pin concept that does not exist at the pin.
...
> `CONCEPT-MAP.md` are largely `{{INVENT_UNITS}}`-adjacent mechanism concepts (e.g. a DI bean, a
...
> `{{AUTHORITY_CLONE_PATH}}` clone at `{{AUTHORITY_PIN}}`) must not anchor an ordering decision — flag it.

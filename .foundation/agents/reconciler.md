...
description: >-
  ...
  and its neighbors to catch contradictions in canonical facts ({{INVENT_UNITS}}),
...
- `LEDGER.md` — the continuity bible (canonical names, {{INVENT_UNITS}} atoms, the pin, hand-off chain).
...
- **Canonical-fact agreement** — every shared {{INVENT_UNITS}} atom matches what `LEDGER.md` and
...
- You report contradictions with a location + a fix; you do not pick a winner between two
  ...
  Anything untraceable goes to `09-flags/`. Respect the `{{AUTHORITY_PIN}}` pin; never reconcile

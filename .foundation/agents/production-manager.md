...
1. **Recompute the workflow state** — for every chapter, read its current stage off
   ...
   **current owner** (the agent/human who holds the next action — e.g. `draft`→drafter, step
   12→human; *(technical profile)* `example`→example-builder + code-reviewer when {{GATES_ON}}
...
5. **Maintain the risk register** — one row per open blocker (from `09-flags/`, a stale-verdict
   chapter, a tentpole at risk; *(technical profile)* a red build when {{GATES_ON}} includes
...
- **Respect the pin & the gate count.** `PIPELINE.md` is the authority on the step order and the
  HARD-gate enumeration; do not re-number steps or re-count gates. Hold the `{{NEUTRALITY_STANCE}}`
  in prose; never write a residue-blocklist term. Never reconcile toward an off-`{{AUTHORITY_PIN}}`
  fact.

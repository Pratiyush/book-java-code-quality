...
description: >-
  ...
  {{AUTHORITY_SOURCE}} ({{AUTHORITY_PIN}}) + reputable sources, and banks a dossier
...
- `00-strategy/SOURCE-PIN.md` — the frozen pin ({{AUTHORITY_PIN}}, clone/fetch `{{AUTHORITY_CLONE_PATH}}`).
- `00-strategy/NEUTRALITY.md` — {{NEUTRALITY_STANCE}}.
...
{{INVENT_UNITS}} atom and every example/snippet, read the actual source under
`{{AUTHORITY_CLONE_PATH}}` via the **{{SUBJECT_SHORT}}-source-verify** skill and record the
exact source path. The pinned source is **ephemeral**: before any verification read, confirm
it is on-pin ({{AUTHORITY_PIN}}); if it is absent or off-pin, run
...
1. **{{NEUTRALITY_STANCE}}.** {{BOOK_SUBJECT}} is the subject. Comparators enter only for a
   necessary direct comparison or migration topic, with a cited {{SUBJECT_SHORT}} source.
   Components shipped as part of {{BOOK_SUBJECT}} are {{BOOK_SUBJECT}}, not rivals.
2. **Never invent** {{INVENT_UNITS}}. Untraceable detail is marked `⚠ UNVERIFIED`
...
3. **Snippets ≤ 9 lines each** (technical profile — see BOOK-TYPE-PROFILES.md; book types without
   {{GATES_ON}} compile/example gates may relax the snippet ceiling), each verified against the
...
5. **Fill the Figure plan (§6).** Figures are load-bearing but no longer rare (GUIDELINES §8): set
   the chapter class and per-chapter image budget per {{FIGURE_POLICY}} — typically 1–2 designed
...
mechanism with the {{INVENT_UNITS}} table (atom · type · default · lifecycle · source), evidence
for/against, current status, the **worked-example spec** (§6) — including its **Catalog demo**
pointer to this `NN_slug`'s row in `DEMO-CATALOG.md` (name the demo, the {{SUBJECT_SHORT}} surface
...
log. Mark every Primary source row verified `@{{AUTHORITY_PIN}}`. Send anything untraceable to
`09-flags/`.

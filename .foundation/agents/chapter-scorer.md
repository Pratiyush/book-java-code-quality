...
when {{GATES_ON}} includes example-build/code-review — `_EXAMPLE.md`, `_CODEREVIEW.md`).

## What you do

1. **Check the floors first** (`{{FLOORS}}` — they gate, they are not averaged in):
   - **FLOOR A — NEUTRALITY:** {{NEUTRALITY_STANCE}}; no winner; no banned
   ...
   - **FLOOR C — SOURCE-TRACE / COMPILE / CODE-REVIEW:** zero invented {{INVENT_UNITS}} —
     everything traces to `{{AUTHORITY_SOURCE}}` at `{{AUTHORITY_PIN}}` per `SOURCE-PIN.md` or is
     flagged to `09-flags/`.
     *(technical profile — see BOOK-TYPE-PROFILES.md; book types without {{GATES_ON}} example-build
     drop the compile/code-review clauses and keep source-trace only):* AND the companion module
     builds green via `{{BUILD_CMD}}` clean at `{{AUTHORITY_PIN}}` AND the module passes the
...
2. **Score the five clusters 1–10** using the SCORING.md anchors, one-line justification each
   (`{{SCORING_CLUSTERS}}`): CLARITY, ACCURACY (traceable to `{{AUTHORITY_PIN}}`; spot-check with
   the **source-verify** skill if a fact is load-bearing), UTILITY, DEPTH (verified substance, NEVER
   word count), READABILITY.
3. **Apply the ship bar (`{{SHIP_BAR}}`):** ships only if all floors PASS **and** the five clusters
...
`{{FLOORS}}` floors + one bar. You do not invent facts.

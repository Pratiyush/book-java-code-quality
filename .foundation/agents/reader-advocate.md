...
already confirmed the mechanism is correct and followable **by a sharp {{SUBJECT_SHORT}} expert**; you
...
- Through the **book-law** skill, read whole: `GUIDELINES.md` (the law + the chapter promise and
  ...
  `{{SCORING_CLUSTERS}}` and the retired-mechanism ban), and `SOURCE-PIN.md` (the pin).
...
- *(technical profile)* The chapter's companion artifact under `{{CODE_ROOT_REL}}/NN_slug/` and any
  `_EXAMPLE.md` — so you can check that the **reproduce-it path** (the run command + expected output, per
  `{{EXAMPLE_POLICY}}`) is one the persona can actually follow without an unstated step.
...
2. **Walk the chapter top to bottom, once, in reading order**, narrating the reader's experience at each
   ...
   - **CANNOT REPRODUCE** — the persona follows the chapter's stated steps and **does not get the stated
     ...
     *(technical profile)* Use Bash/Glob/Grep against `{{CODE_ROOT_REL}}/NN_slug/` to confirm the run

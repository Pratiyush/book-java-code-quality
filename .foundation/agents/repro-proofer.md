...
description: >-
  ...
  copy-paste commands, runs {{BUILD_CMD}} and a smoke check of the chapter's claimed
  behavior across the runtime matrix ({{LANG_RUNTIME}}; plus the native/optimized
...
> example-build/compile under `{{GATES_ON}}` and FLOOR C carries a compile clause. Book types whose
> profile lists those under `{{GATES_OFF}}` (science, business, narrative, most reference) delete this
...
`{{BUILD_CMD}}`, and a smoke check confirming the behavior the chapter claims — repeated across the
...
- `00-strategy/COMPANION-REPO.md` and `00-strategy/templates/EXAMPLES-GUIDE.md` — the committed
  ...
  command and `{{BUILD_CMD}}`) the README is held to.
...
**{{SUBJECT_SHORT}}-source-verify** skill — never guess the supported runtime; `SOURCE-PIN.md` is the
one source of truth ({{LANG_RUNTIME}}).
...
3. **Build green from clean:** `{{BUILD_CMD}}` from the module directory in the scratch checkout. A red
...
5. **Run the environment matrix.** Repeat steps 3–4 across the supported runtime matrix
   ({{LANG_RUNTIME}} — the baseline + the recommended version), and across the native/optimized target
...
6. **Name the durable form: the CI env-matrix.** This gate's durable, machine-checkable form is a CI
   workflow under `.github/workflows/` that runs `{{BUILD_CMD}}` across the same runtime matrix (and the
...
paste the line. If the pinned clone `{{AUTHORITY_CLONE_PATH}}` is absent or off-pin, run
...
4. **Respect the pin; never invent.** The supported runtime/native toolchain and every fact you check
   trace to {{BOOK_SUBJECT}} {{AUTHORITY_PIN}} per `SOURCE-PIN.md`. Never reproduce against an
...
5. **No public push, no remote.** Cloning into local scratch is fine; no `gh`, no push, no public-repo
   action (`{{REPO_REMOTE}}`) — the public companion-repo push stays gated on human/legal sign-off
...
verbatim, in order; the **environment matrix** as a table ({{LANG_RUNTIME}} cells + native/optimized
where claimed) with the runtime version line and the `{{BUILD_CMD}}` result per cell; the smoke-check

...
description: >-
  ...
  conventions, the {{AUTHORITY_PIN}} note + how to use the examples), and the
...
callout taxonomy, the worked-example conventions, the {{AUTHORITY_PIN}} pin note and how to use the
...
visual rhythm), `VOICE-GUIDE.md` (the one locked {{VOICE}} — your prose is held to it), `SOURCE-PIN.md`
(the {{AUTHORITY_PIN}} pin and, *technical profile*, the {{LANG_RUNTIME}} baseline + SNAPSHOT policy),
and `LEGAL-IP-RULES.md` (copyright, {{SUBJECT_LICENSE}}, trademark, the AI-originality clause — the
...
- The worked-example conventions file (e.g. `COMPANION-REPO.md` / `EXAMPLES-GUIDE.md`) — the per-chapter
  example conventions ({{EXAMPLE_POLICY}}) and, where applicable, how the reader runs them.
...
1. **Preface.** Open with the book's premise and **promise** (from the charter, in your own words, not
   ...
   ({{NEUTRALITY_STANCE}}): nothing is crowned or disparaged here.
2. **"How to use this book" page.** Document the reader-facing conventions, each traced to its law file:
   - **Callout taxonomy** — list the named callouts with the one-line meaning each carries, exactly as
     ...
     learns to scan them.
   - **Worked-example conventions** ({{EXAMPLE_POLICY}}) — the per-chapter example unit and how the
   ...
   - **The {{AUTHORITY_PIN}} pin note + how to use the examples** — state the pin
     ({{AUTHORITY_SOURCE}} @ {{AUTHORITY_PIN}}) and how the reader engages the examples.
     **(technical profile)** also state the {{LANG_RUNTIME}} baseline and the exact run commands
     (e.g. {{BUILD_CMD}}) from the example-conventions file. Every version, atom, and command traces to
...
3. **Copyright / colophon.** Author the copyright notice, the license note(s) for the worked material
   ({{SUBJECT_LICENSE}} where applicable) and the separate notice for the book **prose**, the
...
- **Never invent.** No {{INVENT_UNITS}} atom, run command, ISBN, publisher, or audience claim that does
...
- **Respect the pin** ({{AUTHORITY_PIN}} only — never an off-pin source) and the neutrality law
  ({{NEUTRALITY_STANCE}}; its banned phrasings). State the neutrality stance positively in the promise.
- **Hold your own prose to the locked voice** (`VOICE-GUIDE.md` — {{VOICE}}). The front matter is read

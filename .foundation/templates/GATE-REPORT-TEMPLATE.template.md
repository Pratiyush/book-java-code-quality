# GATE REPORT TEMPLATE — {{BOOK_SUBJECT}} Book
...
>
> **Pin (HARD):** Any fact this gate checks — any {{INVENT_UNITS}}, snippet, or example — traces to the
> pinned {{BOOK_SUBJECT}} {{AUTHORITY_PIN}}, clone/fetch at `{{AUTHORITY_CLONE_PATH}}`. A claim that exists
...
- **Gate:** [VERIFY | AUDIT | CLARITY | READER-SIM | RECONCILE | EXAMPLE-BUILD | CODE-REVIEW | other]
  *(EXAMPLE-BUILD and CODE-REVIEW are technical profile — see BOOK-TYPE-PROFILES.md; book types with {{GATES_OFF}} drop those rows.)*
...
- [ ] **VERIFY** — every {{INVENT_UNITS}}, snippet, and example traces to an {{AUTHORITY_PIN}} source path; citations lint clean; no ahead-of-pin facts presented as fact.
- [ ] **AUDIT** — NEUTRALITY floor ({{NEUTRALITY_STANCE}}; no banned phrasings); voice matches `VOICE-GUIDE.md` ({{VOICE}}); authenticity (a sharp reader cannot tell a machine wrote it).
...
- [ ] **RECONCILE** — no contradiction with `LEDGER.md` continuity, the hand-off chain, or facts asserted in other chapters; every figure (per {{FIGURE_POLICY}}) is load-bearing and source-cited; designed diagrams are rendered from committed source text.
- [ ] **EXAMPLE** (Step 4b) *(technical profile — see BOOK-TYPE-PROFILES.md):* companion artifact builds green via `{{BUILD_CMD}}` at {{AUTHORITY_PIN}}; every displayed snippet resolves to a real bounded tag region (per {{EXAMPLE_POLICY}}) in the compiled file; FLOOR C source-trace clean.

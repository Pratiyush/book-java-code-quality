# AGENTS.md — orientation map for the {{BOOK_SUBJECT}} Book
...
3. **`00-strategy/GUIDELINES-{{SUBJECT_SHORT}}.md`** — the law (top of the hierarchy). Read in full before any quality-critical step.
...
Every {{INVENT_UNITS}}, and every example/quote/figure, **must trace to the pinned authority source** — never to a moving/unstable version (e.g. `main` / a draft / a newer edition), never to memory.
...
| Runtime baseline (technical profile — see BOOK-TYPE-PROFILES.md; book types without {{GATES_ON}} build/code-review drop this row) | {{LANG_RUNTIME}} |
...
| File | Governs |
|---|---|
| **`GUIDELINES-{{SUBJECT_SHORT}}.md`** | The law — the hard rules, the pin, voice/structure, the pipeline summary, figures & visual rhythm (§8) |
...
| `COMPANION-REPO.md` | (technical profile — see BOOK-TYPE-PROFILES.md; book types without {{GATES_ON}} build drop this) Runnable companion-module conventions + the public-push gate |
...
- ❌ Don't verify a fact against an unpinned/moving source (e.g. `main` / a draft / a newer edition) — only the pinned {{AUTHORITY_PIN}} copy.
- ❌ Don't invent a {{INVENT_UNITS}}. Untraceable → cut or flag to `09-flags/`.
- ❌ Don't violate the neutrality stance: {{NEUTRALITY_STANCE}} — banned phrasings are listed in `NEUTRALITY.md`.

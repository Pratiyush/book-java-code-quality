# CHAPTER TEMPLATE — {{BOOK_SUBJECT}} Book
...
- **Companion module:** `NN_slug/` (verified <date>) — the runnable example artifact under
  `08-companion-code/NN_slug/`, pinned to {{BOOK_SUBJECT}} {{AUTHORITY_PIN}} and built green by
  `{{BUILD_CMD}}`. Record the date the artifact last built clean; leave the field as `none` for a chapter
  ...
  *(technical profile — see BOOK-TYPE-PROFILES.md; book types with {{GATES_OFF}} for example-build replace
...
## 2. Epigraph (OPTIONAL)
One short line that sets the tone — a phrase from {{AUTHORITY_SOURCE}}, a representative figure, or a
...
statement: what this chapter does NOT cover. Include a one-line, {{NEUTRALITY_STANCE}}-respecting
...
This section carries the full worked path: each snippet/example beat is bounded per `{{EXAMPLE_POLICY}}`
...
example's build/run (the `{{BUILD_CMD}}` run and its run command); it is optional only for the rare
...
*(technical profile — see BOOK-TYPE-PROFILES.md; book types with {{GATES_OFF}} for example-build keep the
do-and-verify beat as a "trace it back to the source" beat that points the reader to {{AUTHORITY_SOURCE}}.)*
...
artifact (see `templates/EXAMPLES-GUIDE.md`). *Book types with {{GATES_OFF}} for example-build drop this
...
- **Module:** `08-companion-code/NN_slug/` — single self-contained example artifact keyed by the frozen
  `NN_slug`, pinned to {{BOOK_SUBJECT}} {{AUTHORITY_PIN}} via the inherited pin property.
- **Dependencies ({{INVENT_UNITS}}):** the exact coordinates the artifact declares (version inherited
...
{{AUTHORITY_SOURCE}} (content-floor: NEUTRALITY, per {{NEUTRALITY_STANCE}}).
...
- **Reference:** important details, with exact names (if applicable — e.g. {{INVENT_UNITS}}).
- **Sources and further reading:** two tiers, {{AUTHORITY_SOURCE}}-anchored, plain-text references (see below).
...
- **Snippets/examples are bounded per {{EXAMPLE_POLICY}}**, and every one is verified against
  {{AUTHORITY_SOURCE}} at the pin {{AUTHORITY_PIN}}. Record the source path for each.
- **Never invent** {{INVENT_UNITS}}. Untraceable detail is cut, or marked `UNVERIFIED` and flagged to
  `09-flags/`.
- **{{NEUTRALITY_STANCE}}.** {{BOOK_SUBJECT}} is the subject. No rival is crowned superior. The banned
  phrasings of {{NEUTRALITY_STANCE}} are absent (e.g. "better than", "unlike X", "the problem with X",
  "superior", "beats").
- **Figures per {{FIGURE_POLICY}}** (see GUIDELINES §8). A figure appears only where one carries a
  load-bearing spatial/flow/architecture mental model; the per-chapter image budget and the zero-figure
  exception are set by {{FIGURE_POLICY}}. **Designed diagrams are authored as source text (HTML) and
...
- **Captioned, numbered listings.** A displayed snippet is referenced by name in prose **before** it
  ...
  parts. The {{EXAMPLE_POLICY}} bound holds for every listing. **The one capstone example is the SINGLE
...
- **One core concept per chapter (HARD structure gate).** Each chapter revolves around a single core
  ...
  **figure plan** — which designed diagrams and which captured surfaces the chapter carries, per
  {{FIGURE_POLICY}}. A draft that violates this gate does not pass.
- **No comparative or rival-named section TITLE (TOC-level neutrality).** A section heading may not carry
  ...
  "Unequaled <X>", "Fastest <X>", "Alternative <rivals>", "<Rival> vs {{BOOK_SUBJECT}}", per-rival
...
- **Every fact traces to {{AUTHORITY_SOURCE}} at the pin.** Citation source ranking (resolve per profile):
...
- **Reach beyond the primary source.** Where one genuinely exists, the two-tier Sources section includes
  at least one authoritative source *outside* {{AUTHORITY_SOURCE}} — the relevant standard, spec, or
  reputable secondary work — each a real, verifiable reference (never invented; verify it resolves before
  citing). This places {{BOOK_SUBJECT}} in the wider landscape without crowning or disparaging any rival.
...
> **Visual rhythm alongside the figure plan.** A chapter carries an image budget per {{FIGURE_POLICY}}
...
## Sources tier ({{AUTHORITY_SOURCE}}-anchored back-matter format)
...
- **Primary doc reference:** the canonical doc/page for the fact (@ {{AUTHORITY_PIN}})
- **Reference entry:** the reference entry or config section, with the exact name ({{INVENT_UNITS}})
- **Source path:** the verifying file in the pinned clone, e.g.
  `{{AUTHORITY_CLONE_PATH}}/...` (or the canonical source location)
...
- [ ] "Verified against {{BOOK_SUBJECT}} {{AUTHORITY_PIN}}" field filled with today's re-check date
- [ ] "Companion module" field filled with the `NN_slug/` path and the date it last built green
      (or `none`); a declared example builds clean under `{{BUILD_CMD}}`
      *(technical profile; book types with {{GATES_OFF}} verify the worked scenario for internal consistency instead)*
...
- [ ] Overview leads with a 3–5 bullet "What this chapter covers" list, then the boundary statement,
      with a {{NEUTRALITY_STANCE}}-respecting prior-knowledge bridge
- [ ] One core concept; ≤ 3 heading levels; chapter promise + planned example (and companion artifact) +
      ...
      {{FIGURE_POLICY}}), all fixed before drafting (HARD structure gate)
...
- [ ] "Verify it works" beat present for any chapter with a companion example, tracing to the example's
      `{{BUILD_CMD}}` and run command; where the topic touches operational behavior it asserts the
...
- [ ] Every snippet/example bounded per {{EXAMPLE_POLICY}} and traced to a {{AUTHORITY_PIN}} source path
- [ ] No invented {{INVENT_UNITS}}
...
- [ ] No rival crowned superior; cross-subject mentions cited (NEUTRALITY floor, per {{NEUTRALITY_STANCE}})
- [ ] Figure plan met (per {{FIGURE_POLICY}}, GUIDELINES §8); each designed diagram is authored as source
...
- [ ] Back-matter sources follow the two-tier {{AUTHORITY_SOURCE}}-anchored format

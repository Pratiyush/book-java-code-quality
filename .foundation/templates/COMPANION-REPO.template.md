# COMPANION CODE REPOSITORY — SPEC — {{BOOK_SUBJECT}}

> **(technical/code profile — see BOOK-TYPE-PROFILES.md; book types without runnable examples drop this spec along with the EXAMPLES-GUIDE and the DEMO-CATALOG. Active only when `{{GATES_ON}}` includes example-build + code-review.)**
...
> in-repo `08-companion-code/NN_slug/` tree, exercised by `{{BUILD_CMD}}`, and kept in lock-step
> with the prose via the `NN_slug` key and the tag-includes (§2.5). What remains gated is the
> **public-facing action**: creating and pushing a public repository at `{{REPO_REMOTE}}` touches the project's IP,
...
>
> Until that sign-off, no `git`-remote, no `gh`, no public repo, no remote, no push to `{{REPO_REMOTE}}` is implied or
...
the book's law: `{{NEUTRALITY_STANCE}}` (`NEUTRALITY.md`), never invent a source
(`LEGAL-IP-RULES.md` §3, `SOURCE-PIN.md`), and the locked `{{VOICE}}` (`VOICE-GUIDE.md`). Where
...
├── .github/workflows/           # CI: {{BUILD_CMD}} across the NN_slug matrix (§2.6)
...
- **{{BOOK_SUBJECT}} version is pinned in exactly one place.** The parent/aggregator build file sets
  the platform pin to `{{AUTHORITY_PIN}}`; every `NN_slug/` module inherits it and carries **no** version literal
...
- **The exact property value and its placement must be confirmed against a real generated build file in the
  pinned clone** ({{AUTHORITY_CLONE_PATH}}) before anything is committed — never assumed. Same
...
- **The build wrapper is committed** (wrapper script(s) + bootstrap dir / lockfile) so the build is reproducible
  ...
  `{{BUILD_CMD}}` (verify both against the pin's tooling guide before stating either in the
  README).
...
- A workflow runs `{{BUILD_CMD}}` across a **matrix of the `NN_slug` chapter modules**, on the
  `{{LANG_RUNTIME}}` baseline recorded in `SOURCE-PIN.md`, pinned to {{BOOK_SUBJECT}} {{AUTHORITY_PIN}}.
...
- **Prerequisites**: the **`{{LANG_RUNTIME}}` baseline from `SOURCE-PIN.md`**, **{{BOOK_SUBJECT}} {{AUTHORITY_PIN}}**,
...
- **Edition tagging at manuscript freeze**: cut a release tag (e.g. the {{AUTHORITY_PIN}} pin or
...
In running prose, refer to a listing by **what it does** (the `{{VOICE}}` rule). Where a
...
- **CODE → propose a permissive OSS license that matches {{BOOK_SUBJECT}}'s own license** (`LEGAL-IP-RULES.md` §5),
...
| Governed by the **book's copyright** | Internal, tracked, built by `{{BUILD_CMD}}` | Code under the proposed OSS license (§3) |
...
by `{{BUILD_CMD}}`, and are **only PUBLISHED to a public repo after the §5 human/legal sign-off**.
...
pinned to {{BOOK_SUBJECT}} {{AUTHORITY_PIN}}, that Step 5 (VERIFY) then checks the prose against. The public repo
...
- **{{NEUTRALITY_STANCE}}** — no crowning of {{BOOK_SUBJECT}} over an alternative anywhere in the README, comments, or commit messages
  (`NEUTRALITY.md`).
- **Never invent a source** — no `{{INVENT_UNITS}}` atom that has not been verified against the
  pin (`LEGAL-IP-RULES.md` §3, `SOURCE-PIN.md`). The repo is held to the pinned {{BOOK_SUBJECT}} {{AUTHORITY_PIN}} the
...
- [ ] **Approve or deny PUBLIC repo creation/push at all.** This is the outward-facing gate; no
      public repo, remote, or push to `{{REPO_REMOTE}}` proceeds without it.
- [ ] **Repo name** — a `<book-slug>-code` / `<book-slug>-examples` candidate (§2.1). Depends on the
      locked book slug.
- [ ] **`{{LANG_RUNTIME}}` baseline** — recorded in `SOURCE-PIN.md`, verified against {{AUTHORITY_CLONE_PATH}}.
...
- The `{{LANG_RUNTIME}}` baseline is a recurring "do not guess" trap — record the verified value in `SOURCE-PIN.md`

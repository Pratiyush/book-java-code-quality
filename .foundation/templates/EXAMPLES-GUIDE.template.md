# EXAMPLES GUIDE — {{BOOK_SUBJECT}}

> **(technical/code profile — see BOOK-TYPE-PROFILES.md; book types without runnable examples drop this entire guide, the COMPANION-REPO spec, and the DEMO-CATALOG. Active only when `{{GATES_ON}}` includes example-build + code-review and FLOOR C carries the COMPILE clause.)**

> The standard for the **Step-4b companion module** (the 9th HARD gate). Every selected chapter ships **one** self-contained, runnable, enterprise-grade `{{LANG_RUNTIME}}` module keyed by its frozen `NN_slug`, pinned to {{BOOK_SUBJECT}} **{{AUTHORITY_PIN}}**, built and tested by `{{BUILD_CMD}}`. The book's displayed bounded snippet is a tag-include region *inside* that compiled file, so the printed listing and the runnable code are **one artifact**.
>
> This guide inherits the book's law and is subordinate to it. Where it would collide, the law wins: `{{NEUTRALITY_STANCE}}` (`NEUTRALITY.md`), never invent a source (`LEGAL-IP-RULES.md` §3, `SOURCE-PIN.md`), the locked `{{VOICE}}` (`VOICE-GUIDE.md`). It is referenced by `COMPANION-REPO.md`, `SOURCE-PIN.md`, and `SCORING.md` (FLOOR C), and by the `/example` pipeline step.

## Where Step-4b sits in the pipeline

Step 4 drafts the chapter. **Step 4b builds the companion module.** Step 5 (VERIFY) then checks the prose against the compiled code, not against an unverified fragment. A module that does not build green at {{AUTHORITY_PIN}} fails FLOOR C, and FLOOR C is fatal regardless of the aggregate score (`SCORING.md`).

```
Step 4   DRAFT          prose, with displayed snippets marked for tag-include
   |
Step 4b  EXAMPLE-BUILD  one runnable enterprise-grade NN_slug module; {{BUILD_CMD}} green
   |
...
- The **public** repository push stays gated on the human/legal sign-off in `COMPANION-REPO.md` §5. Nothing in this guide authorizes a `git`-remote, a `gh` action, or a push to `{{REPO_REMOTE}}`.
...
|---|---|---|
| 1 | **Pinned platform** | {{BOOK_SUBJECT}} is pinned through **one inherited property/manifest entry**, set once in the parent build file; the child module carries no version literal of its own. |
...
| 3 | **At least one integration test** | The module carries `>=1` test (the subject's in-process test-harness annotation) that exercises the chapter's mechanism and runs under `{{BUILD_CMD}}`. |
...
| Property | Value | Why |
|---|---|---|
| platform pin | `{{AUTHORITY_PIN}}` | The single knob that binds the platform BOM, the build plugin, and every imported extension to the pinned tag (`SOURCE-PIN.md`). |
...
- The exact property value and its placement are **confirmed against a real generated build file in the pinned clone** ({{AUTHORITY_CLONE_PATH}}) before anything is committed — never assumed. The same never-invent law that governs the prose governs the build file.
...
- The documented entry points are the dev/run command and `{{BUILD_CMD}}` (the build contract, §6). Confirm both phrasings against the pin's tooling guide before stating them in any README.
...
## 6. The `{{BUILD_CMD}}` contract

`{{BUILD_CMD}}` is the build contract for every companion module. It is the gate's verdict of record at Step 4b.
...
- **CI:** a CI matrix runs `{{BUILD_CMD}}` across the `NN_slug` modules on the verified `{{LANG_RUNTIME}}` baseline. A red build blocks the chapter. CI is treated as an effective HARD gate for the companion code (`COMPANION-REPO.md` §2.6).

```
edit module / snippet  ->  {{BUILD_CMD}}  ->  green  ->  CODE-REVIEW  ->  Step 5 VERIFY (prose vs. compiled code)
...
A green `{{BUILD_CMD}}` proves the module compiles and its tests pass. For a technical book whose
...
PR, across six dimensions — **correctness, idiomatic {{BOOK_SUBJECT}} & {{LANG_RUNTIME}}, security, simplicity/readability,
...
## 7. `{{LANG_RUNTIME}}` baseline

The module targets the pinned `{{LANG_RUNTIME}}` baseline (`SOURCE-PIN.md`), not a guessed value:

| Fact | Value |
|---|---|
| Minimum runtime | **{{LANG_RUNTIME}}** (read from the pinned tree; cite the prerequisite line in the source) |
...
### 8.1 NEUTRALITY (`{{NEUTRALITY_STANCE}}`) — `NEUTRALITY.md`, `SCORING.md` FLOOR A
...
- {{BOOK_SUBJECT}} is the subject. An alternative appears in code **only** for a necessary direct feature comparison or a migration topic, and even then neutrally and with a cited {{AUTHORITY_SOURCE}} reference — never to position any alternative as the winner.
...
- Every `{{INVENT_UNITS}}` atom in the module (config key, dependency coordinate, version number, API signature, CLI flag, annotation) traces to the pinned **{{BOOK_SUBJECT}} {{AUTHORITY_PIN}}** clone. None is invented, and none drifts to an unpinned newer release.
...
- Any file or block taken substantially verbatim from a specific {{AUTHORITY_SOURCE}} listing must **name its guide/file and the pinned {{AUTHORITY_PIN}}** in the chapter sources; an unattributed verbatim copy is a gate **FAIL** (`LEGAL-IP-RULES.md` §5).
...
- **PASS** — Zero invented `{{INVENT_UNITS}}` atoms **AND** the chapter's companion module compiles green via `{{BUILD_CMD}}` at {{BOOK_SUBJECT}} {{AUTHORITY_PIN}}.
...
- [ ] Parent build file sets the platform pin to `{{AUTHORITY_PIN}}`; child module has no version literal (§3).
...
- [ ] Build wrapper committed; `{{BUILD_CMD}}` green locally (§4, §6).
- [ ] `{{LANG_RUNTIME}}` baseline cites `SOURCE-PIN.md`, not a guess (§7).
- [ ] NEUTRALITY and never-invent hold across comments, identifiers, and log/error strings (§8).
- [ ] Every companion-code file confirmed ORIGINAL-FOR-THIS-BOOK; any substantially-verbatim block names its guide/file + pinned {{AUTHORITY_PIN}} (§8.3, `LEGAL-IP-RULES.md` §5).

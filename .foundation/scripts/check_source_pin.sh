...
DEFAULT_TAG="{{TAG}}"                  # fallback if SOURCE-PIN.md parse is empty
DEFAULT_SHA="{{SHA}}"                  # fallback if SOURCE-PIN.md parse is empty
DEFAULT_CLONE_PATH="{{CLONE_PATH}}"    # fallback if SOURCE-PIN.md parse is empty
PIN_FILE_REL="{{PIN_FILE_REL}}"        # e.g. 00-strategy/SOURCE-PIN.md (path under repo root)

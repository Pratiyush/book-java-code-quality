...
PIPELINE_REL="{{PIPELINE_REL}}"      # e.g. 00-strategy/PIPELINE.md
TRACKER_REL="{{TRACKER_REL}}"        # e.g. 01-index/CHAPTER-TRACKER.md
AGENTS_DIR_REL="{{AGENTS_DIR_REL}}"  # e.g. .claude/agents
...
HUMAN_OR_SCRIPT_GATES="{{HUMAN_OR_SCRIPT_GATES}}"   # e.g. "0 4a 12 16"
...
# ({{GATES_OFF}}) drops 4b/4d/9 from this list. Keep this and PIPELINE.md in lockstep.
HARD_GATES="{{HARD_GATES}}"   # e.g. "0 2 3 3b 4a 4b 4d 5 5b 6 6b 7 8 8a 8b 9 10 12 15 16"
# Agents that legitimately own NO pipeline step (pure support roles). Space- or
# newline-delimited basenames (no ".md"). Usually empty.
AGENT_STEP_EXEMPT="{{AGENT_STEP_EXEMPT}}"           # e.g. ""
# Per-chapter HARD gate COLUMNS in pipeline order — the blocking trail this guard
# enforces row by row. A non-code book drops example/figure (its {{GATES_OFF}});
# keep only the columns that exist on THIS book's board, left-to-right.
HARD_COLS_CFG="{{HARD_COLS}}"        # e.g. "verify example clarity audit score figure reconcile approve"
...
COL_STEP_PAIRS="{{COL_STEP_PAIRS}}"  # e.g. "research:1 verify:5 draft:4 example:4b clarity:6 audit:7 score:8 figure:9 reconcile:10 approve:12"
...
RIDEALONG_TOKENS="{{RIDEALONG_TOKENS}}"  # e.g. "4d:repro 5b:originality|verbatim 6b:copyedit 8a:reader-?sim 8b:red-?team|adversarial"
# =====================================================================

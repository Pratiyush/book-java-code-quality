...
Companion module: 08-companion-code/07_naming_structure_formatting/ — EXAMPLE-BUILD = BUILT GREEN (mvn -Pquality verify: 6 tests pass, 0 Checkstyle errors, 0 SpotBugs bugs, 2026-06-27). Spec at foot.
Verified against SOURCE-PIN: 2026-06-27. Sources: Google Java Style Guide (§3 structure, §4 formatting, §5 naming); Effective Java 3e (2018) Item 56, Item 68; JLS SE 21 §6.1 (Declarations; web-confirmed 2026-06-28); Checkstyle 13.6.0; PMD 7.25.0; SonarQube 2026.1 LTA (java:S100/S101/S115/S116/S117, java:S125); Spotless spotless-maven-plugin 3.6.0; google-java-format 1.35.0; palantir-java-format; EditorConfig spec; JDK 21 doc-comment spec; JEP 413 ({@snippet}, JDK 18); JEP 467 (/// Markdown comments, JDK 23 — past the Java 21 anchor); Clean Code (2008); A Philosophy of Software Design (2018).
NEUTRALITY note: the comments debate (Clean Code vs APoSD) is presented two-schools, no crown.
Resolved 2026-06-28: Checkstyle/PMD/Sonar naming-rule default regexes + Checkstyle LineLength default (verified against each tool's source); JLS SE 21 §6.1 wording (web-confirmed); JEP numbers (per 09-flags/07); named-book quotes converted to attributed paraphrases (no verbatim presented). Pending pin-registry rows only: EditorConfig spec, A Philosophy of Software Design (canon row).
...
**Companion module (BUILT GREEN — `mvn -Pquality verify`: 6 tests pass, 0 Checkstyle errors, 0 SpotBugs bugs, 2026-06-27):** `08-companion-code/07_naming_structure_formatting/` — a small `storefront` domain class (an order line) carried through the readability pass, with the Checkstyle naming gate (`TypeName`/`MethodName`/`ConstantName`/`MemberName`/`ParameterName`/`LocalVariableName`) wired live in the `quality` profile, a repo-root `.editorconfig`, and a JUnit 5 (Jupiter) test proving the rename/reformat is semantics-preserving. The deterministic-formatter half (Spotless driving google-java-format with `<ratchetFrom>` and aux steps) is carried as a reference configuration (`config/spotless/spotless-reference.xml`) rather than wired into the live build. **TRY-IT:** swap `<googleJavaFormat>` → `<palantirJavaFormat>` in that reference config to compare the 100-col vs 120-col wrapping. Snippet tags: `naming-good`, `naming-bad`, `checkstyle-naming`, `spotless-config`, `editorconfig-baseline`.

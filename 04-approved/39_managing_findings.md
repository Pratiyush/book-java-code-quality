...
- SpotBugs: @SuppressFBWarnings(value, justification) from edu.umd.cs.findbugs.annotations (GAV spotbugs-annotations; CLASS retention because bytecode) — CONFIRMED load-bearing in 08-companion-code/39_managing_findings/PriceFormatter.java (green build; removing it raises EI_EXPOSE_REP on denominationsCents()). FindBugsFilter root + Match (children = conjuncts) / And / Or / Not; leaves Bug(pattern), Class, Method, Field, Local, Confidence(1-3), Rank(1-20), Source — CONFIRMED present in the pinned engine jar (Bug/Class/Method/Field/Local/Confidence/Rank/Source/And/Or/Not matchers). -exclude/-include (Maven excludeFilterFile/includeFilterFile) — CONFIRMED accepted by the built module. excludeBugsFile / excludeBugsFiles (the SpotBugs "baseline files": "Bugs found in the baseline files won't be reported"; the plural List form is Since: 4.7.1.0 — spotbugs-maven-plugin 4.10.2.0 gapidocs; not exercised by this module, which uses excludeFilterFile); the Bug element also takes code (a comma-separated list of bug abbreviations) and category (a comma-separated list of bug category names: CORRECTNESS, MT_CORRECTNESS, BAD_PRACTICE, PERFORMANCE, STYLE) alongside pattern, and a Package match clause takes name (the package name; nested packages not included) — CONFIRMED verbatim in the pinned SpotBugs filter docs (spotbugs.readthedocs.io/en/stable/filter.html, 4.10.2), though not asserted in the built filter.
...
- SonarQube: //NOSONAR (EOL, suppresses ALL issues on the line — "now and in the future" — rule-blind; docs.sonarsource.com managing-issues); issue transitions False Positive / Accepted (the "Accepted" status replaced the older "Won't Fix" label in Server 10.4 — www.sonarsource.com/.../whats-new/sonarqube-10-4; as of Server 2026.1 LTA both are excluded: "SonarQube Server ignores accepted issues in the quality reports and ratings of the code", likewise false positives); New Code Definition + Clean as You Code gate (conditions on NEW code only) = the ratchet.
RESOLVED 2026-06-28 (web-verify against the pinned tool docs — these two carried no artifact in the built module): the SpotBugs Bug element's code/category attribute forms (alongside pattern) + the Package filter element's name attribute — confirmed verbatim in the pinned SpotBugs filter docs (spotbugs.readthedocs.io/en/stable/filter.html, 4.10.2): code = "a comma-separated list of bug abbreviations", category = "a comma separated list of bug category names" (CORRECTNESS/MT_CORRECTNESS/BAD_PRACTICE/PERFORMANCE/STYLE), Package name = "specify the package name. Nested packages are not included"; Checkstyle @SuppressWarnings name-normalization — confirmed verbatim against checkstyle.org/filters/suppresswarningsfilter.html (page Version 13.6.0, matching the pin): "Name of check in annotation is case-insensitive and should be written with any dotted prefix or 'Check' suffix removed" (SuppressWarningsFilter + required SuppressWarningsHolder on TreeWalker). RESOLVED 2026-06-28 (web-verify against the pinned plugin docs): maven-checkstyle-plugin 3.6.0 failOnViolation Default true and violationSeverity Default error (check-mojo.html); PMD 7.x CLI marker --suppress-marker (legacy -suppressmarker) per pmd.github.io suppressing_warnings; the SpotBugs Maven "baseline files" parameter is excludeBugsFile (String) / excludeBugsFiles (List, Since: 4.7.1.0) — "Bugs found in the baseline files won't be reported" — per spotbugs-maven-plugin 4.10.2.0 gapidocs (there is no parameter literally named baselineFiles). RESOLVED 2026-06-28 (web-verify, docs.sonarsource.com — SaaS/rolling, dated-at-use): the "Accepted" issue status replaced the older "Won't Fix" label in SonarQube Server 10.4 (whats-new/sonarqube-10-4); as of Server 2026.1 LTA the docs use only Accepted/False positive and state "SonarQube Server ignores accepted issues in the quality reports and ratings of the code" (likewise false positives); //NOSONAR is line-level and rule-blind ("suppress all issues on the line ... now and in the future") — no scoped //NOSONAR <ruleKey> form is documented. RESOLVED at FLOOR-C build (08-companion-code/39_managing_findings/, green): EI_EXPOSE_REP/EI_EXPOSE_REP2 patterns verbatim in the pinned engine findbugs.xml (category MALICIOUS_CODE); the FindBugsFilter Match/Bug/Class/Method/Field/Local/Confidence/Rank/Source leaf set; @SuppressFBWarnings(value, justification) + GAV spotbugs-annotations load-bearing; Checkstyle SuppressWarningsFilter + SuppressWarningsHolder + SuppressionFilter wired and green.
...
- **Sonar**: end-of-line `//NOSONAR`. It is **rule-blind**: it drops *every* issue on that line, now and in the future. The reviewed server-side path is the **False Positive** or **Accepted** issue transition; SonarQube Server "ignores accepted issues in the quality reports and ratings of the code" and does the same for false positives (as of Server 2026.1 LTA; the **Accepted** status replaced the older **Won't Fix** label in Server 10.4).
...
**Lever 3 — baseline.** A *baseline* is a recorded snapshot of the findings already present, the tool agrees to ignore from now on. Freeze the findings that exist *today* so the gate reacts only to *change*. That is the precondition for turning a gate on over a noisy codebase at all. SpotBugs has a true baseline: the spotbugs-maven-plugin `excludeBugsFile` (and the multi-file `excludeBugsFiles`, since 4.7.1.0) takes the SpotBugs *baseline files* — "Bugs found in the baseline files won't be reported." Checkstyle's external `suppressions.xml` (one `<suppress>` row per frozen finding) is the de-facto baseline, and `maxAllowedViolations` caps by count. Sonar's baseline is implicit: anything outside the **New Code** window is tracked but excluded from the default gate.
...
Turn the gate on without a baseline and the build is red on day one, which is the failure mode from the hook. **Baseline the past**: a SpotBugs `FindBugsFilter` (or the plugin's `excludeBugsFile`/`excludeBugsFiles` baseline) freezes the legacy class's findings, and Checkstyle's `maxAllowedViolations` is set to today's count. The build goes green, not because the debt is gone, but because the gate now reacts only to change.
...
- **Checkstyle** (`checkstyle.org` filter pages; `maven.apache.org/plugins/maven-checkstyle-plugin` `check-mojo.html`, `maven-checkstyle-plugin` 3.6.0): `SuppressWarningsFilter` (+ required `SuppressWarningsHolder`), `SuppressionCommentFilter` (`// CHECKSTYLE:OFF`/`:ON`), `SuppressWithNearbyCommentFilter`, `SuppressionXpathSingleFilter`, `SuppressionFilter` (`suppressions.xml`); `@SuppressWarnings("checkname")` (case-insensitive, dotted-prefix/`Check`-suffix removed); `checkstyle:check`, `failOnViolation` (Default `true`), `maxAllowedViolations`, `violationSeverity` (Default `error`).
- **PMD** (`pmd.github.io/.../pmd_userdocs_suppressing_warnings.html`, PMD 7.25.0): `@SuppressWarnings("PMD"/"PMD.Rule"/"unused")`; `// NOPMD` (same line; trailing text → report); the CLI marker option `--suppress-marker` (the docs also reference the legacy `-suppressmarker` spelling; default marker `NOPMD`); `violationSuppressRegex` / `violationSuppressXPath`; ruleset `<exclude>`/`<exclude-pattern>`.
- **SpotBugs** (`spotbugs.readthedocs.io/en/stable/filter.html`, 4.10.2): `@SuppressFBWarnings(value, justification)` from `edu.umd.cs.findbugs.annotations` (GAV `spotbugs-annotations`, class retention) — verified load-bearing in the companion module at FLOOR-C build; `FindBugsFilter` root + `Match`(conjunct children)/`And`/`Or`/`Not`; leaves `Bug`(pattern), `Class`, `Method`, `Field`, `Local`, `Confidence`(1–3), `Rank`(1–20), `Source` — confirmed present in the pinned engine jar; `-exclude`/`-include` (Maven `excludeFilterFile`/`includeFilterFile`) — confirmed accepted by the built module. The `Bug` element also takes `code` ("a comma-separated list of bug abbreviations") and `category` ("a comma separated list of bug category names": `CORRECTNESS`/`MT_CORRECTNESS`/`BAD_PRACTICE`/`PERFORMANCE`/`STYLE`) alongside `pattern`, and a `Package` match clause takes `name` ("the package name. Nested packages are not included") — **confirmed verbatim** in the pinned SpotBugs filter docs (not asserted in the built filter). The plugin's baseline parameter is `excludeBugsFile` (and the multi-file `excludeBugsFiles`, **Since: 4.7.1.0**) — "Bugs found in the baseline files won't be reported" (`spotbugs-maven-plugin` 4.10.2.0 gapidocs; there is no parameter literally named `baselineFiles`); not exercised by this module, which uses `excludeFilterFile`.
...
- **SonarQube** (`docs.sonarsource.com` managing-issues, as of Server 2026.1 LTA; `www.sonarsource.com/.../whats-new/sonarqube-10-4`): `//NOSONAR` (EOL, rule-blind — suppresses all issues on the line, "now and in the future"; no scoped `//NOSONAR <ruleKey>` form documented); issue transitions **False Positive** / **Accepted** — the **Accepted** status replaced the older **Won't Fix** label in Server 10.4, and at the pin SonarQube Server "ignores accepted issues in the quality reports and ratings of the code" (likewise false positives); New Code Definition + Clean-as-You-Code gate (conditions on new code only) = the ratchet.
- **Status/discipline**: FindBugs is dead → **SpotBugs** (annotation package `edu.umd.cs.findbugs.annotations`); `@Generated`-style precision applies to suppression annotation *packages* too. The `EI_EXPOSE_REP`/`EI_EXPOSE_REP2` example patterns are **confirmed verbatim** in the pinned engine `findbugs.xml` (category `MALICIOUS_CODE`) at FLOOR-C build. The Sonar `//NOSONAR` rule-blind behavior and the "Won't Fix"→"Accepted" rename (Server 10.4) + exclusion-from-ratings wording are **confirmed** against `docs.sonarsource.com` (web-verify 2026-06-28, dated-at-use as of Server 2026.1 LTA — SaaS/rolling). The plugin defaults and the baseline-parameter name are **confirmed** (web-verify 2026-06-28) against the pinned plugin docs: `maven-checkstyle-plugin` 3.6.0 `failOnViolation` Default `true` / `violationSeverity` Default `error` (`check-mojo.html`); PMD 7.25.0 CLI marker `--suppress-marker` (legacy `-suppressmarker`); the SpotBugs Maven "baseline files" parameter is `excludeBugsFile`/`excludeBugsFiles` (List form **Since: 4.7.1.0**) — no parameter literally named `baselineFiles` (`spotbugs-maven-plugin` 4.10.2.0 gapidocs). Tool versions + GAVs were RESOLVED on Maven Central 2026-06-27. The last two doc-identity atoms — the SpotBugs `Bug` `code`/`category` attribute forms + the `Package` filter element, and Checkstyle `@SuppressWarnings` name-normalization — are **RESOLVED** (web-verify 2026-06-28): the `Bug` `code`/`category` attributes and the `Package` `name` attribute are confirmed verbatim against `spotbugs.readthedocs.io/en/stable/filter.html` (4.10.2), and the Checkstyle normalization ("case-insensitive … dotted prefix or 'Check' suffix removed") against `checkstyle.org/filters/suppresswarningsfilter.html` (page Version 13.6.0, matching the pin). With these closed, every atom in this chapter is now source-traced (`09-flags/39_*` fully RESOLVED).

...
Companion module: 08-companion-code/11_null_safety_optional/ — ✅ EXAMPLE-BUILD GREEN (mvn -Pquality verify on JDK 21, 2026-06-27: 16 tests pass, Checkstyle 10.26.1 + SpotBugs 0 findings, bytecode 65/Java 21). Spec at foot.
Verified against SOURCE-PIN: 2026-06-27 (corrected pin). Sources: JDK 21 API (Optional design-intent verbatim + value-based; Objects.requireNonNull family — module-confirmed); JEP 358 (Helpful NullPointerExceptions, Release 14, on-by-default since JDK 15, active at anchor — openjdk.org JEP 358 web-verified 2026-06-27, module-confirmed on JDK 21); JLS SE 21 §4.1 null type (pinned edition); JSpecify 1.0.0 (@Nullable/@NonNull/@NullMarked/@NullUnmarked, TYPE_USE — GAV org.jspecify:jspecify:1.0.0 module-confirmed); Checker Framework Nullness Checker 4.2.0 (@PolyNull/@MonotonicNonNull/@KeyFor; Init+MapKey sub-checkers; soundness-sentence quote WEB-VERIFIED verbatim 2026-06-28 against CF Manual v4.2.0 §3.1, checkerframework.org/manual/#nullness-checker); JSR-305 javax.annotation (declaration, DORMANT May 2012, JPMS split-package — status verified); NullAway 0.13.4 (Error Prone plugin, req JDK 17 + EP 2.36.0+; -Xep:NullAway:ERROR / AnnotatedPackages / JSpecifyMode; FSE'19 overhead 1.15x vs 2.8x Eradicate / 5.1x CFNullness + "never due to..." quote — WEB-VERIFIED 2026-06-28 against arXiv:1907.02127 §8.2/Fig.4 + abstract); Effective Java 3e Items 54/55 (paraphrased); Sonar java:S3655/S2789/S2259; SpotBugs NP_*/RCN_*; Error Prone NullableOptional/OptionalNotPresent/ReturnMissingNullable (IDs cited; defaults not asserted).
...
⚠ Residual verify-at-pin (genuinely unverifiable / deferred): Sonar/SpotBugs/Error Prone rule default-activation state. (NullAway FSE'19 overhead figures + "never due to..." quote — RESOLVED, web-verified 2026-06-28 against arXiv:1907.02127. Checker Framework soundness-sentence quote — RESOLVED, web-verified verbatim 2026-06-28 against CF Manual v4.2.0 §3.1, checkerframework.org/manual/#nullness-checker.) ⚠ AHEAD-OF-PIN: Spring 7/Boot 4 + IntelliJ 2025.3 JSpecify adoption; Valhalla null-restricted types. (NullAway 0.13.4 is now the pin, not ahead of it.)
DRAFT v1 — gates manual; four-levers layered-defense + family×guarantee shapes; EXAMPLE-BUILD green (JDK 21).
-->
...
![Figure 9.1 — Null-safety: four levers of layered defense. Each lever catches the null the earlier one cannot reach; detection moves left (earlier) across the lifecycle.](../05-figures/11_null_safety_optional/fig11_1.png)
...
- **The Checker Framework Nullness Checker** runs as a `javac` annotation processor and offers a *soundness* guarantee, stated verbatim in its manual: "If the Nullness Checker type-checks your program without errors, then your program will not crash with a NullPointerException that is caused by misuse of null in checked code."[^cf-nullness-guarantee] It pays for that with annotation effort and build time, and adds sub-checkers a bare `@Nullable` cannot replace: an Initialization Checker (non-null fields set in the constructor) and a Map Key Checker (`@KeyFor`, so `map.get(k)` types non-null when `k` is a known key).

[^cf-nullness-guarantee]: The Checker Framework Manual, version 4.2.0, §3.1 "What the Nullness Checker guarantees" (Nullness Checker chapter). https://checkerframework.org/manual/#nullness-checker — verified verbatim 2026-06-28.
...
- **NullAway** picks a *modular, optimistic* proxy. It checks one method at a time, assumes unannotated code is non-null, and assumes callees do not mutate. That optimism is unsound. A real NPE *can* slip through from a mutating callee or an unannotated return.

  NullAway's own FSE'19 paper measured the cost of that optimism and found it well-targeted. On a corpus of production crash data for widely-used Android apps, the remaining NPEs traced to three sources: unchecked third-party libraries (64%), deliberate error suppressions (17%), and reflection or other post-checking code modification (17%). They were "never due to NullAway's unsound assumptions for checked code."[^nullaway-fse19]

  The optimism also buys speed. The same paper reports an average build-time overhead of 1.15× a normal build, against 2.8× for Eradicate and 5.1× for the Checker Framework Nullness Checker, the two comparable tools it benchmarks. Each figure is sourced to NullAway's own paper, not to a rival's docs.
...
- **Checker Framework Nullness Checker 4.2.0** (SOURCE-PIN.md §2) — `@PolyNull`/`@MonotonicNonNull`/`@KeyFor`; Initialization + Map Key sub-checkers; `javac -processor nullness`. The soundness-guarantee sentence is quoted verbatim from the Checker Framework Manual v4.2.0, §3.1 "What the Nullness Checker guarantees" (https://checkerframework.org/manual/#nullness-checker) — web-verified byte-exact 2026-06-28; the manual sits outside the pinned local copy, so the URL + section are cited directly.

...
/**
 * A minimal in-memory test double for the {@code java.sql} surface the SQL examples use, so the
 * module exercises {@link CustomerLookup} and {@link VulnerableCustomerLookup} without a JDBC driver
 * or a live database. It records the SQL text each lookup builds and the single bound value, which is
 * what the injection tests assert on: the fix's query text is the constant {@code ?} form with the
 * input bound, while the concatenated form folds the input into the text.
 *
...
final class FakeConnection implements InvocationHandler {
    ...
    @Override
    public Object invoke(Object target, java.lang.reflect.Method method, Object[] args) {

...
/**
 * A minimal in-memory test double for the {@code java.sql} surface the lookups use, so the module
 * exercises {@link ReviewedLookup} and {@link AiDraftedLookup} without a JDBC driver or a live
 * database. It records the SQL text each lookup builds and the single bound value, which is what the
 * injection tests assert on: the reviewed fix's query text is the constant {@code ?} form with the
 * input bound, while the AI draft's concatenated form folds the input into the text.
 *
 * <p>The wide JDBC interfaces are backed by {@link Proxy} handlers so only the handful of methods the
 * lookups call carry behaviour; any other call returns a JDBC-shaped default. This is test
 * scaffolding, not production code, and lives only under {@code src/test}.
 */

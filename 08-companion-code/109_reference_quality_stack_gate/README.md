...
The **format layer is shown as a reference config, not wired live**: SOURCE-PIN's "Spotless 8.7.0" is the
project / Gradle-plugin line, and `com.diffplug.spotless:spotless-maven-plugin:8.7.0` does not exist —
...
reactor builds green against; JaCoCo **0.8.15** (SOURCE-PIN pins 0.8.16, which is unpublished — see
